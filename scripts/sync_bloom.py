...
REPORT_PATH_V3 = ROOT / "scratchpad" / "bloom_dryrun_report_v3.txt"  # 인코딩·AI확대·무텍스트(Amd#5)
...
# 단독 \bAI\b(이름 'Aiden' 등엔 단어경계로 미매칭) + 생성기명(Amd#5 ②).
_AI_AUTHOR_RE = re.compile(
    r"(\bai\b|gemini|\bgpt\b|chatgpt|midjourney|dall[- ]?e|stable diffusion"
    r"|ai[- ]generated|generated by ai)",
    re.I,
)
...
def fetch_index_html(book: dict[str, Any]) -> tuple[Optional[str], Optional[str]]:
    ...
    try:
        ...
        resp.raise_for_status()
        # HTTP 헤더에 charset 부재 → requests가 ISO-8859-1 오판(mojibake). meta는 UTF-8.
        resp.encoding = "utf-8"  # Amd#5 ① 인코딩 정정
...
def extract_en_text(page_html: str) -> str:
    ...
    for attrs, inner in _EDITABLE_RE.findall(page_html):
        if re.search(r'\blang="en"', attrs, re.I):
            t = re.sub(r"<[^>]+>", " ", inner)
            t = html.unescape(t)  # Amd#5 ① HTML 엔티티 방어(&#8217; 등)
...
def build_bloom_manifest(book: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "creators": extract_all_creators(index_html),  # AI 전수 스캔용(Amd#5 ②)
    }
...
def extract_all_creators(index_html: str) -> list[str]:
    """책 HTML의 모든 data-creator 값(중복 제거) — AI 탐지 전수 스캔용(Amd#5 ②)."""
    seen: list[str] = []
    for c in _DATA_CREATOR_RE.findall(index_html):
        c = c.strip()
...
def is_ai_generated(res: dict[str, Any]) -> bool:
    """
    AI생성 콘텐츠(Amd#3 ① + Amd#5 ② 확대 자동제외).
    모든 data-creator 값 중 하나라도 AI 시그널이거나, 매니페스트 이미지명이 생성기 패턴.
    """
    creators = res.get("creators") or []
    if res.get("author"):
        creators = [*creators, res["author"]]
    if any(_AI_AUTHOR_RE.search(c) for c in creators):
        return True
...
def run_full_dryrun(sleep: float = 0.1) -> str:
    ...
    for i, b in enumerate(candidates, 1):
        ...
        try:
            ...
            continue
        if res.get("ok"):
            # 자동제외 ①-b: AI생성 콘텐츠(Amd#3 ① + Amd#5 ② 확대).
            if is_ai_generated(res):
                ai_excluded.append(res)
            # gate②: 영어 텍스트 0면(이미지-only) 스킵(Amd#5 ③).
            elif res.get("text_count", 0) == 0:
                notext_excluded += 1
            else:
    ...
    L.append(f"    − 자동제외[AI생성] (Amd#3①·#5②): {len(ai_excluded)}")
    L.append(f"    − 자동제외[테스트물] (Amd#3①)  : {len(test_excluded)}")
    L.append(f"    − gate②[무텍스트] (Amd#5③)     : {notext_excluded}")
    ...
    report = "\n".join(L)
    REPORT_PATH_V3.parent.mkdir(parents=True, exist_ok=True)
    REPORT_PATH_V3.write_text(report, encoding="utf-8")
    return report
...
def run_execute(
    ...
    for b in candidates:
        ...
        if res["page_count"] < GATE1_MIN_PAGES:
            skip["gate①:1p이하"] += 1
            continue
        # gate②: 영어 텍스트 0면(이미지-only) — Amd#5 ③
        if res.get("text_count", 0) == 0:
            skip["gate②:무텍스트"] += 1
            continue
        is_review = bool(res.get("review"))
        if is_review and skip_review:
            skip["검수리스트(skip)"] += 1
            continue
        ...
        try:
            ...
            skip["Storage실패"] += 1
            print(f"  [Storage 실패] {source_id}: {type(exc).__name__}: {str(exc)[:120]}")
            continue
        try:
        ...
        if row and len(samples) < 5:
            samples.append((row, content_url))
        if not is_review and len(clean_books) < 5 and row:
    ...
    print()
    print("  [표본 5권] 적재 row:")
    for row, murl in samples:
        rv = row.get("review") if isinstance(row, dict) else None
        print(f"    · id={row.get('id')} is_active={row.get('is_active')} license={row.get('license')}")
        print(f"      title={row.get('title')[:55]!r}")
        print(f"      cover={row.get('cover_url')}")
        print(f"      manifest={murl}")
    print()
    print("  [팀장 검수용 — 검수리스트 비해당(깨끗한) 책 3권]:")
    for bid, title, murl in clean_books[:3]:
