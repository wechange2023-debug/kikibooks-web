-- 04_merge_step2_bloom.sql — step2 (bloom) 본 테이블 머지
...
--
-- 규모: 142권 / 1616행 (page 1474 + cover 142)
...
--   · 모든 검사·삽입은 source_platform = 'bloom' 로 한정된다.
...
-- ============================================================
-- 기대: staged_rows 1616 / staged_books 142 / page 1474 / cover 142
SELECT count(*)                                 AS staged_rows,
       count(DISTINCT source_id)                AS staged_books,
       count(*) FILTER (WHERE kind = 'page')    AS page_rows,
       count(*) FILTER (WHERE kind = 'cover')   AS cover_rows
  FROM public.book_audio_staging_708 WHERE source_platform = 'bloom';

-- ============================================================
-- [2] 매핑 게이트 (fail-closed)
-- ============================================================
-- 기대: mapped_books 142 / unmapped_rows 0 / mismatched_rows 0
SELECT
  (SELECT count(DISTINCT b.id)
    ...
    WHERE s.source_platform = 'bloom')          AS mapped_books,
  (SELECT count(*) FROM public.book_audio_staging_708 s
    WHERE s.source_platform = 'bloom' AND NOT EXISTS (
  ...
  (SELECT count(*) FROM public.book_audio_staging_708 s JOIN public.books b
       ON b.source_platform = s.source_platform AND b.source_id = s.source_id
    WHERE s.source_platform = 'bloom' AND b.id <> s.manifest_book_id)
                                                     AS mismatched_rows;
...
BEGIN
  SELECT count(*), count(DISTINCT source_id) INTO v_rows, v_books
    FROM public.book_audio_staging_708 WHERE source_platform = 'bloom';
  IF v_rows <> 1616 THEN
    RAISE EXCEPTION 'STOP: staging bloom 행 수 % ≠ 기대 1616 — 청크 적재 미완. 머지 중단', v_rows;
  END IF;
  IF v_books <> 142 THEN
    RAISE EXCEPTION 'STOP: staging bloom 권 수 % ≠ 기대 142 — 머지 중단', v_books;
  END IF;
...
SELECT
  (SELECT count(*) FROM public.book_audio_staging_708 s
    ...
    WHERE s.source_platform = 'bloom')          AS conflict_unique,
  (SELECT count(*) FROM public.book_audio_staging_708 s
     JOIN public.book_audio a ON a.audio_path = s.audio_path
    WHERE s.source_platform = 'bloom')          AS conflict_audio_path;
...
-- ============================================================
-- [4] book_audio INSERT — 142권 / 1616행
-- ============================================================
...
--   (= 적재 전 655권 7469행 + 본 step 142권 1616행)
SELECT count(DISTINCT book_id) AS danielle_books,
       count(*)                               AS danielle_rows,
       count(*) FILTER (WHERE kind = 'page')  AS page_rows,
       count(*) FILTER (WHERE kind = 'cover') AS cover_rows
...
-- 본 step 적재분만 재확인 (기대: step_books 142 / step_rows 1616 / page 1474 / cover 142)
...
-- 표지 누락 권 (기대: 0) — 본 step 142권 중 cover 행이 없는 책
...
-- ┌──────────────────────────────────────────────────────────┐
-- │ step2 (bloom) 기대값 대조표 — 팀장 확인용
-- ├──────────────────────────────────────────────────────────┤
-- │ [0] 적재 전  danielle books  655 rows  7469 page  6814 cover  655
-- │            Ruth     books   44 rows   574
-- │ [1] staging rows  1616 books  142 page  1474 cover  142
-- │ [2] 매핑     mapped_books  142 · unmapped 0 · mismatched 0
-- │ [3] 충돌     conflict_unique 0 · conflict_audio_path 0
-- │ [4] INSERT  1616행
-- │ [5] 적재 후  danielle books  797 rows  9085 page  8288 cover  797
-- │            step 적재분 books  142 rows  1616
...
-- │ 다음: 05_merge_step3_bookdash.sql
-- └──────────────────────────────────────────────────────────┘
