-- 03_merge_step1_asb.sql — step1 (african_storybook) 본 테이블 머지
...
--
-- 규모: 527권 / 5855행 (page 5328 + cover 527)
...
--   · 모든 검사·삽입은 source_platform = 'african_storybook' 로 한정된다.
...
-- [0] 사전 상태 스냅샷
-- ============================================================
-- 기대: danielle_books 128 / danielle_rows 1614 / page 1486 / cover 128
...
-- ============================================================
-- 기대: staged_rows 5855 / staged_books 527 / page 5328 / cover 527
SELECT count(*)                                 AS staged_rows,
       count(DISTINCT source_id)                AS staged_books,
       count(*) FILTER (WHERE kind = 'page')    AS page_rows,
       count(*) FILTER (WHERE kind = 'cover')   AS cover_rows
  FROM public.book_audio_staging_708 WHERE source_platform = 'african_storybook';

-- ============================================================
-- [2] 매핑 게이트 (fail-closed)
-- ============================================================
-- 기대: mapped_books 527 / unmapped_rows 0 / mismatched_rows 0
SELECT
  (SELECT count(DISTINCT b.id)
    ...
    WHERE s.source_platform = 'african_storybook')          AS mapped_books,
  (SELECT count(*) FROM public.book_audio_staging_708 s
    WHERE s.source_platform = 'african_storybook' AND NOT EXISTS (
  ...
  (SELECT count(*) FROM public.book_audio_staging_708 s JOIN public.books b
       ON b.source_platform = s.source_platform AND b.source_id = s.source_id
    WHERE s.source_platform = 'african_storybook' AND b.id <> s.manifest_book_id)
                                                     AS mismatched_rows;
...
BEGIN
  SELECT count(*), count(DISTINCT source_id) INTO v_rows, v_books
    FROM public.book_audio_staging_708 WHERE source_platform = 'african_storybook';
  IF v_rows <> 5855 THEN
    RAISE EXCEPTION 'STOP: staging african_storybook 행 수 % ≠ 기대 5855 — 청크 적재 미완. 머지 중단', v_rows;
  END IF;
  IF v_books <> 527 THEN
    RAISE EXCEPTION 'STOP: staging african_storybook 권 수 % ≠ 기대 527 — 머지 중단', v_books;
  END IF;
...
SELECT
  (SELECT count(*) FROM public.book_audio_staging_708 s
    ...
    WHERE s.source_platform = 'african_storybook')          AS conflict_unique,
  (SELECT count(*) FROM public.book_audio_staging_708 s
     JOIN public.book_audio a ON a.audio_path = s.audio_path
    WHERE s.source_platform = 'african_storybook')          AS conflict_audio_path;
...
BEGIN
  SELECT count(*) INTO v FROM public.book_audio_staging_708 s
    ...
    JOIN public.book_audio a
      ON a.book_id = b.id AND a.kind = s.kind
     AND a.page_index = s.page_index AND a.voice = 'danielle'
   WHERE s.source_platform = 'african_storybook';
  IF v <> 0 THEN
...
-- ============================================================
-- [4] book_audio INSERT — 527권 / 5855행
-- ============================================================
...
-- [5] 사후 검증
-- ============================================================
-- 기대: danielle_books 655 / danielle_rows 7469 / page 6814 / cover 655
--   (= 적재 전 128권 1614행 + 본 step 527권 5855행)
SELECT count(DISTINCT book_id) AS danielle_books,
       count(*)                               AS danielle_rows,
       count(*) FILTER (WHERE kind = 'page')  AS page_rows,
       count(*) FILTER (WHERE kind = 'cover') AS cover_rows
...
-- 본 step 적재분만 재확인 (기대: step_books 527 / step_rows 5855 / page 5328 / cover 527)
...
-- 표지 누락 권 (기대: 0) — 본 step 527권 중 cover 행이 없는 책
...
-- ┌──────────────────────────────────────────────────────────┐
-- │ step1 (african_storybook) 기대값 대조표 — 팀장 확인용
-- ├──────────────────────────────────────────────────────────┤
-- │ [0] 적재 전  danielle books  128 rows  1614 page  1486 cover  128
-- │            Ruth     books   44 rows   574
-- │ [1] staging rows  5855 books  527 page  5328 cover  527
-- │ [2] 매핑     mapped_books  527 · unmapped 0 · mismatched 0
-- │ [3] 충돌     conflict_unique 0 · conflict_audio_path 0
-- │ [4] INSERT  5855행
-- │ [5] 적재 후  danielle books  655 rows  7469 page  6814 cover  655
-- │            step 적재분 books  527 rows  5855
