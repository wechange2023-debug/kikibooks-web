...
## 6. 이번 회차 대상 예외 — `catch-that-cat` (기록 시점 2026-08-18)

대상 **18권 33면** 중 **`catch-that-cat` 1권 1면(`page_no` = 5, `page_index` = 4)** 은
이번 회차 실검수 대상에서 제외한다.

- **사유(실측)**: 이 책은 `book_audio` 행을 보유한다(`has_audio = true`). 따라서 상세 화면의
...
- **팀장 확인 사항**: 이 책은 **지난주 교정이 이미 진행된 도서**다.
- **미결**: 그 텍스트 교정이 TTS 합성보다 **나중인지**는 **판별 중**이다. 나중이면 하이라이트
  좌표가 밀려 재합성 대상이 된다(ADR-0058 Amendment #1 G6).
  판별 조회: `scripts/sql/trackB_catchthatcat_audit.sql`(읽기 전용).

> **따라서 이번 회차 실검수 대상은 17권 32면이다.**
> (전체 대상 18권 33면 − `catch-that-cat` 1권 1면)
>
> `catch-that-cat`의 처리는 선후 판별 결과가 나온 뒤 별도로 정한다. **검수 화면에서 이 책의
> 텍스트를 지금 고치지 말 것** — 오디오 보유 도서의 본문 수정은 그 자체가 재합성 발동
> 조건이다(ADR-0058 Amd#1 G6).
