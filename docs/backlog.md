...
- **신규 발생 사항(진행 상태)**:
  ...
  - (v) **Book Dash 재소싱 5권 점검 (STEP 3 후속, ADR-0032)**: STEP 3 표지 이관 중 `bookdash.github.io` 미러가 죽은 링크(404)였던 5권을 WP featured_media로 재소싱해 표지는 복구·이관 완료. 그러나 이관 후 별개 증상 관찰 — **① 목록 노출**: 재소싱 5권 중 라이브러리 검색 노출 **1건만 확인**(`i-can-dress-myself`), 나머지 4건(`hugs-in-the-city`·`it-wasnt-me`·`katiitis-song`·`the-lion-who-wouldnt-try`) **노출 여부 미확인**. **② 리더 진입**: 5권 **모두 리스트→책읽기 진입 실패** 관찰. **표지(`cover_url`) 트랙과 무관** — 원인은 본문 `content_url`/`content_type`·리더 연결 또는 목록 노출 조건(제목 검색어·`is_active`·카테고리) 쪽으로 추정, **별도 정찰 필요(미실측)**. **STEP 3(표지)와 독립 트랙.** 감사기록=`scratchpad/step3_resourced.csv`, ADR-0032 「실행 결과 및 정정」(c).
