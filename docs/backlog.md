...
- **신규 발생 사항(진행 상태)**:
  - (a) **[완료 2026-08-07 — 표기 확정: `Hello Kiki`]** 플랫폼 명칭 변경 결정(2026-06-12): 키키북스 → 헬로키키. **UI·메타데이터 전수 반영 완료.** 팀장 확정 표기는 **`Hello Kiki`(영문, 두 단어 띄어쓰기) 단일**이며 한글 병기 없음 — 구표기 `Kikibooks`·`키키북스`·`HelloKiki`(붙임)는 전부 폐기. 단일 출처 `lib/brand.ts`(`BRAND_NAME`·`BRAND_TAGLINE`) 신설 후 랜딩 copy·metadata 20여 곳·OG 이미지·legal 문안·헤더 로고가 모두 이 상수를 참조한다. `app/`·`components/`·`lib/` 구표기 잔여 0건(grep 검증). `docs/`·ADR·PLAN.md의 구표기는 **이력 문서라 무접촉**. (도메인 `hellokiki.co.kr`은 이미 명칭과 정합.) ※ 아래 원문 보존: ~~**부분 반영**(신규 v2 문서 텍스트는 HelloKiki: PLAN v2.0·PRD v2·license v1.1 신규 문장). **전수 반영 잔여**(backlog·README·UI 라벨·design-system·메타데이터·legal 문안 등) 전수 조사 후 반영 필요.~~
  ...
  - (y) **책 카드 4벌 → 공용 컴포넌트 통합(홈·라이브러리 회귀 검증 포함)** — 2026-08-07 ADR-0024 마이페이지 작업 중 발견. `BookCoverCard`(landing, href='/signup' 하드코딩)·`RecommendationCard`(home, 파일 로컬)·`LibraryBookCard`(library, 2026-08-07 export만 추가)·`ShowcaseBookCard`(showcase, 검수용) 4벌이 거의 동일 마크업. 마이페이지는 카드 5벌째를 만들지 않으려 A안(export 1줄)으로 우회했고, 공용 `components/book/book-card.tsx` 추출은 이미 검수 통과한 홈·라이브러리 회귀 검증이 필요하므로 별도 리팩터링 작업 단위로 분리(B안 이연, C안 기각).
  - (z) **`isAdminPreview` 5줄 래퍼 2파일 중복 — 공용 모듈 추출 후보** (2026-08-20 큐 D-5c 기록, ADR-0063 O-D5-1 관련). `app/(reader)/book/[id]/page.tsx:95-102`와 `app/(reader)/book/[id]/read/page.tsx:124-131`에 같은 래퍼가 2벌 있다. **보안 리스크는 아니다** — 관리자 판정 단일 출처 `lib/admin/gate.ts` `assertAdmin`을 양쪽이 그대로 호출하며(판정 기준 이원화 0건), 중복은 `isPreviewParamValue` 검사 + `assertAdmin` 호출을 감싼 얇은 래퍼뿐이다. 세 번째 호출부가 생기는 시점에 `lib/book/`으로 추출한다. (참고: `lib/book/reading-session.ts:79` `isAdminPreviewEntry`는 Referer 기반의 **다른** 신호원이라 통합 대상이 아니다.)
  - (aa) **O-10 실측 근거 — `reading_sessions` 같은 자녀·같은 책 완독 3행 확인** (2026-08-20 기록, 큐 J 유지). `book_id = 007ce3fc-f7a7-4db5-ab47-2aa3b3a082d4` 한 권에 완독 행이 3건(`2026-07-01 06:51:08` / `06:51:25` / `06:55:17`). **팀장이 Supabase SQL로 직접 조회한 값**이다. 06:51:08→06:51:25는 17초 간격이라 재열람이 아닌 중복 적재로 보인다. O-10(중복 완독 행 정책)의 실측 근거로 편입하며, 처리 방침 결정은 **큐 J에서 계속 다룬다**(본 항목은 근거 보존 목적, 코드 변경 0건). 관련: ADR-0059(reading_sessions 행 상한).
  - (ab) **카드형 섹션 h2 위계 통일 — 별도 큐 후보** (2026-08-20 큐 D-5c에서 실행하지 않음 확정, 팀장 판정). `docs/design-system.md:456` §6.3은 섹션 타이틀을 `text-h2`(22px)로 규정하지만, 구현의 **카드형 정보 섹션**은 `font-display text-body font-semibold`(16px)를 쓴다 — `components/home/streak-chart.tsx:63` · `components/home/level-selector.tsx:76` · `components/book/inactive-book-notice.tsx:61` · `app/(reader)/mypage/page.tsx` `SECTION_TITLE_CLASS`. 문서-구현 불일치이나 **한 파일만 고치면 같은 화면 안에서 어긋난다**: 마이페이지 ②는 `StreakChart` 그 자체라, 마이페이지만 22px로 올리면 ②만 16px로 남는다. 통일하려면 위 4곳을 동시에 바꿔야 하고, 그 순간 메인 `/`(`components/main/member-main.tsx:127`·`:142`)와 책 상세 안내 화면까지 함께 흔들려 회귀 검수 범위가 커진다. → 별도 큐로 분리한다. 결정 필요 사항: `text-h2`(문서에 맞춤) / `text-h3`(§2.2 "카드 타이틀" 역할에 맞춤) / 현행 유지 + 문서 정정 중 택일.
  - (ac) **Vercel CLI 로그인 계정 가드 부재** (2026-08-20 큐 D-5c push 중 관찰). `vercel whoami` 결과가 **`bigwavecto`** 로, git 신원 `crspiegel`과 **불일치**한다. **현재는 무영향** — 저장소에 `.vercel` 링크가 없어(`.vercel/project.json` 부재) CLI 배포 경로 자체가 성립하지 않고, 배포는 전부 Vercel GitHub 연동으로만 일어난다. 문제는 **`vercel link` 등으로 CLI 직접 배포를 도입하는 순간** 드러난다 — git push에는 `scripts/git_hooks/pre-push`(ADR-0040, fail-closed)가 계정을 막지만 **CLI 배포에는 같은 성격의 가드가 없어** 잘못된 계정으로 프로덕션에 나갈 수 있다. CLI 배포 도입 시 선행 조건: ① `vercel switch`/재로그인으로 계정 일원화 ② pre-push와 동등한 계정 확인 게이트 마련. 도입 전까지는 조치 불필요.
  - (ad) **자녀 0명에게 즐겨찾기 하트를 "렌더 + 등록 유도"로 바꾸는 안 — 베타 후 개선 후보** (2026-08-20, ADR-0064 **D5**에서 미채택). 현재는 자녀 0명이면 하트를 **미렌더**한다(`app/(reader)/book/[id]/page.tsx` — 책 상세가 원래 채택하던 선례를 ADR-0064가 정책으로 승격). 대안은 하트를 보여주되 누르면 "자녀 프로필을 만들면 담을 수 있어요" 같은 안내 + 온보딩 링크를 띄우는 것이다. **전환율에는 유리할 수 있으나** 문구 신설 + 판단 분기가 붙고, 무엇보다 ADR-0064 §원칙 절("기록계 기능은 자녀 0명에게 **일괄** 숨김 — 화면별 개별 판단 금지")에 예외를 만든다. 하트만 예외로 두면 완독 버튼·스트릭·추천은 왜 아닌지에 매번 답해야 한다. → 베타 후 전환 데이터를 보고 **기록계 기능 전체**에 대해 한 번에 재검토한다(하트만 따로 바꾸지 않는다). 같은 성격의 미채택 건: ADR-0064 **O-K4**(책 상세 사전 안내 미채택).
  - (ae) **오디오 리더 하단 바 — 자녀 0명일 때 완독 버튼 슬롯 래퍼가 빈 채 잔존** (2026-08-20 큐 K-2 구현 중 관찰, ADR-0064 D4 부수). `app/(reader)/book/[id]/read/page.tsx`가 자녀 0명이면 `finishSlot`에 `null`을 넘기는데, 슬롯을 감싼 래퍼 `<div>`는 `components/book/audio-reader.tsx`가 소유해 **무조건 렌더된다** — 그 결과 하단 1행 오른쪽에 `cursor-not-allowed` + `title="마지막 장까지 보면 완독할 수 있어요"` 툴팁만 붙은 **빈 상자**가 남는다. **무해하다**: 실측상 시각적으로는 빈 여백이고 클릭 대상도 없다(비오디오 경로는 `<footer>` 자체를 조건부로 내려 이 현상이 없다). 정리하려면 래퍼를 `finishSlot` 존재 여부로 감싸야 하는데 그것은 **공유 컴포넌트 `AudioReader` 수정**이며, K-2 지시서가 공유 컴포넌트 무접촉을 못박아 실행하지 않았다. → 별도 큐 후보. 손볼 때 `finishSlot`을 optional로 두는 현재 계약(`audio-reader.tsx` `finishSlot?: ReactNode`)을 그대로 쓰면 호출부 3표면(html·asb·audio) 무변경으로 끝난다.
  - (af) **검증 절차 표준 — `pnpm build`와 `pnpm dev`가 `.next`를 공유하지 않는다** (2026-08-20 채택, 크래시 재발 방지). **표준: `build` → **`.next` 삭제** → `dev` 재기동.** 근거는 2026-08-20 K-2 검증 중 실측된 크래시다 — 프로덕션 빌드 직후 같은 `.next` 위에 dev를 올리자 `⨯ Failed to generate static paths for /book/[id]` + `Jest worker encountered 2 child process exceptions, exceeding retry limit`가 **10회** 발생하고 책 상세 3건이 500을 냈다. `generateStaticParams`는 **소스에 0건**이고 해당 라우트는 `force-dynamic`이라 코드 원인이 아니었으며, `.next` 삭제 + 클린 재기동으로 **재현 0건**이 됐다(WorkerError 0 · 500 0). 종전 이력("dev 실행 중 build가 `.next`를 손상")의 **거울상**이므로 양방향 모두 금지로 정리한다. **오케스트레이터 지시서 표준에도 반영**한다 — 검증 절차를 적을 때 "build 전 dev 종료"만이 아니라 "build 후 `.next` 삭제"까지 명시할 것. 기여 요인으로 메모리 압력(실측 15.72 GB 중 여유 3.08 GB, 80.4% 사용)이 있었으나 OOM 메시지는 0건이라 단독 원인으로 보지 않는다.
  - (ag) **표지 낭독 사양 통일 — 생성기 2경로 정리** (2026-08-21 R-1b에서 발견, 비기 판정 "현상 유지 + 백로그"). `AAAAAHHH!!!! Mmawe!`(book_dash) 표지 오디오만 **저자 표기가 없다** — marks가 `[AAAAAHHH, Mmawe]` 2개뿐이다. 같은 코호트의 다른 book_dash 책은 전부 `제목 + "Created by 저자"`다(실측: `A House for Mouse`·`A Tiny Seed`·`Amazing Daisy` 등 marks에 `Created`·`by`·저자명 포함). 원인은 **생성기가 둘로 갈린 것**이다 — `scripts/tts_pilot/run_tts_full708.py:228`은 `cover_text()`(제목+저자)를 쓰고, `scripts/tts_pilot/gen_cover_danielle.py:12-13`은 *"구 44권 배치는 \"{title}. Created by {author}.\" 였으나 이번 확정 사양은 제목만이다"*라고 명시한다. 즉 **사양 자체가 두 벌**이며 어느 쪽이 정본인지 문서에 확정된 바가 없다. **무해하다** — 리더는 표지 텍스트로 `books.title`만 표시하고(`lib/book/audio-manifest.ts:70`·`:352`) marks 오프셋이 제목 길이를 넘는 것은 `components/book/highlighted-text.tsx:96`의 범위 가드가 건너뛴다. 저자 낭독 유무는 청취 경험 차이일 뿐 기능·라이선스 문제가 아니다(어트리뷰션 법적 의무는 화면의 `AttributionBox`가 이행한다). **R-1b에서는 현상 유지**했다 — 문장부호 결함만 고치고 내용 형태(제목만)를 그대로 두었다. 내용 변경은 별도 승인 사항이기 때문이다. → 정리할 때 ① 표지 낭독 정본 사양을 ADR로 확정(제목만 vs 제목+저자) ② 두 생성기를 그 사양으로 일원화 ③ 어긋난 권 재합성 범위 산정, 세 가지를 한 번에 다룬다. 현재 어긋난 것으로 확인된 권은 1권이나 **전수 미조사**다(표지 marks 단어 수로 일괄 판별 가능).
  - (ah) **`/wordplay` 사전 클라이언트 번들(gzip 18.5KB) — 서버측 8개 뜻 축소는 리뉴얼 시 검토** (2026-08-22 E-2c-flip 구현 중 실측, ADR-0065 Amd#4 영향범위 "뜻은 클라이언트에서 조회"의 대가). `lib/wordplay/ko-meanings.ts` 1,741개 전량이 `components/book/word-play.tsx:8` import를 타고 라우트 청크에 실린다 — `.next/static/chunks/app/(reader)/book/[id]/wordplay/page-*.js` 실측 raw 43,135B / gzip 18,528B이고 대부분이 사전이다. 한 화면이 실제로 쓰는 뜻은 카드 수만큼(보통 8개)이므로, `lib/wordplay/get-word-play.ts`가 `WordPlayCard`에 `meaning`을 실어 보내면 사라지는 비용이다. **무해하다** — 완독 후에만 닿는 라우트이고 초기 로드 경로가 아니다. 다만 그 변경은 `WordPlayCard` 계약 변경이라 Amd#4 영향범위("get-word-play.ts 무변경")를 고쳐야 하므로 디자인 리뉴얼 트랙에서 함께 다룬다.
  - (ai) **`getClaims()` 전환(Auth 왕복 제거·children 병렬, ADR-0067 A1-2 이관): 기대 절감 ~0.1초, 7파일 타입 변경 — 리뉴얼 후 재평가** (2026-08-22 기록). 화면당 Auth 왕복이 2회다 — `middleware.ts:22`와 각 page의 `auth.getUser()`. 페이지 쪽을 `getClaims()`로 바꾸면 JWKS 로컬 검증이라 왕복이 사라지고(프로젝트 JWKS 실측 `alg: ES256`·`kty: EC` = 비대칭 서명키), `user.id`가 네트워크 없이 잡히므로 ADR-0067 D3이 못 이룬 **children 쿼리 병렬**까지 같이 달성된다. 대가는 반환이 `user` 객체가 아니라 클레임이라 **호출부 전량 타입 변경**이다(`app/page.tsx:98` 등 5화면 + `lib/supabase/*`).
...
- **다음 후보 작업**: ① **【착수】순서4 스키마 마이그레이션**(`002_*.sql`: CHECK+트리거에 `cc-by-3-0` 추가, ADR-0022 선행) + **GDL 심화 sync**(`sync_gdl` ALLOWED에 cc-by-3-0 추가·`cc-by-sa-4-0-2` 정규화 → 842→~937) ② ~~HelloKiki 명칭 **전수 반영 잔여**(backlog·README·UI 등)~~ → **[완료 2026-08-07]** 표기 `Hello Kiki` 확정 + `lib/brand.ts` 단일 출처화, UI·metadata 전수 반영(위 (a)항 참조). README 등 문서류는 이력이므로 무접촉 ③ 작업1 level·keyword URL 동기화(코드) ④ 307→308 승격(대시보드, 수일 후) ⑤ 자체 e-book 23권(~960권) ⑥ Phase 1.5 트랙B **TTS·캐릭터 AI 구현 ADR**(ADR-0023 후속).
