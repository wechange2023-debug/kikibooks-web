...
컴포넌트는 카피 상수를 직접 import하지 않는다. `/` 페이지가 `getLandingCopy()`를 호출해 그 결과를 하위 컴포넌트에 **props로** 내려준다. phase-13b에서 Admin이 카피를 DB(`landing_copy` 테이블)로 관리하게 되면 `getLandingCopy()`의 본문만 DB 조회로 교체하면 되고, `LandingCopy` 인터페이스와 컴포넌트 props는 그대로다 — **컴포넌트 수정 0건**으로 교체된다.
