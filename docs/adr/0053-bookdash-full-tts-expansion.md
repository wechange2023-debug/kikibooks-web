# ADR-0053: Book Dash 전체 TTS 확장 — 검수 게이트 폐지·키 보관 정책·입구 정제·비용 통제

## Status

Accepted (2026-07-27) / 기준 HEAD `2e79b95`
**Amendment #1(2026-07-27)로 O1·O2 해소** — 보이스 `Danielle / long-form`, 표지 포함 확정.
**Amendment #2(2026-08-08)** — 확정 사양을 ASb·Bloom 669권으로 확대 적용, `run_tts_fullbatch.py`
"팀장 확정 대기" 주석 해소 명시. O3(환율)은 여전히 미확정.
**Amendment #3(2026-08-08)** — 적용 범위를 **669권 6,899면 → 708권 7,368면**으로 재확대.
Book Dash html **활성 39권 469면**(= 오디오 보유 34권 409면 **재생성** + 무오디오 5권 60면
**신규 합성**)을 편입한다(팀장 승인). 대상 39권은 slug 단위로 확정됐다(ADR-0056 O4·§5-f).
O3은 여전히 미확정.
**2026-08-12 D4 실합성 종결** — 708권 **7,978유닛 / 853,725자 전량 합성 완료, 실패 0**,
무결성 전수 검증 통과(D4-a). 실지출 **$166.18**(크레딧 $115.01 + 실지출 $51.17).
8/12 중단은 AWS 프리 플랜 종료에 따른 계정 정지가 원인이며 유료 전환으로 해소, 과금 영향 없음(D4-b).
다음 단계는 **Storage 업로드 → `book_audio` 적재**이며 둘 다 팀장 실행 영역이다(D4-c).
**2026-08-12 Storage 업로드 폐합** — 15,956키(mp3 7,978 + marks 7,978), 실패 0.
**Amendment #4(2026-08-12)** — 적재 실행 수단으로 일회성 staging 테이블
`book_audio_staging_708`을 경유한다(생성 `00` → 삭제 `07`). SQL Editor 크기 제한 대응이며
합성 사양·범위는 무접촉. 도메인 DDL 변경 0건(Hard Rule 1·2·8 무저촉 근거는 E4).
**Amendment #5(2026-08-13, ADR-0058 D7·D8)** — **D6(실행 경계)를 쓰기 주체 3분류로 개정**한다.
배포 서버의 `book_review.status` 전이가 관리자 인증 가드 하의 좁은 예외로 신설됐다.
D1~D5·합성 사양·범위는 **무접촉**이며 아래 종결(Completed) 표기도 그대로 유지된다 —
본 개정은 D4 실행 트랙을 재개하지 않는다. 문서 반영 2026-08-14.

### 종결 (Completed) — 2026-08-13

**본 ADR의 D4 트랙(합성 → Storage 업로드 → `book_audio` 적재)은 전량 폐합됐다.**
`docs/sql/load708/` **19파일 전량 실행 완료**(팀장 주도), 최종 실측
**`voice='danielle'` 836권 / 9,592행**(`page` 8,756 + `cover` 836).
`02` 전량 게이트·`06` 최종 검증 모두 **verdict PASS**, `07` staging 삭제 완료.
구 `voice='Ruth'` **44권 574행은 무접촉 불변**이다. 상세는 **§E8**, 실행 중 발생한
사고와 재발 방지 규약은 **§E9**.

잔여 미해소는 **O3(원화 환산 환율)** 과 **D4-a 비용 잔차 $4.57** 두 건뿐이며 **둘 다 비차단**이다.
결정문(D1~D6) 자체는 `Accepted` 상태 그대로 유효하며, 본 표기는 **실행 트랙의 종결**을 뜻한다.

## Deciders

팀장, 오케스트레이터

## Related

- ADR-0023 Amd#1(TTS 엔진·보이스·저장 위치), ADR-0029(입수 시점 HTML 엔티티 디코딩),
  ADR-0034(+Amd#1 `kind`, +Amd#2 1-based·성우 층위)(오디오 저장 구조),
  ADR-0050(회전 페이지), ADR-0051(검수 화면), ADR-0052(+Amd#1·#2)(시연용 범위 축소 12권).
- 상위 계획서: `docs/intent/highlight-tts-master-plan.md`.
- 구현: `scripts/tts_pilot/tts_targets.py`, `scripts/tts_pilot/run_tts_fullbatch.py`.

## Context

- ADR-0052는 **7/28 내부 시연**을 위해 TTS 범위를 시범 12권으로 축소했다. 시연 트랙은
  종료됐다 — 12권 `tts_done`, 뷰어·하이라이트 검수 통과.
- 이제 축소 사유(6일 기한)가 사라졌으므로 **Book Dash 전체로 확장**한다. 확장 시 세 가지가
  실제 병목으로 드러났다.
  1. **검수 게이트**: ADR-0052는 `book_review.status='confirmed'`인 권만 TTS 대상으로 삼았다.
     현재 `draft` 140권 / `tts_done` 12권 — 사람이 140권을 선검수해야 한 줄도 못 만든다.
  2. **키 보관**: AWS 자격증명이 팀장 로컬 PowerShell 환경에만 있어, 생성은 항상 팀장이
     수동 실행해야 한다. 향후 관리자 화면의 '음원생성' 버튼으로 옮기려면 서버가 키를 써야 한다.
  3. **비용 가시성**: 실행 전 몇 권·몇 문자·얼마인지 알 방법이 없었다.
- 실측(2026-07-27, 읽기 전용): `book_dash` 209권 / 활성 206권 / `book_text` 보유 152권 /
  `book_audio` 보유 56권(v1 `Ruth` 44 + 시범 `danielle` 12).

## Decision

### D1. 검수 게이트 폐지 — 전량 자동 생성 + 사후 검수

- ADR-0052의 **`confirmed` 상태 게이트를 폐지**한다. `book_review.status`는 더 이상 TTS
  생성의 **선행 조건이 아니다**.
- `book_dash` **활성 도서 전체**를 자동 생성 대상으로 하고, 들어보고 **문제가 발견된 권만
  재생성**하는 사후 검수 방식으로 전환한다.
- 근거: 오디오는 원본을 훼손하지 않는 **파생물**이고, 재생성 단가가 낮다(D4 실측 기준
  권당 $0.01~$0.20). 140권 선검수의 인건비가 재생성 비용을 압도한다.
- **단, 사전 제외 3종은 유지**한다. 이들은 "품질 취향" 문제가 아니라 **낭독하면 뜻이
  무너지는** 결함이라 사후 검수로 걸러도 무의미하다.

  | 제외 | 권수 | 사유 | 출처 |
  |---|---|---|---|
  | 회전 페이지 보유 | 18 | 90° 회전 인쇄로 읽기순서 역전 — 사람이 텍스트를 고쳐야 함 | ADR-0050 D1·D3, `lib/admin/review/rotation-pages.ts` |
  | 제작 메타데이터 오염 | 6 | 편집용 주석 `Story spread N`이 본문에 유입 | 본 ADR D1-a |
  | 이미 오디오 보유 | 12 | `book_audio` 행 존재 — 재생성은 개별 지시로만 | `book_audio` 실측 |
  | (텍스트 없음) | 54 | `book_text` 행 없음 = 낭독 대상 자체가 없음(그중 44권은 v1 배치로 이미 오디오 보유) | 실측 |

- **D1-a. 오염 6권의 확정 근거**: 목록이 코드·문서 어디에도 남아 있지 않아
  `book_text` 전수 스캔으로 **실측 도출**했다. 정규식 `\bstory\s+spread\b`(대소문자 무시)
  히트가 **정확히 6권 / 각 1면**이었다 —
  `and-also`(p09) · `little-goat`(p03) · `look-up`(p01) · `the-rainbow-cloud`(p07) ·
  `where-is-lulu`(p08) · `yes-you-can`(p01).
  - 교차검증: `lib/admin/review/pilot-cohort.ts` 주석("회전 18권·오염 6권 제외 후 알파벳순
    상위 12")을 역산하면 알파벳 선두에서 `and-also` 1권만 빠지는데, 그 권이 위 6권에 속한다.
  - 같은 스캔에서 `you-yes-you` p12 "Almost at the back cover!"도 걸렸으나 **본문 문장(오탐)**
    이며, 해당 권은 이미 회전 18권에 포함돼 판정에 영향이 없다.
  - 목록은 `scripts/tts_pilot/tts_targets.py`의 `POLLUTED_SLUGS`에 근거 주석과 함께 고정한다.

### D2. 키 보관 정책 — Vercel 서버 환경변수 허용 + 관리자 인증 보호

- **허용**: AWS 자격증명(`AWS_ACCESS_KEY_ID` / `AWS_SECRET_ACCESS_KEY` / `AWS_REGION`)을
  **Vercel 서버 환경변수**에 저장할 수 있다. 향후 관리자 화면 '음원생성' 버튼의 전제다.
- **필수 조건 3**:
  1. **`NEXT_PUBLIC_` 접두사 금지.** 서버 컴포넌트·Route Handler·Server Action에서만 읽는다.
     클라이언트 번들 유입 0건을 배포 전 grep으로 확인한다(ADR-0019 CP6-a 선례).
  2. **관리자 인증 보호.** 생성 엔드포인트는 `requireAdmin`(role ∈ admin·curator) +
     zod 검증 + `assertAdmin` 트리플 가드를 통과해야 한다(ADR-0019 D2·D11 승계).
  3. **최소 권한 IAM.** 해당 키는 `polly:SynthesizeSpeech`·`polly:DescribeVoices`만 갖는
     전용 IAM 사용자로 발급한다. S3·기타 권한을 부여하지 않는다.
- **불변(변경 없음)**: **워커 로컬 환경에 AWS·Storage secret 키를 주입하지 않는다.**
  본 ADR은 *서버* 보관을 허용할 뿐, 워커 수령 금지는 그대로다(ADR-0052 D8 승계).
- **Hard Rule 6과의 관계**: Hard Rule 6은 Supabase 비밀 키의 클라이언트 노출 금지다.
  본 결정은 AWS 키를 **서버 전용**으로 두므로 그 원칙과 같은 방향이며, 완화가 아니다.

### D3. 입구 정제 게이트 — Polly 투입 전 필수 정제

- TTS 입력 텍스트는 Polly 호출 **직전에** 반드시 아래 순서로 정제한다
  (`tts_targets.sanitize()` 단일 경로).
  1. **HTML 엔티티 디코딩** — 변화가 멈출 때까지 반복(`&amp;amp;` 같은 중첩 대비, 최대 4회)
  2. **제어문자·zero-width 문자 제거**(`\n`·`\t`는 3에서 처리)
  3. **공백 정규화** — 개행→공백, 연속 공백 1개, 앞뒤 trim
  4. **문장부호 뒤 공백 보정** — `Mom.“Say` → `Mom. “Say`(기존 `generate_tts.py` 규칙 계승)
- 근거: 엔티티가 그대로 넘어가면 Polly가 `&amp;`를 "앰퍼샌드 에이 엠 피"로 읽는다.
  ADR-0029가 **입수 시점** 디코딩을 이미 규정했으나, 본 게이트는 **출구에서 한 번 더** 막는
  이중 방어다. 입수 경로가 늘어나도 낭독 품질이 상류 구현에 의존하지 않는다.
- **실측(2026-07-27)**: 현재 `book_text` 2,128행 중 `&` 문자 포함 행 **0건** — ADR-0029가
  정상 작동 중이다. 즉 본 게이트는 현시점에선 **예방적**이며, 이 사실 자체가 회귀 감시 기준선이다.
- 정제 규칙은 `python scripts/tts_pilot/tts_targets.py --selftest`로 8케이스 자체 점검한다.

### D4. 비용 통제 — dry-run 선행·승인 후 실행을 표준 절차로

- 실제 생성 **전**에 반드시 `--dry-run`을 실행해 **대상 권수 / 총 페이지 / 총 문자수(정제 후)
  / 추산 비용(USD·KRW)** 을 산출하고, **팀장 승인 후** 생성한다. 이를 상시 절차로 명문화한다.
- dry-run은 **Polly 호출 0 · Storage 업로드 0 · DB 쓰기 0**이며, 산출물은 권별 CSV와 요약 JSON이다.
- **추산 비용은 상·하한 두 값으로 보고**한다. 현 구현은 페이지마다 음성 1회 + speech marks
  1회를 호출하므로, marks 요청의 과금 여부에 따라 문자수가 ×1 또는 ×2가 된다.
  실청구서로 확인 전까지 **×2를 보수적 상한**으로 본다.
- 원화 환산 환율은 **가정치**이며 `--krw-rate`로 조정한다(기본 1,380원/USD). 확정 환율은 팀장 확인 사항.
- **2026-08-10 팀장 승인: D4 드라이런 실행 승인**(708권 전량 대상, 비용 미발생 시뮬레이션 —
  Polly 0 · Storage 0 · DB 0). 본 승인은 **드라이런 실행에 한정**되며, 실합성 착수는 드라이런
  산출물에 대한 별도 승인을 요한다.
- **2026-08-10 드라이런 확정치 — 708권 본문 818,002자 + 표지 35,795자 = 853,797자(정제 후),
  합성 유닛 7,978개, long-form ×1 $85.38 / ×2 $170.76(보수적 상한, ₩235,648@1,380 · ₩239,063@1,400).**
  산출물 `scripts/tts_pilot/out/_d4_dryrun_708_final.json`. 39권 참고치 $14.68과 정합(raw 차이 0자).
- **2026-08-10 선행 조건(`book_text` 적재) 충족 — D4 실합성 착수 가능 상태. 팀장 승인 대기.**
  (ADR-0056 적재 트랙 종결: 708권 7,368행 COMMIT 완료)

#### D4-a. 실합성 완료 기록 (2026-08-12)

**D4 실합성이 전량 폐합됐다. 실패 0.**

| 항목 | 값 |
|---|---:|
| 대상 권수 | **708권** |
| 합성 유닛 | **7,978개** (본문 7,270 + 표지 708) |
| 합성 문자수(정제 후) | **853,725자** (본문 818,002 + 표지 35,723) |
| 유닛 상태 | `ok` 7,943 + `reused_pilot` 35 = **7,978** |
| **실패** | **0** |
| 산출 파일 | mp3 **7,978** + marks **7,978** = 15,956개 |
| 산출 용량 | mp3 549.6 MiB + marks 11.3 MiB = **560.9 MiB** |
| speech-mark 워드 이벤트 | 161,311개 |

- 산출 루트 `scripts/tts_pilot/out/audio_full708/{platform}-{source_id}/`,
  요약 `out/_full708_summary.json`, 체크포인트 `out/_full708_checkpoint.json`,
  실패 목록 `out/_full708_failures.json`(빈 배열).
- **드라이런 확정치 853,797자 대비 −72자**다. 표지 텍스트에서 제목이 문장부호로 끝날 때
  마침표를 덧붙이지 않도록 고친 결과다(commit `23d89d3`). 본문 818,002자는 무변동.
- **무결성 검증 통과**(2026-08-12, 로컬 전수):
  708개 매니페스트가 선언한 7,978유닛 전부에 대해 **결손 0 · 매니페스트 `mp3_bytes` ↔ 실파일
  바이트 불일치 0 · marks NDJSON 파싱 실패 0 · 매니페스트 `words` ↔ marks 워드 수 불일치 0**.
- `out/audio_full708/_raw/`(감속 전 원속도 원본 7,943개, 384.2 MiB)는 **디버그 보존용이며
  업로드 대상이 아니다.** 업로더가 매니페스트 `units[]`만 근거로 삼으므로 구조적으로 제외된다.

**비용 — 실청구액과 로컬 이론치가 $4.57 어긋난다(미해소).**

| 출처 | 값 |
|---|---:|
| **AWS 실사용액(팀장 콘솔 확인)** | **$166.18** = 크레딧 $115.01 + 실지출 $51.17 |
| 로컬 이론치 ×2(전 유닛) | $170.75 |
| 로컬 이론치 ×2(`reused_pilot` 35유닛 제외) | $170.13 |

- ×2는 D4 본문대로 **mp3 1회 + speech-marks 1회** 과금을 가정한 보수적 상한이며,
  long-form $100/1M자 단가 기준이다. 재사용 35유닛(3,064자)은 본 캠페인에서 재호출되지 않았다.
- **실청구액이 이론 상한보다 낮으므로 초과 과금 위험은 없다.** 다만 잔차 $4.57(약 2.7%)의
  귀속처는 확정되지 않았다 — 프리 티어 잔여분 소진, marks 요청 과금 구간, 청구 반올림 중
  어느 것인지 콘솔 라인아이템 대조가 필요하다. **본 ADR은 $166.18을 실지출 정본으로 채택하고,
  잔차 규명은 미해소 항목으로 남긴다.**
- O3(원화 환산 환율)은 여전히 미확정이므로 원화 환산액을 확정 기재하지 않는다.

#### D4-b. 2026-08-12 중단 사고 기록

- **증상**: 합성 배치가 AWS 자격 오류로 중단, 잔여 3유닛 미합성 상태로 종료.
- **원인**: **AWS 프리 플랜 종료로 계정이 정지**되어 액세스 키가 무효화됐다.
  스크립트·자격증명 설정·Polly 사양 어느 쪽에도 결함이 없었다.
- **조치**: 팀장이 유료 플랜으로 전환 → 키 유효성 회복 → 체크포인트 재개 실행.
  기존 7,975유닛은 스킵되고 잔여 3유닛만 합성됐다(합성 문자 783자, 청구 $0.1566, 경과 18초).
  재개로 복구된 3유닛: `bloom-bf26d12c…/p07` · `bloom-e6198cba…/p03` · `book_dash-9c9e663c…/p05`
  — 3건 모두 mp3·marks 생성 및 매니페스트 `status='ok'` 반영을 개별 확인했다.
- **과금 영향 없음** — 중단 구간에서 부분 과금·중복 합성이 발생하지 않았다.
  체크포인트 재개 규약(Amendment #1 A4)이 설계대로 작동한 사례다.

#### D4-c. 다음 단계

1. **Storage 업로드** — `book-audio` 버킷에 15,956개 오브젝트(560.9 MiB) 업로드.
   업로더 `scripts/tts_pilot/upload_tts_full708.py`(작성 완료, dry-run 검증 통과).
   **실행은 팀장 영역**(D6 불변 · Hard Rule 6).
2. **`book_audio` 적재** — 업로드 완료 확인 후 SQL 생성 → 팀장이 SQL Editor에서 실행.
- 두 단계 모두 **워커는 스크립트 작성·dry-run까지만** 수행한다.

### D5. 기존 유지 규칙 재확인 (변경 없음)

- **페이지별 mp3** — 오디오는 책 단위가 아니라 페이지 단위(ADR-0034 결정 ①).
- **파일명 1-based / 내부 좌표 0-based** — 오브젝트 키의 `NN` = `page_index + 1`,
  `book_audio.page_index`는 0-based 유지(ADR-0034 Amd#2 · ADR-0052 D5).
- **`audio_path`는 버킷명 없는 오브젝트 키만** — `book_dash-{slug}/{voice}/pNN.mp3`
  (ADR-0034 결정 ② + Amd#2 성우 층위). 버킷명 `book-audio`를 값에 넣지 않는다.
- **`book_audio` UNIQUE `(book_id, kind, page_index, voice)`** 준수 — 표지는
  `kind='cover'`·`page_index=0`(ADR-0034 Amd#1).
- **Content-Type 명시 지정** — mp3 `audio/mpeg`, marks `application/json; charset=utf-8`,
  `Cache-Control: public, max-age=31536000, immutable`(ADR-0034 결정 ③).
- **어트리뷰션 승계** — TTS 음성은 CC BY 원문의 2차 저작물(ADR-0023 §2.6). Hard Rule 1 무접촉.
- **보이스·엔진·속도는 O1로 이관** — 작업지시서는 `Neural / Ruth / 78%`를 "유지 규칙"으로
  적었으나 이는 ADR-0052 Amd#2(`Danielle / long-form / atempo 0.85`)와 충돌한다. 아래 O1 참조.

### D6. 실행 경계 — 워커 DB 읽기 허용, 쓰기·업로드는 팀장 영역

> **Amendment #5 (2026-08-13, ADR-0058 D7로 개정) — 쓰기 주체 3분류**
>
> 본 절은 아래로 **대체된다**:
>
> > **D6. 실행 경계 — 쓰기 주체 3분류 (ADR-0058 D7로 개정, 2026-08-13)**
> >
> > | 주체 | DB 쓰기 | Storage 쓰기 | 비고 |
> > |---|---|---|---|
> > | ① **워커 스크립트**(로컬 개발자 도구) | **금지(불변)** | **금지(불변)** | SELECT만 허용. AWS·Supabase secret 키 주입 금지(ADR-0052 D8 승계) |
> > | ② **배포 서버**(Next.js server action) | **`book_review.status` 전이만 허용** | 금지 | `SUPABASE_SECRET_KEY` service role + ADR-0019 D2 트리플 가드(zod → `assertAdmin` → service role) 통과 필수. `books`·`book_audio`·`book_text` 쓰기 0건 |
> > | ③ **팀장 로컬 실행** | SQL Editor 실행(`book_audio` INSERT 등) | 업로드 | 합성·업로드·적재 SQL 실행 |
> >
> > 종전 D6의 "INSERT/UPDATE/DELETE·Storage 업로드는 전부 팀장 영역"은 **①에 대해 그대로 유효**하며,
> > ②는 관리자 인증 가드 하의 좁은 예외로 신설된 것이다. 스키마 변경은 3주체 모두 금지이며
> > 팀장의 SQL Editor 실행으로만 이뤄진다(Hard Rule 8).
>
> 개정 범위는 **D6 한정**이다. D1·D2·D3·D4·D5는 무개정이며, 특히 D4(dry-run 선행 절차)는
> 위 ①·③ 경로에서 그대로 살아 있다. 상세 근거는 **ADR-0058 D7**을 참조한다.

아래는 최초 기재분이다(이력 보존).

- ADR-0052 D3(A안: 팀장 SQL export → 워커 변환)을 **읽기 조회에 한해 개정**한다. 워커 스크립트는
  기존 sync 스크립트와 동일한 `load_env()` 관행으로 **`book_dash` 대상 선정 SELECT를 직접
  수행**할 수 있다. 152권 규모에서 수기 export 왕복은 오류 표면만 넓힌다.
- **불변**: INSERT/UPDATE/DELETE·스키마 변경·Storage 업로드는 **전부 팀장 영역**이다.
  본 트랙 스크립트는 DB 쓰기 코드 경로를 갖지 않는다.

## Consequences

- **얻는 것**: 116권이 사람 선검수 없이 생성 가능해진다. 비용이 실행 전에 보인다.
  정제 게이트로 낭독 품질이 상류 구현에 덜 의존한다. 오염 6권 목록이 근거와 함께 코드에 고정된다.
- **잃는 것**: 사후 검수라 품질 미달본이 일시적으로 존재할 수 있다. 재생성 비용은 감수한다.
- **이연**: 회전 18권·오염 6권은 검수 화면에서 텍스트 수정 후 별도 배치로 처리한다.
  `book_text` 없는 54권(v1 44 + 기타 10)은 OCR/추출 트랙 사안이다.
- **되돌리기**: D1은 상태 게이트를 다시 켜면 원복된다(스키마 무변경). D2는 Vercel 환경변수
  삭제 + IAM 키 폐기로 원복된다.

## Non-goals

- 관리자 '음원생성' 버튼 UI 구현(D2는 그 **전제 정책**만 확정한다).
  → **2026-08-13 ADR-0058이 이 범위를 인수했다**(D1: 화면은 요청 기록만, 합성은 로컬 파이프라인).
  본 ADR의 Non-goal 지위는 그대로이며, 구현 결정은 ADR-0058에 있다.
- Storage 업로드·`book_audio` INSERT·`books.has_audio` 반영(팀장 영역, 별도 지시서).
- 구 44권 `p00` 축·`voice='Ruth'` 표기의 신 규약 재정렬(ADR-0034 Amd#2가 백로그로 이연).
- ASb·Bloom·GDL 등 타 플랫폼 TTS.

## Open (팀장 확인)

### O1. 보이스·엔진 확정 — `Danielle / long-form` vs `Ruth / neural 78%`

작업지시서 Task 1-5는 `Polly Neural / Ruth / 78%`를 "기존 유지 규칙"으로 적었으나,
**최신 확정본은 ADR-0052 Amendment #2(2026-07-22)의 `Danielle / long-form / atempo 0.85`** 이다.
같은 Amendment는 Ruth 78% 산출물이 **팀장 청취 검수에서 반려**됐다고 기록한다.
실제 `book_audio`에도 시범 12권이 `voice='danielle'` 150행으로 적재돼 있다.

| | Ruth / neural 78% | Danielle / long-form |
|---|---|---|
| 근거 ADR | ADR-0023 Amd#1 | ADR-0052 Amd#2 (최신) |
| 청취 판정 | 반려(2026-07-24) | 채택 |
| 단가 | $16 / 1M자 | $100 / 1M자 |
| 116권 추산(×2 상한) | **$3.81 (약 ₩5,253)** | **$23.79 (약 ₩32,829)** |
| 시연 자산 정합 | 불일치(재생성 필요) | 일치 |

**차액은 약 $20(₩27,600)** 로, 6.25배지만 절대액이 작아 **비용은 결정 근거가 되지 못한다.**
따라서 품질·일관성 기준으로 `Danielle / long-form` 승계를 권고하며, 구현 기본 프리셋도
`danielle-longform`으로 두었다(`--preset ruth-neural`로 전환 가능).
팀장이 Ruth로 확정하면 기본값을 바꾸고 본 ADR에 Amendment로 고정한다.

### O2. 표지 낭독 트랙 포함 여부

시범 배치는 표지(`kind='cover'`, `"{title}. Created by {author}."`)를 생성했다.
전체 확장에도 포함할지 미확정. 포함 시 **+5,269자**로 추산이
`$3.97`(neural) / `$24.84`(long-form)가 된다(×2 상한 기준, 차이 약 ₩1,450).
구현은 `--with-cover` 플래그로 대기 중이며 **기본값은 미포함**이다.

### O3. 원화 환산 환율

`--krw-rate` 기본값 1,380원/USD는 워커의 **가정치**다. 확정 환율을 알려주면 반영한다.

---

## Amendment #1 (2026-07-27, 팀장 승인) — O1·O2 해소 + 재개 규약

### A1. 보이스·엔진 확정 (O1 해소)

- **`Danielle / long-form / us-east-1 / atempo 0.85`** 로 확정한다. ADR-0052 Amd#2 승계.
- `ruth-neural` 프리셋은 폐기하지 않고 **비교·폴백용으로 코드에 남긴다**(`--preset ruth-neural`).
- D5의 "보이스·엔진은 O1로 이관" 문구는 본 항으로 대체된다.

### A2. 표지 낭독 트랙 포함 확정 (O2 해소)

- 전량 배치에 **표지 트랙을 포함**한다(`--with-cover`). 116권 전부 표지 문구가 생성된다
  (`"{title}. Created by {author}."`, author 없는 1권은 제목만).
- 표지도 본문과 **동일 규약**을 따른다: mp3 + word marks, `audio_path`는 버킷명 없는
  오브젝트 키(`book_dash-{slug}/danielle/cover.mp3`), DB 적재 시
  `kind='cover'` · `page_index=0` 으로 UNIQUE `(book_id, kind, page_index, voice)` 충족
  (ADR-0034 Amd#1).

### A3. 확정 수치 (dry-run 실측, 2026-07-27)

| 항목 | 값 |
|---|---|
| 대상 | 116권 |
| 유닛 | 1,464 (본문 1,348 + 표지 116) |
| 문자수 | 124,213 (본문 118,944 + 표지 5,269) |
| 추산 비용 | **$24.84 · 약 ₩34,283** (long-form ×2 보수적 상한, 환율 가정 1,380원) |
| Polly 요청당 문자 상한(3,000자) 초과 | 0건 (최대 페이지 657자) |

### A4. 재개(resume) 규약 — 로컬 산출물 기준

- 중단 후 **같은 명령 재실행**이 표준 재개 방법이다. 유닛의 `mp3`·`marks.json`이
  **둘 다 존재하고 둘 다 0바이트가 아닐 때만** 건너뛴다(재과금 방지). 한쪽만 있거나
  0바이트면 다시 합성한다 — mp3 기록 후 marks 기록 전 중단된 경우를 흘리지 않기 위함이다.
- **재개 판정은 `book_audio` 기준이 아니다.** 본 스크립트는 DB에 쓰지 않으므로 중단 시점까지의
  산출물은 디스크에만 있다. `book_audio` 조회는 *대상 선정* 단계에서 이미 적재 완료된 권을
  제외하는 용도다(현재 12권).
- 전량 재생성은 `--force`. 특정 권만은 `--force --slugs a,b`.
- 실행 절차서: **`docs/ops/tts-fullbatch-run.md`**(팀장 PowerShell 복붙용).

---

## Amendment #2 (2026-08-08, 팀장 결정) — 보이스·엔진 확정의 재확인 및 ASb·Bloom 669권 확대 적용

### B0. 선행 사실 — O1은 이미 Amendment #1에서 해소됐다

본 Amendment는 **닫힌 항목을 다시 여는 것이 아니다.** O1은 2026-07-27 Amendment #1 A1에서
`Danielle / long-form / us-east-1 / atempo 0.85`로 이미 확정됐다. 그럼에도 본 항을 추가하는
이유는 둘이다.

1. **적용 범위가 넓어졌다.** Amd#1의 확정은 Book Dash 116권을 전제로 한 것이었다.
   ASb 527 + Bloom 142 = 669권이 신규 대상으로 편입되면서(ADR-0056 D1), 확정된 보이스·엔진을
   이 코호트에도 그대로 적용할지가 새 판단 대상이 됐다.
2. **코드 주석이 확정 이후에도 갱신되지 않았다.** B3 참조.

### B1. 결정 — `Danielle / long-form`을 ASb·Bloom 669권에도 적용한다

- 보이스 **Danielle**, 엔진 **long-form**, 리전 `us-east-1`, 평문 입력 + ffmpeg `atempo=0.85`
  후처리, marks `time × (1/0.85)` 선형 스케일. **Amendment #1 A1과 완전 동일한 사양**이다.
- 적용 범위를 **Book Dash + ASb + Bloom 전 코호트**로 확대한다.
- 결정일 **2026-08-08**, 결정자 **팀장**.

### B2. 결정 근거

1. **음성 일관성** — 기존 128권(시범 12 + 전량 116)이 이미 `Danielle / long-form`으로 합성
   완료돼 있다(`out/_fullbatch_report.json`: `books_ok=116, total_units=1464`).
   엔진을 바꾸면 **동일 서재 안에서 낭독 음성이 이원화**된다. 유아 대상 서비스에서 책마다
   목소리가 바뀌는 것은 품질 문제로 인식된다.
2. **비용이 결정 근거가 되지 못한다** — ASb·Bloom 669권 실측 `chars_billable` **744,408자**
   기준(2026-08-08 전수 드라이런, ADR-0056 §Context 3):

   | 엔진 | 단가 | ×1 (음성만) | ×2 (음성+marks 보수적 상한) |
   |---|---|---|---|
   | long-form | $100 / 1M자 | $74.44 · 약 ₩104,217 | **$148.88 · 약 ₩208,434** |
   | neural | $16 / 1M자 | $11.91 · 약 ₩16,675 | $23.82 · 약 ₩33,349 |

   (환율 가정 **1,400원/USD** — O3은 여전히 미확정이며 본 표도 가정치다)

   long-form ×2 상한이 **약 21만 원**으로 절대 금액이 작다. neural 전환의 절감액
   (×2 기준 약 **$125**, ×1 기준 약 **$63**)은 음성 일관성 손실을 정당화하지 못한다.
   이는 Amd#1이 116권에서 적용한 판단("차액이 6.25배지만 절대액이 작아 비용은 결정 근거가
   되지 못한다")과 동일한 논리이며, 규모가 6배로 커진 뒤에도 결론이 유지됨을 확인한 것이다.
3. **청취 검수 이력** — `Ruth / neural 78%`는 2026-07-24 팀장 청취에서 반려됐다
   (ADR-0052 Amd#2 기록). 이 판정은 번복되지 않았다.

### B3. `run_tts_fullbatch.py`의 "팀장 확정 대기" 주석은 해소됐다

`scripts/tts_pilot/run_tts_fullbatch.py:59-62`에 다음 주석이 남아 있다.

> ⚠ 보이스·엔진은 ADR-0053 Open 항목이다(팀장 확정 대기).

이 문구는 **Amendment #1(2026-07-27) 시점에 이미 사실과 달랐고**, 본 Amendment로 완전히
해소된다. 코드가 사용하는 기본 프리셋(`DEFAULT_PRESET = "danielle-longform"`)은 확정 사양과
일치하므로 **동작상 문제는 없으며, 주석만 낡았다.**

- **코드 수정은 본 Amendment 범위 밖이다.** 주석 갱신은 다음 코드 작업 트랙에서 함께 처리한다.
- `ruth-neural` 프리셋은 Amd#1 A1대로 **비교·폴백용으로 코드에 남긴다**(폐기하지 않는다).

### B4. 본 Amendment가 바꾸지 않는 것

- Amendment #1 A2(표지 트랙 포함), A3(116권 확정 수치), A4(재개 규약)는 **전부 유효**하다.
- O3(원화 환산 환율)은 **여전히 미확정**이다. B2의 1,400원/USD는 가정치이며, 확정 환율이
  나오면 별도 반영한다.
- 본 Amendment는 ASb·Bloom의 **합성 실행을 승인하지 않는다.** 대상 코호트의 `book_text`
  적재(ADR-0056, Accepted 2026-08-10)가 선행돼야 하고, 합성 전 dry-run 승인 절차(D4)도 그대로 적용된다.

---

## Amendment #3 (2026-08-08, 팀장 결정) — Book Dash html 활성 39권 편입 (669 → 708권)

### C0. 본 Amendment의 성격 — Amendment #2를 재개하지 않는다

**Amendment #2는 닫힌 상태 그대로 유효하다.** B1~B4 어느 항목도 재개·수정하지 않는다.
본 Amendment는 **적용 범위에 코호트 1종을 추가**하는 것뿐이며, 보이스·엔진·속도·후처리
사양은 **Amendment #1 A1과 완전 동일**하다(`Danielle / long-form / us-east-1 / atempo 0.85`,
marks `time × (1/0.85)` 선형 스케일).

추가 사유는 Amendment #2의 그것과 같은 구조다 — **신규 코호트가 대상으로 편입됐다.**
이번 신규 코호트는 ASb·Bloom과 달리 **기존 음원(`voice='Ruth'`)이 이미 존재**하므로,
"확대 적용"에 더해 **"기존 음원의 대체"** 판단이 함께 필요했다(C3).

### C1. 결정 — Danielle 적용 범위를 **708권 7,368면**으로 확대한다

| 코호트 | 권수 | 면수 | 출처 |
|---|---:|---:|---|
| ASb `asb_native` 활성 | 527 | 5,330 | ADR-0056 §Context 3 (2026-08-08 전수 드라이런) |
| Bloom `asb_native` 활성 | 142 | 1,569 | 〃 |
| **소계 (Amendment #2 범위)** | **669** | **6,899** | |
| **Book Dash html 활성·오디오 보유 (ADR-0056 D9-a)** | **34** | **409** | **팀장 SQL 실측 2026-08-08** |
| **Book Dash html 활성·무오디오 (ADR-0056 D9-b)** | **5** | **60** | **워커 json 실측 · ADR-0054 Amd#2** |
| **소계 (본 Amendment 신규)** | **39** | **469** | = html 활성 전량 |
| **합계** | **708** | **7,368** | |

- 669 + 39 = **708** ✅ / 6,899 + 469 = **7,368** ✅ / 409 + 60 = **469** ✅
- 신규 추가분 **469면은 전체 7,368면의 6.4%**(469 ÷ 7,368 = 6.37%)다.
  - 34권분 5.6%(409 ÷ 7,368 = 5.55%) · **5권분 0.8%**(60 ÷ 7,368 = 0.81%).
- 대상 정의는 **ADR-0056 D9**, **확정 명단은 ADR-0056 O4(34권) · §Context 5-f(5권)** 에 박제돼 있다.
- 면 구성: **34권 = 12면 33권 + 13면 1권(`whose-button-is-this`)** / **5권 = 12면 × 5**.
- 표지 트랙도 **Amendment #1 A2대로 포함**한다(`--with-cover`). **39권분 표지 유닛**이 추가된다.

**★ 34권과 5권은 작업 성격이 다르다 — 합성 절차가 갈린다.**

| | 권수 | 기존 `book_audio` | 본 Amendment의 작업 | 선행 조건 |
|---|---:|---|---|---|
| **D9-a** | 34 | `voice='Ruth'` 존재 | **재생성** — Ruth 행은 무접촉 보존(ADR-0056 D13), Danielle 신규 INSERT | 없음(축 정합 실측 완료) |
| **D9-b** | 5 | **없음** | **신규 합성** — 대체할 음원 자체가 없음 | **적재 후 육안 검증 1권**(ADR-0056 D9-b) · **이미지 실재 확인**(ADR-0056 O6) |

- **D9-b는 "재생성"이 아니라 "신규 생성"이다.** C3의 Ruth 반려 논거(가·나·다 3안 비교)는
  **D9-a에만 적용**된다 — 5권에는 반려할 기존 음원이 없다.
- **D9-b 5권은 ADR-0056 O6(팀장 육안 확인) 대기 중이다.** 실패 시 5권분만 철회되며,
  본 Amendment의 범위는 708권 7,368면 → **703권 7,308면**으로 되돌아간다. D9-a는 영향 없다.
- 결정일 **2026-08-08**, 결정자 **팀장**.

> **정정 이력**: 종전 문서(ADR-0054 O2 · `docs/recon/state-audit.md` §5)는 이 코호트를
> "**44권 574행**"으로 기재했다. **실측 정정** — 574 = `page` **530** + `cover` **44** 의
> 합산치다. 본 Amendment의 면수 기준 축은 **`page` 530행**이며, 그중 활성분이 **409면**,
> 비활성분이 **121면**이다(409 + 121 = 530 ✅).
> 수치 자체는 양쪽 모두 옳으며 **집계 축만 달랐다**(ADR-0056 §Context 5-a).
>
> **재정정(2026-08-08)**: 본 항의 활성분은 초판에서 **408면**으로 적혔으나 **409면으로 정정**한다.
> 408은 "13면 2권이 모두 비활성"이라는 가정에서 나온 산정값이었고, 실측에서 1권
> (`whose-button-is-this`)이 활성으로 확인됐다. 상세와 운영 교훈은 **ADR-0056 §Context 5-b·5-c**.

### C2. 선행 조건 — 본 Amendment는 합성을 승인하지 않는다

Amendment #2 B4의 원칙을 그대로 승계한다. 39권 합성 착수 전 아래가 모두 선행돼야 한다.

| # | 조건 | D9-a (34권) | D9-b (5권) |
|---:|---|---|---|
| 1 | **`book_text` 적재** — ADR-0056 D9~D14 | 필요 (ADR-0056 **Proposed**) | 필요 (동일) |
| 2 | **확정 slug 명단 확보** | ✅ **충족** — ADR-0056 O4 Resolved | ✅ **충족** — ADR-0056 §5-f |
| 3 | **이미지 실재 확인** | ✅ 충족(기존 Ruth 음원 서비스 이력) | ⏸ **대기** — ADR-0056 **O6** |
| 4 | **적재 후 육안 검증** | 불필요(축 정합 실측 완료) | ⛔ **필수 1권** — ADR-0056 D9-b |
| 5 | **dry-run 승인** — D4 절차 | ⛔ **필수** | ⛔ **필수** |

- **2번 상세(D9-a)**: 34권이 slug·`source_id`·면수 단위로 확정됐고, 워커 전수 대조
  (json 부재 0 · `ruth_pages` 34/34 일치 · 장면 합계 409 · **블랙리스트 교집합 0건**)를 통과했다.
- **2번 상세(D9-b)**: 5권 json 실존 5/5 · 장면 12×5 = 60 · 블랙리스트 교집합 0건.
  slug·`source_id` 출처는 `scripts/copy_bookdash_images.py:94-100`(`DONE_BOOKS`).
- **5번은 어느 쪽도 면제되지 않는다.** dry-run은 Polly 호출 0 · Storage 업로드 0 · DB 쓰기 0이다.

### C3. [D9-a 34권 한정] 기존 `voice='Ruth'` 음원을 재생성으로 대체하는 결정의 근거

> **적용 범위 주의**: 본 항은 **D9-a(34권) 전용**이다. D9-b(5권)는 기존 음원이 없어
> 반려·대체를 논할 대상 자체가 없으며, 처음부터 **신규 합성**이다.

정찰(2026-08-08)은 34권이 `book_text` 적재만으로는 AudioReader에 진입하지 못함을 확인했다.
`app/(reader)/book/[id]/read/page.tsx:158`이 `hasReaderAudio(book.id)`를 **voice 인자 없이**
호출하고, 기본값이 `lib/book/audio-manifest.ts:94`의 `DEFAULT_READER_VOICE = 'danielle'`이며,
판정 쿼리가 `:152-153`에서 `.eq('voice', voice)`이므로 **`'Ruth' ≠ 'danielle'`로 게이트가
`false`를 반환**하기 때문이다.

해소안 3종이 제시됐고 **팀장이 (나)만 채택**했다.

| 안 | 내용 | 판정 | 사유 |
|---|---|---|---|
| (가) | `DEFAULT_READER_VOICE` 또는 `page.tsx:158` 호출부 코드 수정 → Ruth 노출 | **반려** | `Ruth / neural 78%`는 **2026-07-24 팀장 청취 검수에서 반려**된 음원이다(ADR-0052 Amd#2 기록 · 본 ADR `:154`·`:263`). 코드를 고쳐 노출시키면 그 판정을 뒤집는다 |
| (나) | **34권 Danielle 재생성** | **채택** | 아래 근거 3종 |
| (다) | `book_audio.voice` 값을 `'danielle'`로 표기 갱신 | **반려** | Storage 오브젝트는 Ruth 음원 그대로이므로 **DB 값과 실 파일의 성우가 괴리**된다. `voice`가 Storage 성우 폴더명과 동일해야 한다는 ADR-0034 Amd#2 규약과 충돌한다 |

**(나) 채택 근거**

1. **음성 일관성** — Amendment #2 B2-1과 동일 논리다. 기존 128권(시범 12 + 전량 116)과
   신규 669권이 모두 Danielle이다. 34권만 Ruth면 **동일 서재 안에서 낭독 음성이 이원화**된다.
2. **청취 검수 이력이 번복되지 않았다** — Amendment #1 A1(2026-07-27)과 Amendment #2 B2-3이
   각각 재확인한 판정이다. 본 Amendment가 세 번째 재확인이다.
3. **비용이 결정 근거가 되지 못한다** — C4 참조. 추가분이 전체의 5.6%다.

**기존 Ruth 행은 삭제하지 않는다.** `book_audio` UNIQUE가 `(book_id, kind, page_index, voice)`
이므로(ADR-0034 Amd#1) Danielle 행 신규 INSERT가 Ruth 행과 충돌하지 않는다. 보존 규정은
ADR-0034 `:256` · 본 ADR Non-goals `:145` · **ADR-0056 D13**을 따른다.

### C4. 비용 — 드라이런 전량 게이트에서 재산출한다

- **확정 수치는 D4 드라이런 산출물이다.** 아래는 착수 여부 판단용 **참고치**이며,
  **승인 근거로 쓰지 않는다.**
- 참고치 — **확정 39권 명단 기준** 워커 로컬 실측(2026-08-08, `scripts/tts_pilot/out/{slug}.json`
  39개 전수):

  | 항목 | D9-a (34권) | D9-b (5권) | **합계 (39권)** |
  |---|---:|---:|---:|
  | 대상 면수 | 409 | 60 | **469** |
  | **미정제** raw 문자수 | 63,923 | 9,489 | **73,412** |
  | 면당 평균 | 156.3자 | 158.2자 | **156.5자** |
  | 최장 면 | 1,023자 (`together-were-strong` p4) | 405자 (`a-fish-and-a-gift`) | **1,023자** |
  | Polly 3,000자 상한 초과 | 0 | 0 | **0건** ✅ |
  | 무텍스트 면 | 0 | **1** (`a-house-for-mouse` p10) | **1면** |

  - 권별 5권 내역: `a-beautiful-day` 882 · `a-dancers-tale` 2,264 · `a-fish-and-a-gift` 3,599 ·
    `a-house-for-mouse` 524 · `a-tiny-seed` 2,220.
  - (참고) 오디오 보유 44권 전체 = 530면 / 73,500자 — 팀장 실측 `page`+`Ruth` **530행과 일치** ✅

- **추정: long-form ×2 보수적 상한 기준 약 $14.68(약 ₩20,600, 환율 가정 1,400원/USD).**
  - 내역: D9-a 약 $12.78 + D9-b 약 $1.90. 전체 7,368면 대비 6.4%라는 비율과 정합한다.
  - **D9-b 5권 추가분은 약 ₩2,700**이다. C3 근거 ②("합성 비용은 (가)·(나) 동일")대로
    **경로 선택이 비용을 바꾸지 않았다** — 5권은 어느 경로로 가든 신규 합성이 필요했다.
- **이 값을 확정으로 쓰지 않는 이유**: ① **정제(D3) 후 문자수가 달라진다**
  ② **표지 트랙 39권분 문자수가 포함되지 않았다**(Amd#1 A2로 표지 포함이 확정돼 있다)
  ③ marks 요청 과금 여부가 미확인이라 ×1/×2 구간이 남는다(D4)
  ④ **무텍스트 면 1건은 합성 대상에서 빠진다**(D3 정제 후 `empty_pages` 분류,
  `tts_targets.py:262-265`) — 실제 유닛 수는 469보다 1 적다.
  **확정치는 D4 드라이런 산출물이다.**
- O3(원화 환산 환율)은 **여전히 미확정**이다. 1,400원/USD는 가정치다.

### C5. 본 Amendment가 바꾸지 않는 것

- **Amendment #1 A1~A4 전부 유효**(보이스·엔진 확정 / 표지 포함 / 116권 확정 수치 A3 /
  재개 규약 A4). A3의 116권 수치는 **당시 Book Dash 전량 배치의 실측 기록**이며, 범위가
  넓어져도 그 기록은 변하지 않는다.
- **Amendment #2 B1~B4 전부 유효.**
- **D1 사전 제외 3종 유지** — 회전 18권 · 오염 6권 · 이미 오디오 보유 12권.
  - ⚠ **D1 표 4행("텍스트 없음 54권")의 전제는 본 Amendment로 39권만큼 축소된다.**
    그 행은 "`book_text` 행 없음 = 낭독 대상 자체가 없음(그중 44권은 v1 배치로 이미 오디오
    보유)"으로 적혔는데, ADR-0056 D9가 39권의 `book_text`를 적재하므로 **그 39권에 한해
    제외 사유가 소멸**한다. **잔여 15권 = 비활성 15권(= 블랙리스트 15권)** 이며 제외 상태 유지다.
    - 검산: 54 − 39(활성 전량) = **15** ✅. 비활성 15 = 블랙리스트 15임이 slug 1:1로
      확정됐다(ADR-0056 §Context 5-e). 즉 **잔여 제외분은 전원 사용자 비노출 도서**다.
  - **D1 표 3행("이미 오디오 보유 12권")은 무접촉**이다. 그 12권은 시범 `danielle` 코호트이며
    본 Amendment의 39권과 서로소다.
- **D2(키 보관 정책) · D3(입구 정제 게이트) · D4(비용 통제) · D5(기존 유지 규칙) ·
  D6(실행 경계) 전부 무접촉.** 39권 전량이 D3 정제와 D5 규약(페이지별 mp3, 1-based 파일명 /
  0-based `page_index`, 버킷명 없는 `audio_path`, UNIQUE, Content-Type 명시)을 그대로 따른다.
- **Non-goals `:145`("구 44권 `p00` 축·`voice='Ruth'` 표기의 신 규약 재정렬") 유지.**
  D9-a의 Danielle 음원은 **신 규약**(`book_dash-{source_id}/danielle/pNN.mp3`, 1-based)을
  따르며, **구 Ruth 오브젝트(`p00~` 축)는 이동·개명하지 않는다.** 백로그를 열지 않는다.
  D9-b 5권은 구 오브젝트가 없어 **처음부터 신 규약**으로만 생성된다.
- **본 Amendment는 39권의 합성 실행을 승인하지 않는다**(C2).

---

## Amendment #4 (2026-08-12, 팀장 지시) — 적재 실행 수단: 일회성 staging 테이블 도입

### E0. 본 Amendment의 성격 — 합성 사양을 재개하지 않는다

**Amendment #1~#3은 닫힌 상태 그대로 유효하다.** A·B·C 어느 항목도 재개·수정하지 않는다.
본 Amendment가 다루는 것은 **이미 합성·업로드가 끝난 산출물을 `book_audio`에 적재하는
"실행 수단"** 뿐이다. 보이스·엔진·속도·후처리·범위(708권 7,978유닛) 전부 무접촉이다.

기록 사유는 하나다 — **적재 과정에서 DB에 테이블 1개가 임시로 생겼다 사라진다.**
도메인 스키마 변경은 아니지만 `claude.md` Hard Rule 8(DB 스키마 변경 시 ADR 선행)의
경계에 닿으므로, 사실관계와 무접촉 근거를 남긴다.

### E1. 배경 — v1 단일 파일이 SQL Editor에서 실행 불가

적재 SQL v1은 플랫폼별 단일 파일이었고, `load708_step1_asb.sql`이 **1,139,390바이트**였다.
Supabase SQL Editor가 `Query is too large to be run via the SQL Editor`로 **실행을 거부**했다.
step1조차 실행이 불가능해 파일 구조 자체를 바꿔야 했다.

팀장은 **SQL Editor만 사용**한다(psql 직접 연결 도입 금지). 따라서 해법은
"실행 단위를 Editor 제한 안쪽으로 쪼개되 원자성·무접촉 원칙은 유지"로 한정됐다.

### E2. 결정 — 일회성 staging 테이블 1개를 경유한다

`public.book_audio_staging_708` 을 만들고, 적재가 끝나면 지운다.

| 컬럼 | 타입 | 용도 |
|---|---|---|
| `chunk_no` | int | 어느 청크 파일에서 왔는지(누락 청크 추적) |
| `source_platform`, `source_id` | text | `books` 조인 키 |
| `manifest_book_id` | uuid | 매니페스트가 기록한 `book_id` — **대조용**(삽입값 아님) |
| `kind`, `page_index` | text, int | ADR-0034 Amd#1 / ADR-0052 D5 규약 그대로 |
| `audio_path`, `marks_path` | text | 버킷명 미포함 오브젝트 키(ADR-0034) |
| `duration_ms` | int | 감속 후 mp3 실측 |

- 제약: `CONSTRAINT book_audio_staging_708_audio_path_key UNIQUE (audio_path)`
- 인덱스: `book_audio_staging_708_src_idx (source_platform, source_id)`
- **RLS 켜고 정책 0개** → anon·authenticated 전면 차단. SQL Editor(소유자)·service_role만 통과.
  `books`·`book_audio` 선례(ADR-0034 Phase A-1 [3])와 동일한 방식이다.

**TEMP 테이블을 쓰지 않는 이유**: SQL Editor는 실행 사이에 세션이 유지된다는 보장이 없어
TEMP 테이블이 청크 사이에 사라질 수 있다. 그래서 영구 테이블로 만들고 명시적으로 지운다.

### E3. 수명주기 — 생성부터 삭제까지 파일로 고정

| 파일 | 역할 |
|---|---|
| `docs/sql/load708/00_staging_create.sql` | staging **생성** (`CREATE TABLE IF NOT EXISTS`, 멱등) |
| `docs/sql/load708/01_chunk_01~12.sql` | staging에만 INSERT. 청크 12개, **최대 156,228바이트** |
| `docs/sql/load708/02_staging_verify.sql` | 전량 게이트(읽기 전용) → `verdict` PASS/FAIL |
| `docs/sql/load708/03~05_merge_step1~3.sql` | `book_audio` 머지. BEGIN → INSERT SELECT → 검증 → ROLLBACK |
| `docs/sql/load708/06_final_verify.sql` | 최종 검증(읽기 전용) → `verdict` PASS/FAIL |
| `docs/sql/load708/07_staging_drop.sql` | staging **삭제** (`DROP TABLE IF EXISTS`) |

- **07은 절차의 마지막이며 생략 대상이 아니다.** staging은 적재 작업이 끝나면 DB에 남지 않는다.
- v1 파일 4종은 삭제하지 않고 `docs/sql/deprecated/`로 이동했다(이력 보존).
- 생성기는 `scripts/tts_pilot/gen_book_audio_sql_708.py` 하나이며 v2만 산출한다.

### E4. 도메인 DDL 무접촉 근거 — Hard Rule 저촉 없음

| Hard Rule | 저촉 여부 | 근거 |
|---|---|---|
| 1. `books.attribution_text` NOT NULL | **무접촉** | `books` DDL을 건드리지 않는다. 본 적재는 `books`를 **읽기만** 한다(조인) |
| 2. `enforce_commercial_license` 트리거 | **무접촉** | 비활성화·삭제·우회 없음. `books` INSERT/UPDATE 자체가 없다 |
| 8. DB 스키마 변경 시 ADR 선행 | **본 Amendment로 충족** | 아래 성격 판단 참조 |

**성격 판단**: `book_audio_staging_708`은 도메인 모델이 아니라 **적재 작업용 일회성 그릇**이다.

- `books`·`book_audio`·`book_text`·`book_review` 어느 테이블의 **컬럼·제약·트리거·인덱스도
  변경하지 않는다.** 기존 객체에 대한 `ALTER`는 0건이다.
- 앱 코드가 이 테이블을 참조하지 않는다(생성~삭제가 팀장 SQL 실행 구간 안에서 끝난다).
- 마이그레이션(`supabase/migrations/`)에 넣지 않는 이유도 같다 — **영속 스키마가 아니다.**

그럼에도 "public 스키마에 테이블이 생긴다"는 사실은 남으므로, 팀장 지시에 따라 본 Amendment로
기록한다. **사후 승인이 아니라 사실 기록이다** — 도입은 팀장이 구조를 지정해 지시했다.

### E5. 기존 행 보호 — v1에서 강화된 지점

- 머지 `INSERT`에 **`ON CONFLICT` 절이 없다.** 충돌 시 에러로 죽는다(덮어쓰기 구조적 불가).
  v1 이전 116권 배치는 `ON CONFLICT DO UPDATE`였으나, 본 적재는 "기존 행 무접촉"을
  SQL 레벨에서 보장하는 쪽을 택했다.
- `book_id`를 VALUES에 박지 않는다. `(source_platform, source_id)`로 `books`를 조인해 얻고,
  매니페스트 `book_id`와 불일치하면 `RAISE EXCEPTION`(fail-closed). 머지 파일당 가드 6개.
- 모든 검사·삽입이 `source_platform`으로 한정된다 → 앞 step을 COMMIT한 뒤 다음 step을
  실행해도 앞 step 행이 충돌로 잡히지 않는다.
- 청크는 `ON CONFLICT (audio_path) DO NOTHING` → 멱등. 재실행해도 중복이 쌓이지 않으므로
  `TRUNCATE` 없이 실패 지점부터 이어서 실행할 수 있다.
- 구 44권(`voice='Ruth'`)은 UNIQUE에서 자연 분리 — 무접촉이다.

### E6. 적재 규모 (로컬 실측 재검산)

| 코호트 | 권 | 행 | page | cover |
|---|---:|---:|---:|---:|
| african_storybook | 527 | 5,855 | 5,328 | 527 |
| bloom | 142 | 1,616 | 1,474 | 142 |
| book_dash | 39 | 507 | 468 | 39 |
| **합계 (신규)** | **708** | **7,978** | **7,270** | **708** |
| 기존 danielle (pilot12+fullbatch116) | 128 | 1,614 | 1,486 | 128 |
| **최종 danielle** | **836** | **9,592** | **8,756** | **836** |

- 업로드 체크포인트 15,956키 = mp3 7,978 + marks 7,978 — 매니페스트와 **1:1 일치**.
- 기존 danielle 128권과 `book_id` 교집합 **0건**. `duration_ms` NULL **0행**.

### E7. 본 Amendment가 바꾸지 않는 것

- **Amendment #1 A1~A4 / #2 B1~B4 / #3 C1~C5 전부 유효.** 합성 사양·범위 무접촉.
- **D1~D6 전부 무접촉.** 적재 대상·정제 게이트·비용 통제·실행 경계 모두 그대로다.
- **D4-c의 실행 경계 유지** — 워커는 SQL 텍스트만 산출하고 DB에 직접 쓰지 않는다
  (ADR-0052 D8). 생성기는 DB 접속 0건이다.
- **`books.has_audio` 갱신과 `book_review.status` 전이는 본 적재 범위 밖이다.** 앱이
  `has_audio`를 읽지 않고 `book_audio` 행 존재로 판정하므로(`lib/book/audio-manifest.ts`)
  화면 영향이 없다. `06_final_verify.sql` [7]에 현재 상태 **조회만** 넣었다.

### E8. 적재 실행 완료 기록 (2026-08-13) — D4 전량 적재 폐합

**`docs/sql/load708/` 19파일 전량 실행 완료. 실패 0 · 데이터 손실 0.**

| 항목 | 값 |
|---|---:|
| 실행 파일 | **19개** (`00` + `01_chunk_01~12` + `02` + `03`·`04`·`05` + `06` + `07`) |
| **최종 `voice='danielle'`** | **836권 / 9,592행** (`page` **8,756** + `cover` **836**) |
| 구 `voice='Ruth'` | **44권 / 574행 — 무접촉 불변** |
| `02` 전량 게이트 | **verdict PASS** |
| `06` 최종 검증 | **verdict PASS** (`dup_unique_key` 0 · `books_missing_cover` 0 · `null_duration` 0 · `gap_books` 0) |
| `07` staging 삭제 | 완료 — `to_regclass('public.book_audio_staging_708')` → **NULL** |

**머지 단계별 팀장 실측**(danielle 누계, 각 머지 COMMIT 직후 별도 COUNT 쿼리로 확인):

| 단계 | 코호트 | 누계 권 | 누계 행 |
|---|---|---:|---:|
| (기존) | pilot12 + fullbatch116 | 128 | 1,614 |
| `03` step1 | + african_storybook 527권 5,855행 | **655** | **7,469** |
| `04` step2 | + bloom 142권 1,616행 | **797** | **9,085** |
| `05` step3 | + book_dash 39권 507행 | **836** | **9,592** |

**E6 예측치와 완전 일치**(워커 로컬 재검산, DB 접속 0건):
128 + 527 = 655 ✅ / 1,614 + 5,855 = 7,469 ✅ · 655 + 142 = 797 ✅ / 7,469 + 1,616 = 9,085 ✅ ·
797 + 39 = 836 ✅ / 9,085 + 507 = 9,592 ✅ · 8,756 + 836 = 9,592 ✅

- **Ruth 574행 = `page` 530 + `cover` 44**(C1 정정 이력)로 적재 전후 변동이 없다. E5의 UNIQUE
  자연 분리(`(book_id, kind, page_index, voice)`)가 설계대로 작동한 것이다.
- **`ON CONFLICT` 없는 머지가 에러 없이 통과했다** = 기존 행과의 충돌 0건. E5의 "덮어쓰기
  구조적 불가" 보장이 실행으로 확인됐다.
- 성우 합산 행수는 9,592 + 574 = **10,166행**(계산치). **권 수는 단순 합산할 수 없다** —
  D9-a 34권이 양쪽에 겹친다(계산상 846권이나 실측 대상이 아니다).
- 본 실행으로 **D4-c 2단계("`book_audio` 적재")가 종료**되며, D4-c 1단계(Storage 업로드,
  15,956키)는 2026-08-12에 이미 폐합됐다.

### E9. 1차 실행 사고 — 리허설 `ROLLBACK`을 `COMMIT`으로 오인 (2026-08-12~13)

- **증상**: 1차 실행에서 머지 `03`~`05`를 **파일 원형(끝이 `ROLLBACK;`)인 리허설 상태 그대로**
  실행한 뒤, 끝줄을 `COMMIT;`으로 고쳐 재실행하는 **교체 단계를 건너뛰고** `07`(staging drop)까지
  진행했다. 결과적으로 **`book_audio` 적재 0건 상태에서 staging 테이블이 소실**됐다.
- **원인**: Supabase SQL Editor가 **다중 문장 실행 시 마지막 문장의 결과만 표시**한다.
  머지 파일은 `BEGIN → INSERT SELECT → 검증 SELECT → ROLLBACK`이라 화면에 남는 정보가 빈약했고,
  판정 근거로 **파일 안의 기대값 주석 대조표를 실행 결과로 오인**했다. **오케스트레이터의
  판독 오류가 여기에 포함된다** — 워커가 화면 표시와 파일 주석만으로 COMMIT 성립을 단정했다.
- **복구**: 청크 12개 전량이 `docs/sql/load708/`로 **git에 보존**돼 있어 `00`부터 전량 재실행으로
  해소했다. 청크가 `ON CONFLICT (audio_path) DO NOTHING` 멱등이라 재적재에 제약이 없었다(E5).
  **데이터 손실 0 · 기존 행 훼손 0 · 추가 과금 0**(DB 작업이므로 Polly 재호출 없음).
- **교훈 · 규약(신설)**:
  1. **머지류의 COMMIT 성립 판정은 파일 주석·화면 표시로 하지 않는다.** 반드시 **별도 실측
     COUNT 쿼리**를 단독 실행해 확정한다. 2차 실행에서 각 머지 직후 팀장 실측(E8 단계별 표)을
     넣은 것이 그 적용이다.
  2. **`07`(정리) 실행 전에는 `06` PASS와 적재 행수 실측을 각각 확인한다.** `07` 파일 머리의
     실행 조건 주석은 이미 이를 명시하고 있었으나, **주석은 게이트가 아니다** — 실측이 게이트다.
  3. 다중 문장 파일은 **마지막 문장이 곧 화면 결과**라는 SQL Editor 특성을 전제로 읽는다.
     중간 검증 SELECT의 결과는 별도 실행하지 않으면 보이지 않는다.
- 본 항은 D4-b(AWS 계정 정지 사고)와 같은 성격의 **사실 기록**이며, 결정 사항을 바꾸지 않는다.
