...
**Amendment #3(2026-08-08)** — 적용 범위를 **669권 6,899면 → 703권 7,308면**으로 재확대.
Book Dash html 활성 **34권 409면**을 편입하며, 해당 권의 기존 `voice='Ruth'` 음원을
**Danielle로 재생성**한다(팀장 승인). 대상 34권은 slug 단위로 확정됐다(ADR-0056 O4).
O3은 여전히 미확정.
...
## Amendment #3 (2026-08-08, 팀장 결정) — Book Dash html 활성 34권 편입 (669 → 703권)
...
### C1. 결정 — Danielle 적용 범위를 **703권 7,308면**으로 확대한다
...
| **Book Dash html 활성 (본 Amendment 신규)** | **34** | **409** | **팀장 SQL 실측 2026-08-08** |
| **합계** | **703** | **7,308** | |

- 669 + 34 = **703** ✅ / 6,899 + 409 = **7,308** ✅
- 신규 추가분 **409면은 전체 7,308면의 5.6%**(409 ÷ 7,308 = 5.60%)다.
- 대상 정의는 **ADR-0056 D9**에 있고, **확정 34권 slug 명단은 ADR-0056 O4**에 박제돼 있다 —
  `source_platform='book_dash'` AND `content_type='html'` AND `kind='page'` 오디오 보유
  AND `book_text` 0행 AND `is_active=true`.
- 면 구성: **12면 33권 + 13면 1권(`whose-button-is-this`) = 409면.**
- 표지 트랙도 **Amendment #1 A2대로 포함**한다(`--with-cover`). 34권분 표지 유닛이 추가된다.
- 결정일 **2026-08-08**, 결정자 **팀장**.
...
Amendment #2 B4의 원칙을 그대로 승계한다. 34권 합성 착수 전 아래가 모두 선행돼야 한다.

1. **`book_text` 적재** — ADR-0056 D9~D14 (현재 **Proposed**).
2. ~~활성 34권 확정 slug 명단 확보~~ — ADR-0056 **O4 ✅ Resolved (2026-08-08)**.
   34권이 slug·`source_id`·면수 단위로 확정됐고, 워커 전수 대조(json 부재 0 · `ruth_pages`
   34/34 일치 · 장면 합계 409 · **블랙리스트 교집합 0건**)를 통과했다. **본 조건은 충족됐다.**
3. **dry-run 승인** — D4 절차(대상 권수 / 총 페이지 / 정제 후 총 문자수 / 추산 비용 USD·KRW
   상·하한 산출 → 팀장 승인). **여전히 필수다.**

### C3. 기존 `voice='Ruth'` 음원을 재생성으로 대체하는 결정의 근거
...
- **확정 수치는 D4 드라이런 산출물이다.** 아래는 착수 여부 판단용 **참고치**이며,
  **승인 근거로 쓰지 않는다.**
- 참고치 — **확정 34권 명단 기준** 워커 로컬 실측(2026-08-08, `scripts/tts_pilot/out/{slug}.json`
  34개 전수. 종전 초판은 명단 미확정이라 44권 비례 환산치를 썼으나, **명단 확정으로 직접
  집계로 대체**한다):

  | 항목 | 값 | 비고 |
  |---|---:|---|
  | 대상 면수 | **409** | ADR-0056 O4 검산 통과 ✅ |
  | **미정제** raw 문자수 | **63,923** | `tts_targets.sanitize()`(D3) 미적용 |
  | 면당 평균 | 156.3자 | |
  | 최장 면 | 1,023자 (`together-were-strong` p4) | **Polly 요청당 3,000자 상한 초과 0건** ✅ |
  | 무텍스트 면 | **0** | ADR-0056 D7 적용 대상 0면 |
  | (참고) 오디오 보유 44권 전체 | 530면 / 73,500자 | 팀장 실측 `page`+`Ruth` **530행과 일치** ✅ |

- **추정: long-form ×2 보수적 상한 기준 약 $12.78(약 ₩17,900, 환율 가정 1,400원/USD).**
  전체 7,308면 대비 5.6%라는 비율과 정합한다.
- **이 값을 확정으로 쓰지 않는 이유**: ① **정제(D3) 후 문자수가 달라진다**
  ② **표지 트랙 34권분 문자수가 포함되지 않았다**(Amd#1 A2로 표지 포함이 확정돼 있다)
...
- **D1 사전 제외 3종 유지** — 회전 18권 · 오염 6권 · 이미 오디오 보유 12권.
  - ⚠ **D1 표 4행("텍스트 없음 54권")의 전제는 본 Amendment로 34권만큼 축소된다.**
    그 행은 "`book_text` 행 없음 = 낭독 대상 자체가 없음(그중 44권은 v1 배치로 이미 오디오
    보유)"으로 적혔는데, ADR-0056 D9가 34권의 `book_text`를 적재하므로 **그 34권에 한해
    제외 사유가 소멸**한다. **잔여 20권**(비활성 10 + 무오디오·블랙 등 10)은 제외 상태 유지다.
  - **D1 표 3행("이미 오디오 보유 12권")은 무접촉**이다. 그 12권은 시범 `danielle` 코호트이며
    본 Amendment의 34권(Ruth)과 서로소다.
- **D2(키 보관 정책) · D3(입구 정제 게이트) · D4(비용 통제) · D5(기존 유지 규칙) ·
  D6(실행 경계) 전부 무접촉.** 34권도 D3 정제와 D5 규약(페이지별 mp3, 1-based 파일명 /
...
- **Non-goals `:145`("구 44권 `p00` 축·`voice='Ruth'` 표기의 신 규약 재정렬") 유지.**
  재생성되는 Danielle 음원은 **신 규약**(`book_dash-{source_id}/danielle/pNN.mp3`, 1-based)을
  따르며, **구 Ruth 오브젝트(`p00~` 축)는 이동·개명하지 않는다.** 백로그를 열지 않는다.
- **본 Amendment는 34권의 합성 실행을 승인하지 않는다**(C2).
