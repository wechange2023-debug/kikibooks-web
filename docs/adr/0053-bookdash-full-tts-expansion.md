...
**Amendment #3(2026-08-08)** — 적용 범위를 **669권 6,899면 → 703권 7,307면**으로 재확대.
Book Dash html 활성 **34권 408면**을 편입하며, 해당 권의 기존 `voice='Ruth'` 음원을
**Danielle로 재생성**한다(팀장 승인). O3은 여전히 미확정.
...
### C1. 결정 — Danielle 적용 범위를 **703권 7,307면**으로 확대한다
...
| **Book Dash html 활성 (본 Amendment 신규)** | **34** | **408** | **팀장 SQL 실측 2026-08-08** |
| **합계** | **703** | **7,307** | |

- 669 + 34 = **703** ✅ / 6,899 + 408 = **7,307** ✅
- 신규 추가분 **408면은 전체 7,307면의 5.6%**(408 ÷ 7,307 = 5.58%)다.
- 대상 정의는 **ADR-0056 D9**에 있다 — `source_platform='book_dash'` AND `content_type='html'`
  AND `kind='page'` 오디오 보유 AND `book_text` 0행 AND `is_active=true`.
...
> 합산치다. 본 Amendment의 면수 기준 축은 **`page` 530행**이며, 그중 활성분이 **408면**이다.
...
2. **활성 34권 확정 slug 명단 확보** — ADR-0056 **O4**(차단 게이트). 팀장 SQL export로 확정하며,
   워커 파생 목록을 대상으로 쓰지 않는다.
...
- **확정 수치는 D4 드라이런 산출물이다.** 아래는 착수 여부 판단용 **참고치**이며,
  **승인 근거로 쓰지 않는다.**
- 참고치(워커 로컬 실측, `scripts/tts_pilot/out/*.json` 44권 전수):

  | 항목 | 값 | 비고 |
  |---|---:|---|
  | 오디오 보유 44권 면수 | 530 | 팀장 실측 `page`+`Ruth` **530행과 일치** ✅ |
  | 44권 **미정제** raw 문자수 | 73,500 | `tts_targets.sanitize()` 미적용 |
  | 면당 평균 | 138.7자 | |
  | 최장 면 | 1,023자 (`together-were-strong` p4) | **Polly 요청당 3,000자 상한 초과 0건** |
  | 활성 408면 환산(면당 평균 적용) | 약 **56,600자** | **추정: 명단 미확정 상태의 비례 환산치** |

- **추정: 위 환산치 기준 long-form 추산은 ×2 보수적 상한에서 약 $11(약 ₩15,800, 환율 가정
  1,400원/USD)** 수준이다. 전체 7,307면 대비 5.6%라는 비율과 정합한다.
- **이 값을 확정으로 쓰지 않는 이유**: ① 정제(D3) 후 문자수가 달라진다 ② 활성 34권 확정
  명단이 미확정이다(ADR-0056 O4) ③ 표지 트랙 문자수가 포함되지 않았다.
