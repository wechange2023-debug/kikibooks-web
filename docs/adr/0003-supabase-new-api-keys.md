...
- **잔여(후속 조치) → Amendment #2에서 해소**: 노출됐던 옛 secret 키(대시보드 라벨 `default`)의 폐기(revoke)는 본 Amendment 시점엔 미완(PM 결정 보류)이었다. 2026-06-10 폐기 완료 — 아래 §7 Amendment #2 참조.
...
## 7. 갱신 (2026-06-10) — 옛 secret 키 폐기 완료 (Amendment #2)

Amendment #1(§6)에서 보류했던 후속 조치, 즉 노출됐던 옛 secret 키의 폐기(revoke)를 완료했다.

- **폐기 일자**: 2026-06-10
- **대상**: 노출됐던 옛 secret 키 (Supabase 대시보드 라벨 `default`) — 키 값·평문 비기록(Hard Rule 6)
- **실행 주체·방법**: PM이 Supabase 대시보드에서 직접 revoke (코드 변경 0줄)
- **안전성**: Amendment #1에서 신규 키(`SUPABASE_SECRET_KEY`)로 교체·전체 검증(가입·인증·DB·재로그인) 통과 완료 + 옛 키 사용처 0건 확인 후 폐기 → 무영향
- **결과**: §5 메모 3 "노출 시 1순위 행동"(키 회전 → 환경변수 갱신) 사이클이 옛 키 폐기로 완결됐다. `docs/backlog.md` §4의 폐기 잔여 항목도 동일 일자로 Resolved 처리.
