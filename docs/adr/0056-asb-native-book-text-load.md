...
**Accepted** (2026-08-10) / 최초 제안 2026-08-08 · 기준 HEAD `e2f4c14`
...
**2026-08-08 개정 (당시 Proposed 유지 — 2026-08-10 Accepted)** — 적재 범위를 **669권 → 708권**으로 확대한다.
Book Dash html 코호트 중 **활성 39권(469면)** 을 편입하며(**D9~D14** 신규), 팀장 승인 3건에 근거한다.
...
3. **(2026-08-08 추가 확정)** 잔여 **활성·무오디오 5권(60면)** 도 본 경로로 편입한다
   (ADR-0054 Amd#1 E4-a **(나)** 채택). 이로써 ADR-0054 D2의 활성 전환 대상은 **0권**이 된다.

편입 39권은 성격이 둘로 갈린다 — **34권 = 기존 Ruth 음원 재생성**(축 정합 실측 완료) /
**5권 = 신규 합성**(기존 오디오 없음). **D9-a·D9-b로 분리 기재**하며, 5권 편입분만 단독
철회할 수 있다.

### 변경 이력

- **2026-08-08** Proposed. 적재 범위 669권 → 708권 확대(D9~D14 신규).
- **2026-08-10 팀장 승인. O4·E4 해소 및 708권 검산 폐합 근거.** → **Status `Accepted`**
  - 착수 전 워커 전수 검증 통과: 39권 원천 json **39/39 실존 · git clean**(M/`??` 0건),
    O4 표 파싱 검산 409면 · §5-f 표 60면 일치, 미추적 slug json 12건과의 **교집합 0건**.
  - ~~⚠ O6(D9-b 5권 이미지 육안 확인)은 여전히 ⏸ 대기다~~ → **2026-08-10 O6 Resolved**(65객체
    전부 정상 로드). D9-b 적재 차단이 해제되어 **708권 7,368면 전량이 착수 가능**하다.
- **2026-08-10 팀장 실행 — 15파일 전량 COMMIT, 최종 검증 ①~⑧ 통과.**
  D9-b 육안 검증 통과(이미지↔텍스트 대조 방식, `admin/review` 미시드로 대체 절차 사용).
  **총계 708권 7,368행 확정. 본 ADR의 적재 트랙 종결.**
  - 실행 산출: Bloom 소량 10권 135행(선행 게이트) + 전량 15파일 698권 7,233행 = **708권 7,368행**
  - 라벨 분포: `manifest_txt_v1` 669권 6,899행 · `html_scene_json_v1` 39권 469행
  - 잔여 Open은 전부 비차단이며, **O5만 실사용 발현** 상태로 다음 세션 설계 검토 안건이다.
- **2026-08-13** O5 종결 — **ADR-0058 D5로 흡수**(708권 `status='draft'` 시드 결정·실행,
  `book_review` 860권 체제 확정). 본 ADR의 D8(미시드)은 그 결정으로 대체된다. 문서 반영 2026-08-14.
...
| └ `is_active = true` | **34권 409면** ← **본 개정의 적재 대상** (12면 33권 + 13면 1권) |
| └ `is_active = false` | **10권 121면** ← 이번 범위 제외(D9) (12면 9권 + 13면 1권) |
| 면수 분포(44권) | 12면 **42권** / 13면 **2권** |
| `source_id` 형식 | 44권 **전량 UUID** |
| ASb·Bloom `asb_native` 활성 | **669** (검수 미반영, 2026-08-08 CSV 유효) |

검산: 42×12 + 2×13 = **530** ✅ / 409 + 121 = **530** ✅ / 1,486 + 530 + 128 + 44 = 2,188.

**13면 2권의 활성/비활성 배분(확정)**: `whose-button-is-this` **활성** ·
`who-is-our-friend`(Who is our Friend?) **비활성**. 두 권이 한쪽에 몰려 있지 않다.
`content_url` 포함 export로 slug 단위 확정됐다(팀장 SQL 실측 2026-08-08).
...
#### 5-b. 정정 이력 ② — **「408면」 정정은 철회됐다. 409면이 정본**

> **⚠ 본 항은 철회된 정정의 이력이다.** 삭제하지 않고 남기는 이유는, 같은 형태의 추정 오류를
> 다시 만들지 않기 위해서다(§5-c 운영 교훈).

| 시점 | 기재 | 상태 |
|---|---|---|
| 2026-08-08 정찰 #1 | 활성 34권 **409면** | ✅ **옳았다** |
| 2026-08-08 지시서 #2 | 활성 34권 **408면** (종전 409를 정정) | ❌ **철회** |
| 2026-08-08 지시서 #3 | 활성 34권 **409면** (408 정정을 철회) | ✅ **정본** |

- **철회 대상**: 본 문서 초판(2026-08-08)이 "정찰 #1의 34권 명단이 실측과 어긋났다 —
  정본은 408면"이라고 적은 정정 이력. **이 정정은 성립하지 않는다.**
- **철회 사유**: **408은 실측이 아니라 산정값이었다.** "면수 분포 12면 42권 / 13면 2권"에서
  **13면 2권이 모두 비활성**이라는 오케스트레이터 **가정**을 적용해 `34 × 12 = 408`을 도출한
  것이다. 실측(`content_url` 포함 export)에서 **13면 2권 중 1권(`whose-button-is-this`)이
  활성**으로 확인되어 그 가정이 깨졌다.
- **정본 배분**(팀장 SQL 실측 2026-08-08, slug 단위 확정):

  ```
  활성 34권 = 12면 33권 + 13면 1권(whose-button-is-this)      = 409면
  비활성 10권 = 12면  9권 + 13면 1권(who-is-our-friend)        = 121면
  검산 409 + 121 = 530 = book_audio (kind='page', voice='Ruth') 행수 ✅
  ```

  ※ 종전 배분(408 + 122)도 합이 530이라 **총계 검산만으로는 오류가 드러나지 않았다.**
  이것이 오류가 한 회차를 통과한 직접 원인이다.
- **워커 재검산 결과(2026-08-08 지시서 #3, 읽기 전용)**: 확정 34권 명단의
  `scripts/tts_pilot/out/{slug}.json` 장면 수 합계 = **409** ✅ ·
  `ruth_pages` 값과 **34/34 일치, 불일치 0건** · json 부재 **0건**.
- **부수 정정 — `blacklist.ts:32-33`의 서술은 현재 상태와 어긋난다.**
  초판은 그 주석("주간 cron이 `is_active=True`로 되돌리므로 `is_active=false` 대신 코드 측
  블랙리스트로 차단한다 — cron-proof")을 근거로 "블랙리스트 15권은 `is_active=TRUE`로 유지된다"고
  적었다. **실측은 반대다** — 비활성 15권이 블랙리스트 15권과 **1:1 완전 일치**한다(§5-e).
  추정: Book Dash cron이 2026-08-07 기준 disabled라(§Consequences) 주석의 전제가 소멸했거나,
  이후 별도 비활성화 작업이 적용된 것으로 보인다. **주석 갱신은 본 ADR 범위 밖**이며 코드
  작업 트랙으로 이연한다.
- **결론**: 정찰 #1의 34권 파생 명단은 **권수·구성·면수가 모두 실측과 일치**했다.
  「구성이 다르다」는 초판 서술은 **철회**한다.

#### 5-c. 운영 교훈 — 면수 분포로 활성/비활성 배분을 추정하지 않는다

- **`is_active`와 면수는 독립 축이다.** 한쪽 분포(면수)에서 다른 축(활성 여부)의 배분을
  역산할 수 없다. 반드시 **교차표**(`is_active` × 면수, 또는 slug 단위 export)로 확인한다.
- **총계 검산은 배분 오류를 잡지 못한다.** 408+122와 409+121이 둘 다 530이 되듯,
  합이 맞는 잘못된 배분은 얼마든지 만들 수 있다. 검산은 **합계와 배분을 각각** 확인해야 한다.
- **유일해라는 사실이 참을 보증하지 않는다.** 「408 = 34×12가 유일해」는 참이었으나,
  그 전제("13면 2권이 모두 비활성")가 거짓이었다. 유일성 논증은 전제를 검증하지 않는다.
- **적용**: 이후 코호트 산정에서 권수·면수를 다룰 때, 팀장 실측 요청은 **집계값이 아니라
  slug(또는 id) 단위 export**로 받는다. 본 개정의 O4 해소가 그 방식이었다.

**교훈 ② — 원천 파일 인용 시 실존 여부만이 아니라 git 상태(`M`/`??`)까지 확인한다**

- **커밋본과 작업본의 괴리는 `M` 플래그로만 드러난다.** 파일을 열어 읽으면 언제나 작업본이
  보이므로, 두 버전이 갈라져 있다는 사실 자체가 보이지 않는다.
- **이번 건은 실존·장면 수 검산을 모두 통과했으나 내용이 달랐다.** O4 검산(§5-b)은
  json 부재 0건 · `ruth_pages` 34/34 일치 · 장면 합계 409를 확인했지만, 그 검산은 **면의 개수만**
  본다. `a-beautiful-day`는 커밋본에서도 12면이었고 텍스트만 비어 있었다.
- **왜 위험한가**: "원천은 git에 커밋돼 있으므로 안전하다"(D10)는 서술이 참이려면
  **커밋본 = 작업본**이어야 한다. 다른 세션·다른 클론이 커밋본으로 적재하면 다른 결과가 나온다.
- **적용**: 원천 파일을 결정문에 인용할 때 아래 3단을 모두 확인한다.
  1. **실존** — 파일이 있는가
  2. **구조** — 면 수·스키마가 기대와 맞는가
  3. **git 상태** — `git status --short <경로>` 가 무출력인가. `M`이면 **어느 쪽이 정본인지
     먼저 판정**하고, 생성 스크립트의 커밋 시계열과 대조해 원인을 특정한다.
- **부수 원칙**: 산출물을 커밋할 때는 **생성 스크립트가 clean인지 함께 확인**한다.
  스크립트가 미커밋이면 산출물만 커밋해도 재현이 불가능해진다.

#### 5-d. 원천 실존 — 오디오 보유 44권 전량 json 보유
...
"이미지 전무(무텍스트책) 5권"). 따라서 **오디오 보유 44권은 전량 json을 보유**한다. → D10의 전제.

확정 명단 기준 재검증(2026-08-08 지시서 #3): **활성 34권 json 부재 0건**,
`ruth_pages` 대조 **34/34 일치**, 장면 합계 **409** ✅.

#### 5-e. 비활성 15권 = 블랙리스트 15권 (slug 1:1 확정)

**개수 일치로 단정하지 않고 slug 단위로 대조했다.** 활성 39권(= 확정 34권 + 무오디오 5권)이
slug로 전부 확정되므로, 비활성 15권은 `html 54 − 활성 39`로 **역산 확정**된다.

| 대조 | 결과 |
|---|---|
| 역산 비활성 15권 vs `lib/shared/blacklist.ts` `BOOK_DASH_404_SOURCE_IDS` 15권 | **완전 일치** (차집합 양방향 0건) |
| 블랙리스트 ∩ 활성 34권 | **0건** |
| 블랙리스트 ∩ 활성·무오디오 5권 | **0건** |
| 비활성 15권 중 json 부재 | **5권** = 팀장 실측 "비활성·무오디오 5권"과 일치 |
| 비활성 15권 중 json 실존 | **10권** = 팀장 실측 "비활성·오디오 보유 10권"과 일치 · 장면 합계 **121** ✅ |

- **의의 ①**: 적재 대상 34권에 **차단 도서가 1건도 섞이지 않는다.** 보이지 않는 책에 `book_text`를
  적재하거나 Danielle 비용을 투입할 위험이 없다 → **O4 Resolved**의 직접 근거.
- **의의 ②**: `content_type='html'` 54권의 **활성/비활성 경계가 블랙리스트 경계와 정확히 같다.**
  즉 이 코호트에서는 차단이 **코드(5표면 필터) + 데이터(`is_active`) 이중**으로 걸려 있다.
- **주의**: 이는 **2026-08-08 시점 실측**이다. `blacklist.ts:32-33`이 서술한 cron 되돌림이
  재개되면 어긋날 수 있다(§5-b 부수 정정). 적재 직전 재확인을 권고한다.

#### 5-f. 활성·무오디오 5권 (2026-08-08 편입 확정 — ADR-0054 Amd#1 E4-a (나))

`content_type='html'` 활성 39권 중 **`book_audio` 행이 0인 5권**이다. 종전에는 ADR-0054 D2
(WP PDF 재하베스트 → `asb_native` 전환) 대상으로 남아 있었으나, 팀장 결정으로 본 경로에 편입됐다.

| # | 제목 | slug | `source_id` | 면 | raw 문자 |
|---:|---|---|---|---:|---:|
| 1 | A Beautiful Day | `a-beautiful-day` | `9c9e94e0-fe46-11e5-86aa-5e5517507c66` | 12 | 882 |
| 2 | A Dancer's Tale | `a-dancers-tale` | `9c9e8586-fe46-11e5-86aa-5e5517507c66` | 12 | 2,264 |
| 3 | A Fish and a Gift | `a-fish-and-a-gift` | `9c9e6754-fe46-11e5-86aa-5e5517507c66` | 12 | 3,599 |
| 4 | A House for Mouse | `a-house-for-mouse` | `9c9e72e4-fe46-11e5-86aa-5e5517507c66` | 12 | 524 |
| 5 | A Tiny Seed | `a-tiny-seed` | `9c9e7a6e-fe46-11e5-86aa-5e5517507c66` | 12 | 2,220 |
| | | | **합계** | **60** | **9,489** |

- **slug·`source_id` 출처**: `scripts/copy_bookdash_images.py:94-100`(`DONE_BOOKS` — slug +
  `source_id` 쌍 하드코딩). 워커 로컬 실측으로 json 실존 **5/5**, 장면 수 **12×5 = 60** 확인.
- **블랙리스트 교집합 0건** — 5권 전량 사용자 노출 대상이다(§5-e).
- ⚠ **무텍스트 면 1건**: `a-house-for-mouse` **p10**(`page_index = 9`, `10.jpg`). 그림만 있는
  면이며 **D7 적용 대상**이다(`text = ''` 행 적재, TTS 대상 선정에서만 제외). 나머지 4권은 0면.
- **34권과의 결정적 차이 — 기존 오디오가 없다.** 34권은 Ruth 음원의 `page_index` 축(0..11)이
  적재될 `book_text`와 정합함을 **실측으로 대조**할 수 있었으나(D11 근거 ③), 5권은 대조할
  기준 자체가 없다. 따라서 **적재 후 육안 검증이 필수**다 → **D9-b**.

#### 5-g. 5권 이미지 실재 — 검산 폐합 + **팀장 육안 확인 대기 (2026-08-08)**

편입의 전제는 "5권 본문 이미지가 `book-images` 버킷에 이미 존재"다. **기록 기반 검산은
폐합한다.**

```
정예 39권 508객체 = 38권×13 + whose-button-is-this 14        ← ADR-0036 Amd#1:128 기재식
  (13 = 본문 12 + 표지 1,  14 = 본문 13 + 표지 1)
표지 39 · 본문 469 = 38×12 + 13
본문 469 = 34권 409(§5-b) + 5권 60(§5-f)   ✅      469 + 39 = 508   ✅
```

- **출처 주의**: 위 **분해식의 출처는 `docs/adr/0036-bookdash-image-storage.md:128`** 이다.
  `docs/recon/state-audit.md:110`은 "본문 보유 39권(508객체) + cover-only 10"까지만 적고
  **본문/표지 분해를 기재하지 않는다.** 두 문서는 모순되지 않으며 상세도만 다르다.
  동일 분해식이 `ADR-0038:42` · `docs/recon/2026-07-09-viewer-architecture-evidence.md:134` ·
  `scripts/copy_bookdash_images.py:346`에도 있다.
- 5권이 정예 39권에 포함되는 근거: `copy_bookdash_images.py:93-100`이 이 5권을 **"완료 5권"**
  (`DONE_BOOKS`)으로 명시한다 — CSV 49권보다 먼저 업로드가 끝난 권들이다.
- ⏸ **팀장 육안 확인 대기 (2026-08-08)**: `book_dash-9c9e94e0-fe46-11e5-86aa-5e5517507c66/`
  (`a-beautiful-day`) 폴더의 `01.jpg`~`12.jpg` 실재를 팀장이 직접 확인 중이다.
  **본 검산은 기록 기반이며 워커는 Storage 접근 권한이 없다**(정찰 #1에서 "추정"으로 표기한
  항목과 동일 성격).
- **확인 실패 시 조치**: **D9-b(5권)만 단독 철회**한다. D9-a(34권)는 영향받지 않는다 —
  34권의 이미지 실재는 별개 근거(기존 Ruth 오디오가 이미 그 이미지 위에서 서비스되던 이력)를
  갖는다. 철회 시 5권은 ADR-0054 D2 경로로 되돌아가며, 그 트랙은 폐기되지 않았다
  (ADR-0054 Amd#2 E8).

#### 5-h. 정정 이력 ③ — D9-b 원천 2건의 **커밋본이 낡아 있었다** (2026-08-08 해소)

**발견 경위**: 세션 종료 점검(`git status`)에서 `scripts/tts_pilot/out/` 아래 tracked 파일
2건이 `M`(수정·미커밋) 상태임이 드러났다. 두 건 모두 **D9-b 5권의 원천**이다.
D9-a 34권은 **수정 0건**으로 영향이 없다.

| slug | 커밋본(종전) | 작업본(정본) | 차이 |
|---|---|---|---|
| `a-beautiful-day` | 688자 · **무텍스트 2면**(p4·p12) | **882자 · 무텍스트 0면** | +194자 · −2면 |
| `a-fish-and-a-gift` | 3,599자 · **`image_url` 12면 이중경로** | 3,599자 · URL 정상 | 텍스트 동일 · **URL 12건** |
| 나머지 3권 | 동일 | 동일 | — |
| **합계** | **9,295자 · 무텍스트 3면** | **9,489자 · 무텍스트 1면** | **+194자 · −2면** |

**원인 — 산출물이 스크립트 수정 이전 버전이었다** (커밋 시계열로 확정)

```
7e26495  2026-07-02  a-beautiful-day.json 최초·유일 커밋      ← 커밋본은 이 시점 산출물
44928d3  2026-07-07  extract_text.py에 <img alt> 폴백 추가     ← 텍스트 증가의 원인
124336e  2026-07-13  a-fish-and-a-gift.json 커밋 (.gitignore 예외)
b3ba162  2026-07-28  urljoin URL 수정 + 5xx 재시도            ← URL 버그 수정
```

- **`a-beautiful-day` 텍스트 증가 = `44928d3`의 alt 폴백.** 현재 코드 위치는
  `extract_text.py:64`(`_IMG_ALT_RE`) · `:152`(alt 수집) · `:171-175`(폴백 적용):

  ```python
  if not text.strip() and s["_alts"]:      # :172
      text = "\n".join(s["_alts"])         # :175
  ```

  p4·p12는 `<p>` 본문이 없고 `<img alt>`만 있는 면이라, 폴백 도입 후 재실행에서 채워졌다.
- **`b3ba162`(URL 수정)는 텍스트 증가의 원인이 아니다** — 반증 확인 완료. 해당 커밋의 변경은
  `fetch_html`(재시도) · `GH_PAGES_BASE` · `to_abs_image_url`(urljoin) **3곳뿐**이며,
  텍스트 추출 경로(`extract_scenes` · `clean_text` · `isolate_body` · `_P_BLOCK_RE`)는 **무변경**이다.
- **`a-fish-and-a-gift` URL 이중경로**는 `b3ba162` **이전**에 커밋된 산출물이기 때문이다.
  그 버그는 `extract_text.py:113-121` 주석에 이미 문서화돼 있다
  (`.../a-fish-and-a-gift/en/bookdash-books/a-fish-and-a-gift/en/images/01.jpg`).

**결론 — 작업본이 정본이다.** 커밋본은 스크립트 수정 이전 산출물이며 폐기 대상이다.
본 개정 커밋에 2건을 포함해 **커밋본 = 작업본**으로 맞춘다(팀장 승인 2026-08-08,
baseline 47 → 45 축소).

- **재현 가능성 확인**: `scripts/tts_pilot/extract_text.py`가 워킹트리에서 **clean**이다
  (`git status`·`git ls-files -m` 양쪽 무출력, 2026-08-08 확인). 산출물만 커밋해도
  **생성 스크립트가 이미 정본 상태**이므로 재현 가능하다. D10의 재크롤 금지와 무관하다.
- **적재 영향**: `image_url`은 `book_text` 적재에 쓰이지 않으므로(이미지는 D14 키 규약으로
  조립) URL 버그는 적재 결과를 오염시키지 않았다. **텍스트 194자·2면 차이가 실질 영향**이며,
  본 ADR과 ADR-0053 Amd#3 C4의 수치는 **전부 작업본 기준**이다.
- **`_fullbatch_dryrun_report.json`도 `M` 상태이나 커밋하지 않는다** — 재생성 가능한
  리포트이며 적재 원천이 아니다(팀장 결정).

### 6. 적재 범위 합산 — 669권 → 708권
...
| **Book Dash html 활성·오디오 보유 (D9-a)** | **34** | **409** | §Context 5-b (팀장 SQL 실측 · 워커 json 재검산) |
| **Book Dash html 활성·무오디오 (D9-b)** | **5** | **60** | §Context 5-f (워커 json 실측 · ADR-0054 Amd#1 E4-a) |
| **소계 (D9~D14 범위)** | **39** | **469** | = `book-images` 정예 39권 본문 객체 수와 폐합(§5-g) |
| **합계** | **708** | **7,368** | |

- 669 + 39 = **708** ✅ / 6,899 + 469 = **7,368** ✅ / 409 + 60 = **469** ✅
- 신규 편입분은 전체 면수의 **6.4%** (469 ÷ 7,368 = 6.37%).
  - 34권분 **5.6%**(409 ÷ 7,368 = 5.55%) · 5권분 **0.8%**(60 ÷ 7,368 = 0.81%).
- **39권은 `content_type='html'` 활성 전량**이다(활성 39 = 34 + 5, §Context 5-e).
  즉 본 개정 후 **Book Dash html 활성 도서 중 `book_text`가 없는 권은 0**이 된다.
...
| **D7** 빈 면도 `text=''` 행 적재 | **[전 코호트]** | 규칙은 공통. html 39권의 적용 대상은 **1면** — `a-house-for-mouse` **p10**(`page_index=9`). 34권분은 0면(정찰 #1 실측), 5권분에서 1건 발생(§Context 5-f) |
| **D8** `book_review` 시드하지 않음 | **[ASb·Bloom]** | **html 39권에 대해서는 본 개정이 결정하지 않는다** → **O5** |
| **D9~D14** | **[Book Dash html]** | 본 개정 신규. **D9는 D9-a(34권)·D9-b(5권)로 분리** |
...
- 근거 ③: `book_text.text`는 **NOT NULL DEFAULT `''`**이므로 스키마상 허용된다
  (마이그레이션 006, ADR-0046).
- 적용 대상: **ASb·Bloom 21권 97면**(ASb 2권 2면 / Bloom 19권 95면 — §Context 3 실측, 잠정치)
  \+ **Book Dash html 1권 1면**(`a-house-for-mouse` p10 = `page_index 9`, §Context 5-f 실측).
  합계 **22권 98면**.
  - Book Dash html 34권분은 **0면**(정찰 #1 전수 실측). 1면은 전부 5권분(D9-b)에서 나왔다.
  - `a-house-for-mouse` p10은 **정제 후 빈 면이 아니라 원천부터 빈 면**이다 — GH Pages 원본에
    해당 장면의 `<p>` 텍스트가 없고 `<img alt>` 폴백도 비어 있다(`extract_text.py:171-175`).
    그림만 있는 면이며, ADR-0035 Amd#1 A4가 규정한 "텍스트 없는 면은 에러가 아니라 그림만
    있는 면으로 정상 렌더"에 해당한다.
  - **수치 기준 주의**: 위 "1면"은 **작업본(= 본 개정 커밋본) 기준**이다. 종전 커밋본으로는
    3면이었다 → **§Context 5-h 정정 이력**.
...
### D9. [Book Dash html] 활성 39권(469면)을 `book_text` 적재 범위에 포함한다

- 대상 = `source_platform='book_dash'` AND `content_type='html'` AND `book_text` 0행
  AND **`is_active = true`** → **39권 469면**. 이는 **html 활성 전량**이다(§Context 5-e).
- **성격이 다른 두 집합으로 구성되며, D9-a·D9-b로 분리한다.** 분리 기재의 목적은
  **5권 편입분만 단독 철회 가능하게** 두는 것이다(§Context 5-g 육안 확인 대기).

  | | 권수 | 면수 | 기존 오디오 | 적재 후 필요 작업 | 근거 |
  |---|---:|---:|---|---|---|
  | **D9-a** | 34 | 409 | `voice='Ruth'` 530행 중 활성분 | **재생성**(축 정합 실측 완료) | O4 명단 |
  | **D9-b** | 5 | 60 | **없음** | **신규 합성** + **육안 검증** | §Context 5-f |
  | 계 | **39** | **469** | | | |

- 이로써 본 ADR의 총 적재 범위는 **669권 6,899면 → 708권 7,368면**이 된다(§Context 6).
- **비활성 10권(121면)은 이번 범위에서 제외한다.**
...
### D9-a. [Book Dash html] 오디오 보유 34권(409면) — 기존 Ruth 음원 재생성

- 확정 명단은 **O4**에 박제. 면 구성 = 12면 33권 + 13면 1권(`whose-button-is-this`).
- 기존 `voice='Ruth'` 음원(활성분 409행)은 **D13대로 무접촉 보존**하고, Danielle을
  **신규 INSERT**한다(ADR-0053 Amendment #3).
- **축 정합이 실측으로 확인됐다** — 기존 Ruth `page_index`(0..11)와 적재될 `book_text`
  면수가 34/34 일치한다(O4 검산). 따라서 **적재 후 축 검증이 불필요**하다.

### D9-b. [Book Dash html] 무오디오 5권(60면) — 신규 합성 + **육안 검증 필수**

- 확정 명단은 **§Context 5-f**. 면 구성 = 12면 × 5권.
- **기존 오디오가 없다.** 따라서 D9-a와 달리 **대조할 기준선이 존재하지 않는다** —
  "적재된 `book_text`의 면 순서가 실제 그림 순서와 맞는가"를 **데이터만으로 판정할 수 없다.**
- **그래서 육안 검증을 결정 사항으로 못박는다.**

  | 항목 | 내용 |
  |---|---|
  | 시점 | `book_text` 적재 완료 후, **Danielle 합성 착수 전** |
  | 표본 | **최소 1권**. 권장 표본 = **`a-house-for-mouse`** |
  | 표본 선정 사유 | 5권 중 유일하게 **무텍스트 면(p10)** 을 가져 D7 동작까지 한 번에 확인된다 |
  | 확인 축 | ① 면 N의 자막이 면 N의 그림과 맞는가 ② p10이 **에러가 아니라 그림만 있는 면**으로 렌더되는가 ③ 슬라이드 총 수 = 12(+표지) |
  | 확인 위치 | `/admin/review`의 해당 권 상세(이미지+텍스트 병렬 표시). 오디오 리더는 합성 전이라 진입 불가 |
  | 실패 시 | **D9-b만 철회**한다(§Context 5-g). D9-a·ASb·Bloom은 영향 없음 |

- **합성 전 검증인 이유**: 축이 틀린 채로 합성하면 mp3·marks 60면분이 전부 폐기 대상이 된다.
  적재는 `DELETE`로 되돌릴 수 있으나 합성 비용은 되돌릴 수 없다.
- **5권은 34권보다 제약이 적다**(비블랙 · json 실존 · 이미지 정예 39권 포함 · Ruth 대체 문제
  없음). 육안 검증 1건이 유일한 추가 부담이다.
...
- **★ "전량 실존 + git 커밋됨"의 적용 범위 (2026-08-08 정정)**
  - **D9-a 34권**: 무조건 성립한다. 34/34 실존 · **git 수정 0건**(tracked·clean) — 즉
    **커밋본과 작업본이 동일**하다. 어느 클론에서 읽어도 같은 결과가 나온다.
  - **D9-b 5권**: 5/5 실존하나, **2건(`a-beautiful-day`·`a-fish-and-a-gift`)의 커밋본이
    낡아 있었다**(§Context 5-h). **본 개정 커밋으로 작업본을 반영해 D9-a와 동일 상태가 된다.**
  - 즉 **본 개정 커밋 이후에는 39권 전량이 "커밋본 = 작업본"** 이다. 그 이전 커밋을 기준으로
    적재하면 D9-b 2권에서 텍스트 194자·무텍스트 2면이 어긋난다.
  - **적재 착수 전 확인**: `git status --short scripts/tts_pilot/out/` 무출력.
    출력이 있으면 §Context 5-c 교훈 ②의 3단 확인을 수행한다.
- **실존 확인(정찰 #1 실측)**: html 54권 중 **49권** 존재. 부재 5권은 전부 "블랙리스트 중
  무오디오" 권이라 대상 밖이다. **오디오 보유 44권은 전량 실존**하며, 그 부분집합인 활성
  34권도 **명단과 무관하게 전량 실존**한다(§Context 5-d).
- **git 상태 (2026-08-08 확인 — 적용 범위 주의)**

  | 대상 | 실존 | git 상태 | 판정 |
  |---|---|---|---|
  | **D9-a 34권** | 34/34 | **수정 0건** (tracked·clean) | ✅ **커밋본 = 작업본**. 그대로 사용 가능 |
  | **D9-b 5권** | 5/5 | 3권 clean · **2권 `M`**(작업 미커밋) | ⚠ **§5-h 정정 이력 참조** — 본 개정 커밋으로 해소 |

  - 즉 **"전량 실존 + git 커밋됨"이 무조건 성립하는 것은 D9-a 34권**이다.
    D9-b 5권 중 `a-beautiful-day` · `a-fish-and-a-gift` 2건은 **커밋본이 낡아 있었고**,
    본 개정과 함께 작업본을 커밋해 **D9-a와 동일 상태로 맞춘다.**
- **`.gitignore` 예외로 커밋 대상임이 확인됐다** — `.gitignore:6`의 전역 `out/` 규칙에 걸리나
...
### D13. [Book Dash html · D9-a 한정] 기존 `voice='Ruth'` 행은 수정·삭제·이동하지 않는다

> **적용 범위**: **D9-a(34권) 전용.** D9-b(5권)는 `book_audio` 행이 0이라 **해당 없음**이며,
> Danielle 합성 시 UNIQUE 충돌 대상도 없다.
...
- **D9-a·D9-b 모두 동일 적용.** 정예 39권 = 활성 39권이므로, 편입 39권의 이미지가 **전부**
  이 508객체 안에 있다(§Context 5-g 폐합 검산). 5권은 `DONE_BOOKS` 경유로 먼저 업로드된
  분이며 키 규약은 34권과 같다(`copy_bookdash_images.py:93-100,139`).
...
### 얻는 것 — Book Dash html 39권 (D9~D14 신규)

- **`content_type='html'` 활성 도서 중 `book_text` 없는 권이 0이 된다.** 39권 = html 활성 전량
  이므로, 이 코호트의 "본문 없음" 상태가 완전히 소멸한다.
- **ADR-0054 D2의 활성 전환 대상이 0권이 된다** — WP PDF 재하베스트·페이지 이미지 렌더·
  Storage 재업로드·매니페스트 합성 **전 공정이 불필요**해진다(ADR-0054 Amd#2).
- **39권이 `content_type='html'`을 유지한 채 AudioReader로 진입한다. `content_type` 변경이
  불필요하다.**
  ...
  - 즉 **`book_text` 적재(D9) + Danielle 생성(ADR-0053 Amd#3) 두 가지만으로 회수가 완결**된다.
    DB 스키마·`books.content_type`·뷰어 코드 어느 것도 건드리지 않는다.
    (34권은 **재생성**, 5권은 **신규 합성** — D9-a·D9-b)
...
### 잃는 것 / 감수하는 것 — Book Dash html 39권
...
- **해상도.** GH Pages 이미지는 스프레드 1134×567 · 단면 567×567로, 152권 코호트
  ...
  대가로 PDF 재하베스트 전 공정과 O2 게이트를 면제받는다. → 판단 근거는 ADR-0054 Amendment #1·#2.
  - **5권에도 동일하게 적용된다.** 5권만 고해상도로 두면 **같은 코호트 안에서 화질이 갈리므로**,
    일관성을 위해 39권을 같은 해상도로 통일한다(ADR-0054 Amd#2 F2 근거 ④).
  - **고해상도를 원하면 39권 일괄 재하베스트가 일관된 방향**이며, 그 경로는 살아 있다
    (ADR-0054 Amd#2 F4 — D1~D6 절차·WP PDF 자산 무폐기).
...
- **`content_type='html'`인 채로 AudioReader를 타는 책이 생긴다.** `content_type`이 렌더 경로를
  ...
  추가할 때 이 코호트가 예외가 된다.
- **5권은 축 대조 기준선이 없다**(D9-b). 34권처럼 기존 오디오와의 `page_index` 정합을
  실측할 수 없어, **육안 검증 1건이 유일한 안전장치**다. 자동 검증으로 대체되지 않는다.
- **5권 이미지 실재가 아직 기록 기반 검산이다**(§Context 5-g). 팀장 육안 확인 전까지
  D9-b는 **철회 가능 상태**로 둔다.
...
- **39권 TTS 실제 합성·Storage 업로드·`book_audio` INSERT.** 사양 확대는 ADR-0053 Amendment #3에
  기록되나, **실행은 dry-run 승인 절차(ADR-0053 D4)를 거쳐 별도 작업지시서**로 한다.
- **본문 적재 SQL 생성기 작성.** D9~D14는 결정문일 뿐이며, 스크립트 착수는 별도 지시서 소관이다.
- **비활성 10권(121면)의 적재.** D9에서 제외 — 사유 규명 후 별도 판단(O-a).
- **39권 일괄 고해상도 재하베스트.** 가능하나 본 개정 범위 밖이다(ADR-0054 Amd#2 F4).
...
### O4. 활성 34권의 확정 slug 명단 — ✅ **Resolved (2026-08-08)**

**해소 경위**: 팀장이 `content_url` 포함 SQL export로 34권을 slug·`source_id`·`ruth_pages`
단위로 확정했고, 워커가 로컬 원천과 전수 대조해 검산을 통과했다. **D9 적재 전 차단 게이트는
해제된다.**

**검산 결과 (2026-08-08, 워커 읽기 전용)**

| 검산 항목 | 기준 | 결과 |
|---|---|---|
| 권수 | 34 | **34** ✅ (slug 중복 0건) |
| `{slug}.json` 실존 | 34/34 | **부재 0건** ✅ |
| json 장면 수 vs `ruth_pages` | 34/34 일치 | **불일치 0건** ✅ |
| 장면 합계 | 409 | **409** ✅ (12면 33권 + 13면 1권) |
| `source_id` vs `scratchpad/tts_recon_49.csv` | 34/34 일치 | **불일치 0건** · UUID 중복 0 · 전량 UUID 형식 ✅ |
| **블랙리스트 ∩ 34권** | **0건이어야 함** | **0건** ✅ (§Context 5-e) |
| 총계 폐합 | 409(활성 34) + 121(비활성 10) = 530 | **530 = `book_audio` `page`+`Ruth` 행수** ✅ |

**확정 명단 (34권) — 본 항이 D9 적재 대상의 정본이다**

| # | slug | 면 | `source_id` |
|---:|---|---:|---|
| 1 | `amazing-daisy` | 12 | `9c9e7b9a-fe46-11e5-86aa-5e5517507c66` |
| 2 | `bathtub-safari` | 12 | `9c9e86da-fe46-11e5-86aa-5e5517507c66` |
| 3 | `come-back-cat` | 12 | `9c9e7820-fe46-11e5-86aa-5e5517507c66` |
| 4 | `gracas-dream` | 12 | `9c9e71cc-fe46-11e5-86aa-5e5517507c66` |
| 5 | `grandpas-gold` | 12 | `9c9e6524-fe46-11e5-86aa-5e5517507c66` |
| 6 | `how-about-you` | 12 | `9c9e819e-fe46-11e5-86aa-5e5517507c66` |
| 7 | `i-will-help-you` | 12 | `9c9e9396-fe46-11e5-86aa-5e5517507c66` |
| 8 | `is-there-anyone-like-me` | 12 | `9c9e8c0c-fe46-11e5-86aa-5e5517507c66` |
| 9 | `karabos-question` | 12 | `9c9e9e5e-fe46-11e5-86aa-5e5517507c66` |
| 10 | `lara-the-yellow-ladybird` | 12 | `9c9f566e-fe46-11e5-86aa-5e5517507c66` |
| 11 | `little-ants-big-plan` | 12 | `9c9e55de-fe46-11e5-86aa-5e5517507c66` |
| 12 | `londi-the-dreaming-girl` | 12 | `9c9e96ac-fe46-11e5-86aa-5e5517507c66` |
| 13 | `lory-dory` | 12 | `9c9e83b0-fe46-11e5-86aa-5e5517507c66` |
| 14 | `maddy-moona` | 12 | `9c9e7dca-fe46-11e5-86aa-5e5517507c66` |
| 15 | `miss-helens-magical-world` | 12 | `9c9e6196-fe46-11e5-86aa-5e5517507c66` |
| 16 | `queen-of-soweto` | 12 | `9c9e640c-fe46-11e5-86aa-5e5517507c66` |
| 17 | `rafikis-style` | 12 | `9c9e6e52-fe46-11e5-86aa-5e5517507c66` |
| 18 | `sbus-special-shoes` | 12 | `9c9e9fc6-fe46-11e5-86aa-5e5517507c66` |
| 19 | `searching-for-the-spirit-of-spring` | 12 | `9c9e76ea-fe46-11e5-86aa-5e5517507c66` |
| 20 | `sima-and-siza` | 12 | `9c9ea96c-fe46-11e5-86aa-5e5517507c66` |
| 21 | `sindi-and-the-moon` | 12 | `9c9ea21e-fe46-11e5-86aa-5e5517507c66` |
| 22 | `sindiwe-and-the-fireflies` | 12 | `9c9e596c-fe46-11e5-86aa-5e5517507c66` |
| 23 | `singing-the-truth` | 12 | `9c9e62ea-fe46-11e5-86aa-5e5517507c66` |
| 24 | `sizwes-smile` | 12 | `9c9e87f2-fe46-11e5-86aa-5e5517507c66` |
| 25 | `sleepy-mr-sloth` | 12 | `9c9e7cb2-fe46-11e5-86aa-5e5517507c66` |
| 26 | `thatos-birthday-surprise` | 12 | `9c9eb68c-fe46-11e5-86aa-5e5517507c66` |
| 27 | `there-must-be-a-rainbow` | 12 | `9c9ea48a-fe46-11e5-86aa-5e5517507c66` |
| 28 | `together-were-strong` | 12 | `9c9e663c-fe46-11e5-86aa-5e5517507c66` |
| 29 | `tortoise-finds-his-home` | 12 | `9c9e6f9c-fe46-11e5-86aa-5e5517507c66` |
| 30 | `walking-together` | 12 | `9c9e9102-fe46-11e5-86aa-5e5517507c66` |
| 31 | `what-if` | 12 | `9c9f3292-fe46-11e5-86aa-5e5517507c66` |
| 32 | **`whose-button-is-this`** | **13** | `9c9eb2cc-fe46-11e5-86aa-5e5517507c66` |
| 33 | `why-is-nita-upside-down` | 12 | `9c9e5f48-fe46-11e5-86aa-5e5517507c66` |
| 34 | `zanele-situ-my-story` | 12 | `9c9e6d12-fe46-11e5-86aa-5e5517507c66` |
| | **합계** | **409** | |

- **`maddy-moona` 포함 확인**(#14). ADR-0054 D1-b의 `hold` 1권이며, 그 ADR의 D2 대상 38권
  **밖**이다. 본 경로로는 편입된다 — `hold`는 **PDF 전환 보류**이지 서비스 차단이 아니기 때문이다.
- **`whose-button-is-this` 활성 확인**(#32, 13면). §5-b 철회의 직접 근거다.
- 적재 시 이 명단을 **파일로 재export하지 않고 본 표를 정본으로 사용**한다(중간 사본이 늘면
  어긋남 표면이 늘어난다). 대상 CSV가 필요하면 본 표에서 생성한다.
- **재확인 권고**: 적재 착수 직전 `is_active` 재조회 1회. §5-e 주의 참조.

> **범위 확장 메모 (2026-08-08)**: 본 표는 **D9-a(34권)** 의 정본이다.
> **D9-b(5권)** 명단은 **§Context 5-f**에 있으며, 두 표를 합친 **39권이 D9의 전체 적재 대상**이다.
> 5권은 `book_audio` 행이 0이라 위 export 조건(`EXISTS(book_audio …)`)에 걸리지 않았고,
> `DONE_BOOKS`(`copy_bookdash_images.py:94-100`) 하드코딩으로 slug·`source_id`가 확정된다.
...
*(위 문안은 ADR-0058 D8이 규정한 종결 초안 그대로다. 아래는 반영 시점 실행 결과.)*

- **실행 완료 (2026-08-13, 팀장 SQL Editor)** — `book_review` **860권** 체제 확정
  (`african_storybook` 527 / `bloom` 142 / `book_dash` 191). 중간 상태 없음.
  산출물: `docs/sql/adr0058/02_seed_708_book_review.sql`.

아래는 최초 기재 이후의 경과다(이력 보존).

> **2026-08-10 실사용 발현.** 적재 완료 후 **신규 708권**(Book Dash html 39 + `asb_native` 669)이
> **`book_review` 미시드**여서 **검수 화면 목록에 나타나지 않는다.** 그 결과 D9-b 육안 검증을
> `/admin/review`로 수행하지 못했고, **이미지↔텍스트 대조**(`o6_image_check.html`의 65객체와
> 생성 SQL 텍스트를 나란히 대조) **대체 절차**로 통과시켰다. 검증 자체는 통과했으나,
> 본 항이 예고한 연동 리스크(아래 판단 재료 2번째 항목)가 실제로 발현된 사례다.
>
> **범위가 39권에서 708권으로 확대됐다.** 최초 기재는 html 39권 한정이었으나, ASb·Bloom 669권도
> D8로 미시드가 확정돼 있어 동일 증상을 공유한다.
>
> **다음 세션 설계 검토 안건**으로 이월한다. 검토 항목:
> 1. **시드 범위** — 708권 전량 / html 39권만 / 시드하지 않고 검수 화면에 별도 진입 경로 신설
> 2. **`status` 초기값** — `draft`(Book Dash 152권 선례, ADR-0048 D4) / 별도 값 / D8 유지
> 3. **기존 `draft` 116권 정리와의 연계** — 신규 시드가 기존 검수 큐에 섞이는 문제. 정리 순서를
>    먼저 정하지 않으면 검수 대상이 한꺼번에 불어난다
> 4. `ADR-0053 D1`이 `confirmed` 게이트를 폐지했으므로 **TTS 착수에는 여전히 비차단**이다 —
>    본 안건은 검수 운영 편의 문제이지 합성 선행 조건이 아니다

아래는 최초 기재분이다(이력 보존).

- D8은 **ASb·Bloom 669권**에 대한 결정이며, 39권(D9-a 34 + D9-b 5)에는 적용 범위를
  확대하지 않았다(D0 표).
- 판단 재료(사실만 기록):
  - D8 근거 ①("OCR 오류 유형이 존재하지 않는다")은 39권에도 **성립**한다 — 원천이 원본 HTML의
    `<p>` 텍스트이지 OCR 산출물이 아니다(`extract_text.py:126-131`).
  - **단 D9-b 5권은 육안 검증(D9-b)이 필요하고, 그 확인 위치가 `/admin/review` 상세다.**
    시드가 없으면 그 화면에 권이 뜨지 않을 수 있다 — **O5와 D9-b 검증 절차는 연동된다.**
    시드하지 않기로 하면 검증 위치를 별도로 정해야 한다.
...
### O-a. 비활성 10권(121면)의 비활성 사유 — **미확인 (비차단)**
...
| ~~O4~~ | ~~활성 34권 확정 slug 명단~~ | Book Dash html | ✅ **Resolved 2026-08-08** |
| ~~O5~~ | ~~`book_review` 시드 여부 (범위 39권 → **708권**)~~ | 전 코호트 | ✅ **Resolved 2026-08-13** (ADR-0058 D5 — 708권 `draft` 시드) |
| ~~O6~~ | ~~5권 이미지 실재 — 팀장 육안 확인~~ | Book Dash html D9-b | ✅ **Resolved 2026-08-10** (65객체 전부 로드) |
...
**전면 차단 Open은 0건이다.** **2026-08-10 O6 해소로 D9-b(5권)의 조건부 대기까지 풀렸다** —
ASb·Bloom 669권 + Book Dash html 39권(D9-a 34 + D9-b 5) = **708권 전량이 착수 가능**하다.
§Context 5-g의 철회 경로는 발동하지 않는다.

### O6. D9-b 5권의 이미지 실재 — ✅ **Resolved (2026-08-10)**

> **2026-08-10 팀장 육안 확인 — `book-images` 5폴더 65객체 전부 정상 로드**
> (검증 도구 `scripts/tts_pilot/out/o6_image_check.html`, URL 조립은 `audio-manifest.ts:289`와
> 동일 규칙). **D9-b 적재 차단 해제.**
>
> 확인 규모: 5권 × (`01.jpg`~`12.jpg` + `cover.jpg`) = **65객체**(본문 60 + 표지 5).
> 이로써 적재 범위는 **708권 7,368면으로 확정**되며, 아래 "실패 시" 철회 경로는 **발동하지 않는다**
> (ADR-0054 F7 불발동 종결).

아래는 확인 전 기재분이다(이력 보존).

- **기록 기반 검산은 폐합**한다(§Context 5-g): 정예 39권 508객체 = 본문 469(= 34권 409 +
  5권 60) + 표지 39. 분해식 출처는 `ADR-0036 Amd#1:128`이다.
- **그러나 워커는 Storage 접근 권한이 없어 실물을 확인하지 못했다.** 정찰 #1에서도 이 항목은
  "추정"으로 표기했다. 검산은 **문서 간 정합**을 보인 것이지 **객체 실재**를 보인 것이 아니다.
- **확인 방법**: `book-images` 버킷의
  `book_dash-9c9e94e0-fe46-11e5-86aa-5e5517507c66/`(`a-beautiful-day`) 폴더에
  `01.jpg`~`12.jpg` + `cover.jpg` = **13객체**가 있는지 육안 확인. 나머지 4권도 같은 규약이다.
- **성공 시**: D9-b 확정. 본 항 Resolved.
- **실패 시**: **D9-b(5권)만 철회.** 적재 범위는 708권 7,368면 → 703권 7,308면으로 되돌아가고,
  5권은 ADR-0054 D2 경로로 복귀한다(그 트랙은 폐기되지 않았다 — ADR-0054 Amd#2 F4).
  **D9-a·ASb·Bloom은 영향 없다.**

> **O-b 부분 해소 메모(2026-08-08)**: 팀장 교차표(html 54권 = 활성 39 + 비활성 15)에서
> 비활성 15권이 블랙리스트 15권과 1:1 일치함이 확정됐다(§Context 5-e). 즉 **html 코호트의
> 비활성·오디오 보유는 10권으로 확정**이며, "11권 중 1권"의 차이분은 여전히 **html 밖**에 있다.
> O-b는 미확인 상태를 유지한다(비차단).
...
| **ADR-0053 Amd#3** | **Danielle 적용 범위 669권 → 708권 확대 (본 ADR D9의 39권 편입)** |
| **ADR-0054 Amd#2** | **E4-a (나) 확정 — D2 전환 대상 0권 · D2 자산 무폐기(F4) · Status는 팀장 결정(F6)** |
