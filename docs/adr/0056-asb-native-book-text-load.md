...
Book Dash html 코호트 중 **활성 34권(409면)** 을 편입하며(**D9~D14** 신규), 팀장 승인 2건에 근거한다.
...
| **Book Dash html 활성 (D9~D14 신규)** | **34** | **409** | §Context 5 (팀장 SQL 실측 · 워커 json 재검산) |
| **합계** | **703** | **7,308** | |

- 669 + 34 = **703** ✅ / 6,899 + 409 = **7,308** ✅
- 신규 편입분은 전체 면수의 **5.6%** (409 ÷ 7,308 = 5.60%).
...
### D9. [Book Dash html] 활성 34권(409면)을 `book_text` 적재 범위에 포함한다

- 대상 = `source_platform='book_dash'` AND `content_type='html'` AND `kind='page'` 오디오 보유
  AND `book_text` 0행 AND **`is_active = true`** → **34권 409면**(팀장 SQL 실측 2026-08-08,
  §Context 5). **확정 slug 명단은 O4에 박제**돼 있다.
- 이로써 본 ADR의 총 적재 범위는 **669권 6,899면 → 703권 7,308면**이 된다(§Context 6).
...
**차단 Open 0건이다.** ASb·Bloom 669권은 종전대로 착수 가능하며(O1·O2·O3 비차단),
**Book Dash html 34권도 O4 해소로 착수 가능**해졌다. 잔여 5건은 전부 비차단이다.
