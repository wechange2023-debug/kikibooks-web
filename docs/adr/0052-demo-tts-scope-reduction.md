...
## Amendment #2 (2026-07-22, 팀장·오케스트레이터)

### D6 재개정 — TTS 보이스·엔진 최종 확정: Danielle long-form + atempo 후처리

- Amendment #1의 Amy(en-GB) 확정을 **대체**한다. 근거: 팀장 청취 라운드 3~3.5 결과
  en-US 요구 + "밋밋함" 해소를 위해 **long-form 엔진** 채택.
- 확정: **Danielle / long-form / us-east-1**(호출 파라미터 오버라이드, `~/.aws` 기본 리전 무변경) /
  **평문 합성 → ffmpeg `atempo=0.85` → marks `time×(1/0.85)` 스케일**.
- **SSML `prosody rate` 감속은 금지**: long-form 엔진 내부 타임스트레치가
  울림(에코성 왜곡)을 유발함을 청취로 확인(r85 최악, r90·r95도 잔존).
  atempo 후처리는 동일 속도에서 울림 0 확인.
- 인코딩 **VBR `-q:a 2` / 24000 Hz**. ffmpeg는 `imageio_ffmpeg` 동봉본(7.1) 사용(신규 설치 없음).
- Ruth neural 78% 산출물(`out/audio/*_Ruth_r78.*`)·Amy 샘플은 삭제하지 않고 **보존**한다.
- 파일 키 구조: `book_dash-{slug}/danielle/pNN.mp3`(+`.marks.json`) — 성우 층위 반영.
- 근거 데이터: `scripts/tts_pilot/out/voice_samples_r35/_r35_report.json` (WPM·길이비 실측).
