...
## D5. bbox 좌표 저장 [2026-07-10 개정 — 팀장 결정 K1 반영]

OCR 시 **word 레벨 bounding box(left/top/width/height) + confidence를 텍스트와 함께 보존**한다(팀장 결정 I3).

- 용도 ①(개정): bbox는 좌표 하이라이트용이 **아니라**, **텍스트 초벌 추출의 읽기 순서·페이지
  귀속 판정 보조** 용도다(K2·K3). 하이라이트는 A안(이미지 아래 별도 텍스트 레이어 +
  marks char offset)으로 확정되어 화면 좌표를 쓰지 않는다.
  - 개정 근거: 팀장 결정 **K1 — "하이라이트는 A안(이미지 아래 별도 텍스트 레이어). C안 폐기.
    번복 없음."** (ADR-0035 Amd#3). 개정 전 문안("이미지 내 인쇄 텍스트 위 좌표
    하이라이트(C안) — ADR-0035 Amd#2 E2의 좌표 소스")은 C안 전제였으므로 폐기.
- 용도 ②(유지): **재추출 비용 회피** — 검수 단계에서 텍스트만 고치고 좌표는 재사용, 향후 필요 시 재OCR 불필요.
