...
`components/app/app-header.tsx`(`'use client'`)는 `usePathname()`으로 `/book/*/read` · `/book/*/celebrate` 및 not-found 렌더 경로에서 **`null`을 반환(미렌더)**한다. admin-nav의 검증된 usePathname 패턴을 재사용하며 read·celebrate·not-found page 수정은 0건이다.
...
`lib/auth/routes.ts`에 `POST_LOGIN_PATH='/home'`는 있으나 네비용 `HOME_PATH`·`LIBRARY_PATH`는 미중앙화 상태(`celebrate/page.tsx:69`가 로컬 `const LIBRARY_PATH='/library'` 선언). AppHeader 구현 시 routes.ts 중앙 상수 도입을 검토한다.
