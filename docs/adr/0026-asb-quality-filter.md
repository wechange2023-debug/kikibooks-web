...
## Amendment #2 — ASb 표지 미표시 원인 정정 및 해결 (2026-06-19)

### 배경
clean 744권 공개 후 라이브러리 카드에서 ASb 표지가 전부 미표시(파스텔 폴백만 노출).
당초 "cover_url 404"로 추정했으나, PM 실측으로 반증됨:
- 예시 cover_url: https://africanstorybook.org/illustrations/covers/996.png → HTTP 200, 표지 정상 생존.
- 즉 표지는 살아있고, 미표시는 별개 원인.

### 진짜 원인
라이브러리 카드(components/library/library-browser.tsx)는 next/image의 <Image> 사용.
next/image는 next.config.js의 images.remotePatterns에 등록된 호스트만 최적화·렌더.
등록 현황: content.digitallibrary.io(GDL)·bookdash.github.io(Book Dash)만 존재, africanstorybook.org 누락.
→ 살아있는 ASb 표지가 호스트 미등록으로 차단되어 미표시.
대조: asb-reader.tsx 뷰어 본문은 평문 <img>라 정책 무관 → 정상 표시(증상과 정합).

### 결정
next.config.js images.remotePatterns에 africanstorybook.org 추가.
- 직전 정찰의 "본문 첫 그림 백필/새 동기화(③)" 결론은 폐기 — 표지가 생존하므로 불필요.
- DB 변경·마이그레이션·재sync 없음. 설정 1행 추가로 해결.

### 영향 범위
- 카드 표지: ASb 공개분(현 active 2,141권 중 표지 생존분) 본연의 일러스트 표지 정상 노출.
- 폴백(파스텔+책아이콘)은 잔존: 표지가 실제 404·로드실패인 책(broken 등)에 한해 onError로 계속 발동.
