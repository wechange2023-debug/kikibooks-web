...
**Proposed** (2026-07-30) / 기준 HEAD `2bfce41`
...
> 목록 출처는 `lib/shared/blacklist.ts`(source_id UUID + slug 주석)다. 2026-07-28 정찰 문서는
> 사유·권수(15권)·MANUAL/MINOR 소속만 기록하고 15권 **전체 이름은 담고 있지 않다** — 이름은 코드에서 인용했다.
...
> ⚠️ 오케스트레이터가 제시한 "**128 → 최대 190권**" 중 **190의 산술 근거는 본 ADR 범위에서 확인되지 않았다.**
> 38권 전환만으로는 128 + 38 = **166권**이다. 미해결 항목 **O1** 참조.
...
> 따라서 "페이지 수 일치"의 대상은 **전환 전후 비교가 아니라 전환 후 3자산 사이의 정합**이다. 미해결 항목 **O3** 참조.
...
- **위험**: 38권 중 일부가 이미 v1 오디오를 보유한다면 기존 `book_audio.page_index`가 새 페이지 체계와
  어긋난다. **교집합 미확인 상태**이며 실행 전 실측이 필요하다 — 미해결 항목 **O2**. 이 항목이 해소되기 전에는
  step3 전환 SQL을 실행하지 않는다.
...
### O1. "Book Dash 오디오 128 → 최대 190권"의 산술 근거 — **확인 불가**

작업지시서는 본 전환을 "128 → 최대 190권 계획의 선행 단계"로 규정했으나, **38권 전환만으로는 166권**이다.

| 셈 | 값 |
|---|---:|
| 현재 계획된 danielle 배치 116권 + 기존 시범 12권 | 128 |
| + 본 ADR 전환 38권 | **166** |
| 목표로 제시된 상한 | **190** |
| 차이 | **24권** |

차이 24권은 ADR-0053 D1의 **사전 제외 24권(회전 페이지 18 + 제작 메타데이터 오염 6)** 과 정확히 일치한다.
따라서 190은 **"38권 전환 + 회전 18권·오염 6권의 텍스트 수정 복구까지 모두 완료했을 때의 상한"** 으로 읽는 것이
가장 자연스럽다. 이 해석이 맞는지 확인 부탁드린다.

- 맞다면 → 본 ADR D5의 기여분은 **+38권(166권)** 이며, 190은 별도 복구 트랙 2건이 추가로 필요하다.
- 아니라면 → 190의 구성 내역을 알려주시면 D5를 정정한다.

> 참고: v1 Ruth 오디오 44권을 더하면 별도의 셈(128 + 44 + 18 = 190)도 성립한다. 어느 쪽인지에 따라
> 후속 작업 목록이 달라지므로 확인이 필요하다.

### O2. ⚠️ **근거 문서 내부 모순** — 38권과 v1 Ruth 오디오 44권의 교집합 미확인

`docs/recon/2026-07-28-html54-slide-feasibility.md` §5는 다음과 같이 적었다.

> "**비블랙 39권 중 44권분의 오디오가 이미 적재된 v1 코호트와 겹친다**(CSV `has_audio=True` 44권 = Ruth 44권)"

**39권 중 44권은 산술적으로 불가능하다.** ADR-0053 D1 표는 44권을 **"54권 중 44권"** 기준으로 기록하므로,
정찰 문장의 모집단 표기가 잘못됐을 가능성이 높다. 결과적으로 **전환 대상 38권 중 몇 권이 Ruth 오디오를
보유하는지는 어느 문서로도 확정되지 않는다**(가능 범위 29~38권).

이것은 문서 오탈자 문제가 아니라 **실행 위험**이다.

- Ruth 오디오의 `page_index`는 **html 장면 번호**(권당 12) 기준으로 적재됐다.
- 전환 후 페이지 체계는 **PDF 렌더 면 번호**(권당 약 14)가 된다.
- 겹치는 권은 기존 `book_audio` 행이 **엉뚱한 페이지에 붙는다.**

**따라서 step3 전환 SQL 실행 전에 다음 실측을 선행할 것을 권고한다**(읽기 전용 SELECT 1건):

```
전환 대상 38권 × book_audio 보유 여부 · voice · page_index 범위
```

결과에 따라 (가) 겹치는 권의 Ruth 오디오를 전환과 함께 폐기하고 danielle로 재생성, (나) `page_index`
재매핑, (다) 겹치는 권을 전환 대상에서 제외 — 중 하나를 팀장이 선택해야 한다. **본 ADR은 이 선택을 하지 않는다.**

### O3. 검증 항목 "도서별 페이지 수 일치"의 기준 정의 — 지시서 문구 그대로는 성립하지 않음

작업지시서는 "전환 후 도서별 페이지 수 일치 확인"을 검증 항목으로 지시했으나, **무엇과 무엇을 일치시키는지**가
지정되지 않았다. 경로 (B)를 택한 이상 **전환 전후 페이지 수는 원리적으로 일치하지 않는다**(html 12장면 →
PDF 렌더 약 14면). 따라서 V1에서 **"전환 후 3자산(이미지·`book_text`·매니페스트) 사이의 정합"** 으로 재정의했다.

이 재정의가 의도와 다르다면 알려주시기 바란다. 만약 "전환 전후 페이지 수 보존"이 진짜 요구사항이라면
경로 (B)는 그 요구를 충족할 수 없고, 경로 (A)(GH Pages 이미지 그대로 = 12장면 보존, 단 저해상도)로
D2를 재검토해야 한다.

### O4. 전환 후 슬라이드 총 수의 예상치 — 하베스트 실측으로만 확정

152권 코호트 규약(13 + 1 = 14면)을 그대로 적용하면 38권 × 14 = **532면**이 예상되나, 이는 **추정치**다.
PDF 실물의 면 구성은 권마다 다를 수 있고, 정찰은 PDF를 열어보지 않았다(WP 카탈로그에 slug이 존재한다는
사실까지만 확인). **D6 드라이런 산출로 확정**하며, 그 숫자를 본 ADR에 Amendment로 고정한다.

참고로 전환 전 html 장면 수는 정확히 **457장면**이다(비블랙 39권 469장면 − `maddy-moona` 12장면).

---

*ADR-0054 초안 끝. 팀장 승인 시 Status를 Accepted로 바꾸고 O1~O4 해소 내용을 Amendment로 고정한다.*
