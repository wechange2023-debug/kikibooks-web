...
**잔여 활성 전환 대상 = 5권**으로 확정됐다(E4, 2026-08-08 실측).

**Amendment #2 (2026-08-08, 팀장 결정)** — E4-a **(나) 편입 채택**. 잔여 5권도 ADR-0056 경로로
편입되어 **D2 전환 대상은 0권**이 된다. **본 ADR의 전환 트랙은 실행 없이 종료**된다.
단 **D1~D6 절차·WP PDF 자산은 폐기하지 않는다** — 향후 39권 일괄 고해상도 재하베스트 경로로
살아 있다(F4). 본 ADR 전체의 Status는 **2026-08-10 팀장 결정: `Accepted` 유지(해상도 트랙 미대체)**
로 확정됐다 — **F6 Resolved**. `Superseded` 표기는 채택하지 않는다.
...
### E4-a. 잔여 5권의 처리 — ✅ **(나) 편입 확정 (2026-08-08 팀장 결정)**

> **본 항은 Amendment #1 시점에 "두 경우 병기 · 미확정"으로 작성됐고,
> 같은 날 Amendment #2로 (나)가 채택됐다.** 아래 두 경우 표는 **판단 근거의 이력**으로 남긴다.
> 확정 결정과 승인 근거는 **Amendment #2 (F1~F7)** 를 보라.

**5권의 ADR-0056 편입은 팀장 미승인 상태다.** 따라서 본 ADR의 활성 전환 대상은 아래 두 값
사이에서 미정이며, **본 Amendment는 어느 쪽도 확정 처리하지 않는다.**
*(↑ Amendment #1 시점 서술. Amendment #2로 **(나) 확정** → 활성 전환 대상 **0권**.)*
...
> **✅ 결정됨 (2026-08-08)**: **(나) 편입 채택.** 활성 전환 대상 **0권**. → **Amendment #2**
> 위 "(나)를 자동 채택하지 않는 이유"에서 든 두 논점은 Amendment #2 F2에서 다음과 같이
> 해소됐다 — ① 신규 합성 비용은 **(가)·(나) 동일**(어느 경로든 5권은 오디오가 없어 신규
> 합성이 필요) ② 해상도 열위는 **감수**하되, 5권만 고해상도로 두면 동일 코호트 화질이
> 갈리므로 **39권 일괄 재하베스트가 일관된 방향**이며 그 경로는 가역적으로 열려 있다.
...
## Amendment #2 (2026-08-08, 팀장 결정) — E4-a (나) 편입 확정. D2 전환 대상 0권

### F0. 본 Amendment의 성격

**Amendment #1의 E4-a가 미결로 남긴 선택지 하나를 확정하는 것뿐이다.** D1~D6·V1~V5·
Non-goals·E0~E7 어느 항목도 재개·수정하지 않는다. E4-a 표의 (가)/(나) 중 **(나)** 가 채택됐다.

### F1. 결정 — 잔여 5권을 ADR-0056 경로로 편입한다. **D2 활성 전환 대상 = 0권**

- 대상 5권: `a-beautiful-day` · `a-dancers-tale` · `a-fish-and-a-gift` ·
  `a-house-for-mouse` · `a-tiny-seed` (각 12면, **합계 60면**. 명단·`source_id`는 E4 표).
- 이관처: **ADR-0056 D9-b**(`book_text` 적재 `source='html_scene_json_v1'` + Danielle **신규 합성**).
- 결과: **본 ADR D2의 활성 전환 대상은 0권.** 전환 트랙은 **실행 없이 종료**된다.
- 결정일 **2026-08-08**, 결정자 **팀장**.
...
```
html 54
 − 비활성 15 (= 블랙리스트 15, slug 1:1 확정 — ADR-0056 §5-e)   ····· 비노출·범위 밖
 = 활성 39
   ├ 34 → ADR-0056 D9-a (Ruth 재생성)   409면   ← Amendment #1
   └  5 → ADR-0056 D9-b (신규 합성)      60면   ← 본 Amendment
 = ADR-0054 D2 전환 대상  0권
```

### F2. 승인 근거 (팀장 제시 5항, 원문 기록)

1. **5권 이미지가 이미 업로드 완료**돼 있다(`copy_bookdash_images.py:93-100` `DONE_BOOKS` 경유).
   → **(나)의 이미지 추가 작업 0.**
2. **합성 비용은 (가)·(나) 동일.** 5권은 어느 경로로 가든 오디오가 없어 신규 합성이 필요하다.
   **차이는 이미지 재작업 유무뿐**이다.
3. **(가)는 5권만을 위해 ADR-0054 D1~D6 파이프라인을 별도 유지**해야 한다
   (PDF 하베스트 · 페이지 이미지 렌더 · Storage 업로드 · 매니페스트 합성 · `books` UPDATE ·
   3중 백업 · 드라이런 게이트). 5권을 위한 유지 비용이 이득을 넘는다.
4. **해상도 열위는 감수한다.** 34권이 이미 GH Pages 해상도로 확정된 이상, **5권만 고해상도로
   두면 동일 코호트 안에서 화질이 갈린다.** 고해상도를 취하려면 **39권 일괄 재하베스트**가
   일관된 방향이며, 이는 향후 별건으로 가능하다(**가역** — F4).
5. **60면은 전체의 0.8%** (60 ÷ 7,368). 규모가 판단을 바꾸지 못한다.

> **워커 검산(2026-08-08, 읽기 전용)**: 근거 ①의 전제인 "5권 이미지 실재"는
> **기록 기반 검산은 폐합**하나(정예 39권 508객체 = 본문 469 + 표지 39,
> 분해식 출처 `ADR-0036 Amd#1:128`) **Storage 실물 확인은 미완**이다.
> → **ADR-0056 O6**(팀장 육안 확인 대기). 실패 시 F7 참조.

### F3. E5(슬라이드 총 수) 확정 — **0면. O4 소진**

- E5의 두 경우 중 **(나) 채택으로 모집단이 0권** 확정 → 전환 후 슬라이드 총 수 = **0면**.
- 이로써 초판 **O4(전환 후 슬라이드 총 수)는 산출 대상이 소멸**한다. 미해소로 남기지 않고
  **소진(moot)** 으로 종결한다. **D6 드라이런도 실행하지 않는다.**
- 폐기하지 않는 실측 기록: 초판 O4의 "전환 전 html 장면 수 **457장면**", E4의 "잔여 5권
  전환 전 60면", C2의 해상도·분류 실측은 **전부 유효한 이력**이다.

### F4. **폐기하지 않는 것** — D2 절차와 WP PDF 자산은 살아 있다

**전환 대상이 0권이 된 것이지, 방법론이 틀린 것이 아니다.** 아래는 그대로 보존한다.

| 자산 | 상태 |
|---|---|
| D1~D6 결정문(대상 선정·방식·DB 영향·롤백·회계·게이트) | **유효** — 재하베스트 시 그대로 재사용 |
| `harvest.py` · `render_page_images.py` · `upload_page_images.py` · `gen_book_text_sql.py` · `sync_book_dash_v2.py` | **무접촉** — 152권 코호트가 계속 쓰는 현역 자산 |
| WP 카탈로그 PDF 원본(38권분 확보 확인) | **폐기 안 함** |
| V1~V5 검증 계획 | **유효** — 재하베스트 시 적용 |

- **향후 경로**: 39권 화질을 152권 수준(1600×800 / 1600×1600)으로 올리려면
  **39권 일괄 재하베스트**를 별건으로 수행한다. 그때 D2 6단계 중 1~3·5단계를 그대로 태우고,
  `book_text`는 ADR-0056으로 이미 적재돼 있으므로 **이미지만 교체**하면 된다
  (키 규약이 같아 `--overwrite` 업로드로 끝난다 — ADR-0056 D14).
- **단, `maddy-moona`는 WP 카탈로그에 PDF가 없다**(D1-b). 39권 일괄 재하베스트 시 그 1권은
  여전히 hold이며, 제목 대조로 PDF를 찾는 것이 선행 조건이다.

### F5. 본 Amendment가 바꾸지 않는 것

- **D1-a 블랙리스트 15권 제외 · D1-b `maddy-moona` hold · D1-c 이연** — 전부 유지.
  - `maddy-moona`는 ADR-0056 D9-a에 편입됐으나, 그것은 **PDF를 쓰지 않는 경로**여서 가능했다.
    **본 ADR의 `hold`(WP 카탈로그 PDF 부재)는 해제되지 않는다.**
- **D3 DB 영향** — 대상이 0행이 되어 `books` UPDATE·step1~step3 SQL은 **실행하지 않는다.**
  파일 구조·`ROLLBACK;` 종결 규약은 향후 재하베스트를 위해 결정문으로 남긴다.
- **D4 3중 백업 롤백** — 실행 대상이 없어 발동하지 않는다. 규약은 유지.
- **D5 오디오 상한 회계** — Amendment #1 E6의 재산정(최종 상한 **191권**)이 그대로 유효하다.
  경로가 전부 ADR-0056으로 갈렸을 뿐 **총량은 변하지 않는다**(34 + 5 = 39, 128 + 39 = 167).
- **V1~V5** — 적용 대상 0권. 향후 재하베스트 시 되살아난다.
- **Non-goals 전부 유지** — 블랙리스트 15권 복구·해제는 여전히 이연 상태다.

### F6. 본 ADR의 Status — ✅ **Resolved (2026-08-10)**

**2026-08-10 팀장 결정: (A) `Accepted` 유지.** 사유 = ADR-0056은 `book_text`·오디오 트랙만
대체했고 **이미지 해상도 트랙은 대체하지 않았다**. 따라서 `Superseded` 표기는 쓰지 않는다.
Status는 `Accepted (2026-08-05)` 그대로 유지되며, 실행 종료 사실은 Amd#2 주석으로 표시한다.

아래는 결정 당시의 판단 재료다(이력 보존).

| 선택지 | 근거 | 부작용 |
|---|---|---|
| **(A) `Accepted` 유지 + Amd#2 주석** *(워커 권고)* | D1~D6 방법론이 **폐기되지 않았고**(F4), 39권 일괄 재하베스트의 근거 문서로 계속 필요하다. 실측 기록(C2 해상도·분류, O2 교집합, O4 457장면)도 현역이다 | 전환 대상 0권인데 `Accepted`라 **오독 여지** — Amd#2를 Status에 명시해 완화 |
| (B) `Superseded by ADR-0056` | 실행 대상이 0권이 된 상태를 가장 직관적으로 표현 | **F4가 무효화되어 보인다.** 재하베스트 시 "폐기된 ADR"을 되살려야 하는 모순. ADR-0056은 이미지 트랙을 대체하지 **않으므로** 엄밀히는 supersede가 아니다 |
| (C) `Accepted (실행 종료)` 같은 복합 표기 | 사실에 가장 가까움 | 프로젝트에 선례가 없는 표기 — 일관성 훼손 |

- **워커 권고는 (A)** 였다. 사유: ADR-0056은 **`book_text`·오디오 트랙만 대체**했고
  **이미지 해상도 트랙은 대체하지 않았다.** supersede는 후자까지 포함할 때 쓰는 표기다.
- **2026-08-10 팀장이 (A)를 채택**했다. 권고와 일치한다.

### F7. 철회 경로 — ADR-0056 O6 실패 시 → ✅ **불발동 종결 (2026-08-10)**

> **2026-08-10: ADR-0056 O6가 Resolved**됐다(5권 65객체 전부 정상 로드). 따라서 본 철회 경로는
> **발동하지 않고 종결**된다 — D9-b 5권은 ADR-0056 경로에 확정 편입되고, **D2 전환 대상은
> 0권으로 유지**된다. F1 철회도 없다. 아래는 조건부 설계분이며 이력으로 보존한다.

- **ADR-0056 O6**(5권 이미지 실재 육안 확인)이 실패하면 **D9-b만 철회**되고,
  **본 Amendment F1도 함께 철회**된다 → 잔여 D2 전환 대상이 **5권으로 복귀**한다.
- 그 경우 E4-a **(가)** 가 채택된 것과 같은 상태가 되며, **F4 덕분에 D2 절차가 그대로 살아 있어
  즉시 진행 가능**하다. 별도 복구 작업이 없다.
- **철회 단위는 5권 전체다.** 권 단위 부분 철회는 하지 않는다 — 5권이 같은 근거(`DONE_BOOKS`
  일괄 업로드)에 서 있어 하나가 부재면 나머지도 재확인이 필요하기 때문이다.

---

*ADR-0054 끝. Accepted 2026-08-05 / Amendment #1·#2 2026-08-08.*
*Amd#1: **O2 실질 해소** · **E4 확정**(잔여 활성 5권).*
*Amd#2: **E4-a (나) 확정** → **D2 활성 전환 대상 0권** · **O4 소진**(F3) · **D2 자산 무폐기**(F4).*
*2026-08-10: **F6 Resolved** — 팀장 결정 `Accepted` 유지(해상도 트랙 미대체).*
*2026-08-10: **F7 불발동 종결** — ADR-0056 O6 Resolved(65객체 전부 로드)로 5권 철회 경로 소멸.
D2 전환 대상 0권 확정 유지.*
*잔여 미결: **E7 O-c**(백로그 8번 원문 — 비차단).*
