...
**잔여 전환 대상 권수는 미확정**(하한 5권)이다 — 확인 방법은 Amendment #1 E4 참조.
...
- **제외 대상**: `source_platform='book_dash'` AND `content_type='html'` AND `kind='page'`
  오디오 보유 AND `book_text` 0행 AND `is_active=true` → **34권 408면**
  (팀장 SQL 실측 2026-08-08). 정의의 정본은 **ADR-0056 D9**다.
...
### E3. O2 상태 갱신 — 34권 범위 해소, 잔여 범위로 축소

| 시점 | O2 상태 |
|---|---|
| 2026-08-05 (Accepted) | ✅ 규모 실측 완료(33권) / ⏸ **처리 방안 미확정 — step3 게이트 유지** |
| **2026-08-08 (Amd#1)** | ✅ **활성 34권 범위: 해소** — 제외되어 재매핑 대상이 아님<br>⏸ **잔여 대상 범위: 게이트 유지** — 잔여 중 오디오 보유 권이 있으면 처리 방안 미확정 그대로 |

- **게이트는 폐지되지 않는다.** O2 본문의 불변 문구("이 결정이 내려지기 전에는 step3 전환
  SQL을 실행하지 않는다")는 **잔여 대상에 대해 그대로 유효**하다.
- 부록 A의 재확인 SELECT는 **그대로 사용하되 조건 1개를 추가**해야 한다 —
  ADR-0056 D9로 이관된 34권을 결과에서 빼야 하므로, `targets` CTE에
  `AND NOT (b.is_active = true AND EXISTS(SELECT 1 FROM book_audio a2 WHERE a2.book_id = b.id AND a2.kind='page'))`
  에 상당하는 제외를 넣는다. **정확한 최종 형태는 E4 해소 후 확정**한다.
- 종전 O2 표의 "**전환 대상 38권 ∩ 오디오 보유 = 33**"은 **폐기하지 않는다.**
  2026-08-05 시점 실측으로 유효하며, E4의 산술 근거로 계속 쓰인다.

### E4. 잔여 전환 대상 권수 — **미확정 (하한 5권). 단정하지 않는다**

**확정할 수 없는 이유**: D2 대상 38권은 **블랙리스트 기준**으로 정의된 집합이고
(§D1: 54 − 블랙 15 − `maddy-moona` 1), 제외되는 34권은 **`is_active` 기준**으로 정의된 집합이다
(ADR-0056 D9). **두 기준의 교집합 크기는 `is_active` 값을 봐야만 알 수 있고, 그 값은 DB만 안다.**

단순 뺄셈 `38 − 34 = 4`는 **성립하지 않는다** — 34권이 38권의 부분집합이 아니기 때문이다.
`maddy-moona`(hold, 38권 밖)와 블랙리스트 보유 오디오 10권(38권 밖)이 34권에 섞여 있을 수 있다.

**산술로 좁힐 수 있는 범위** (본 ADR O2 실측 + 팀장 2026-08-08 실측):

```
오디오 보유 html 44권 = 38권 내 33  +  maddy-moona 1  +  블랙리스트 10
활성 34권 = a + b + c        (a ⊂ 33,  b ⊂ {maddy-moona},  c ⊂ 블랙 10)
잔여 전환 대상 = 38 − a = 38 − (34 − b − c) = 4 + b + c
```

- **하한 5권** — `a ≤ 33`이므로 `b + c ≥ 1`. 즉 잔여는 **최소 5권**이다.
  이 5권은 **무오디오 5권**(`a-beautiful-day` · `a-dancers-tale` · `a-fish-and-a-gift` ·
  `a-house-for-mouse` · `a-tiny-seed`)에 대응한다 — 오디오가 없으므로 34권에 들어갈 수 없고,
  따라서 **어떤 경우에도 D2 대상으로 남는다.**
- **상한 15권** — `b ≤ 1`, `c ≤ 10`이므로 `잔여 ≤ 15`.
- **추정: 블랙리스트 15권은 `lib/shared/blacklist.ts:32-33`에 따라 설계상 `is_active=TRUE`로
  유지되므로**("주간 cron이 `is_active=True`로 되돌리므로 `is_active=false` 대신 코드 측
  블랙리스트로 차단한다 — cron-proof") **`c`가 10에 가까울 가능성이 있다.**
  그 경우 잔여는 14~15권이 된다. **이는 추정이며, 적재·전환 근거로 쓰지 않는다.**

**확인 방법 (팀장 SQL 1회, 읽기 전용)** — 아래 3값을 산출하면 잔여가 확정된다.

```sql
-- ADR-0054 Amd#1 E4 — 잔여 전환 대상 확정 (읽기 전용, SELECT만)
-- ① D2 대상 38권 중 ADR-0056 D9로 이관되지 않고 남는 권수 = 잔여 전환 대상
WITH targets AS (   -- 부록 A의 targets CTE와 동일(블랙 15 제외 + maddy-moona 제외)
  SELECT b.id, b.title, b.content_url, b.is_active
  FROM books b
  WHERE b.source_platform = 'book_dash'
    AND b.content_type    = 'html'
    AND b.source_id NOT IN ( /* 부록 A의 블랙리스트 15 UUID 그대로 */ )
    AND b.content_url NOT LIKE '%/maddy-moona/%'
)
SELECT
  COUNT(*) AS d2_targets_38,
  COUNT(*) FILTER (
    WHERE t.is_active = true
      AND EXISTS (SELECT 1 FROM book_audio a WHERE a.book_id = t.id AND a.kind = 'page')
  ) AS moved_to_adr0056,           -- ADR-0056 D9로 이관되는 권수 (= 위 산식의 a)
  COUNT(*) - COUNT(*) FILTER (
    WHERE t.is_active = true
      AND EXISTS (SELECT 1 FROM book_audio a WHERE a.book_id = t.id AND a.kind = 'page')
  ) AS remaining_d2                -- ★ 잔여 전환 대상
FROM targets t;
```

**검산 기준**: `d2_targets_38 = 38` · `remaining_d2 = 4 + b + c` 이며 **5 ≤ `remaining_d2` ≤ 15**.
범위를 벗어나면 그 자체가 정지 사유다(모집단 정의가 어긋났다는 뜻).
아울러 ADR-0056 **O4**(활성 34권 확정 명단)와 **같은 회차에 함께 조회**하면 두 값이
서로 검산된다 — `moved_to_adr0056 = a` 이고 `a + b + c = 34`여야 한다.

**확정 전 금지 사항**: 잔여 권수가 확정되기 전에는 **D6 드라이런 대상 목록을 만들지 않는다.**
잘못된 모집단으로 드라이런하면 그 산출이 O4(슬라이드 총 수)를 잘못 고정한다.

### E5. O4(전환 후 슬라이드 총 수) — **재산정 필요**

- 종전 추정 "38권 × 14면 = **532면**"은 **모집단이 38권일 때의 값**이다. E1로 모집단이 축소되므로
  **이 추정치는 무효화된다.**
- 새 추정은 **E4 확정 후** `잔여 권수 × 14면`으로 다시 세우며, **확정은 D6 드라이런 산출로**
  한다(종전과 동일하게 Amendment로 고정). O4의 "미해소" 상태는 유지된다.
...
- **D3 DB 영향 · D4 3중 백업 롤백 · D5 오디오 상한 회계 · D6 드라이런 게이트** — 유지.
  - ⚠ **D5 회계표의 "① 본 ADR — 38권 전환 완료 시 128 + 38 = 166"은 재산정 대상**이다.
    34권이 본 ADR이 아니라 ADR-0056 경로로 오디오를 갖게 되므로, 기여분의 **출처가 갈릴 뿐
    최종 상한 190권은 변하지 않는다**(34권은 어느 경로로든 오디오를 얻는다).
    **추정: 정확한 재분배는 E4 확정 후 산출 가능**하며, 본 Amendment는 수치를 고치지 않는다.
- **V1~V5 검증 계획** — 잔여 대상에 그대로 적용. V4 표본 5권 목록(`a-beautiful-day` ·
  `a-house-for-mouse` · `bathtub-safari` · `come-back-cat` · `why-is-nita-upside-down`)은
  **재선정이 필요**하다 — 이 중 일부가 34권으로 이관될 수 있다. **E4 확정 후 재선정한다.**
...
*잔여 O2(처리 방안·잔여 범위)·O4(슬라이드 총 수)·E4(잔여 권수)·E7 O-c는 후속 산출로 고정한다.*
