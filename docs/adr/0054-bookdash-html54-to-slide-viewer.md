...
살아 있다(F4). 본 ADR 전체의 **Status 변경(Superseded 등)은 팀장 결정 대기**(F6).
...
### F6. 본 ADR의 Status — **제안만 하고 팀장 결정에 맡긴다**

**워커는 Status를 임의 변경하지 않았다.** 현재 `Accepted (2026-08-05)` 그대로다.
판단 재료만 기록한다.
...
- **워커 권고는 (A)** 다. 사유: ADR-0056은 **`book_text`·오디오 트랙만 대체**했고
  **이미지 해상도 트랙은 대체하지 않았다.** supersede는 후자까지 포함할 때 쓰는 표기다.
- **결정은 팀장이 한다.** 본 Amendment는 Status를 변경하지 않았다.
...
*잔여 미결: **F6**(본 ADR Status — 팀장 결정) · **ADR-0056 O6**(5권 이미지 육안 확인 — F7 연동) ·*
