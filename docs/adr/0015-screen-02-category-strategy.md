...
## Amendment #1 (2026-05-21 phase-10 CP2-a)

CP2-a 진단에서 `lib/home/categories.ts`에 분포(distribution) 함수 1개를 추가하기로 결정했다. 본 ADR §6 "다중 매칭 과다" 위험 보완과 결정 7 "phase-13b 정식 컬럼 도입 트리거"의 baseline 확보가 목적이다.

### 추가 함수

```ts
// lib/home/categories.ts (CP2-b 신규)
export async function getCategoryDistribution(
  supabase: SupabaseClient,
): Promise<Record<CategorySlug, number>>;
```

- 각 카테고리에 매칭되는 활성 책 권수만 반환 (0건 카테고리 식별 가능 → 결정 6 폴백 UI 적용 대상 자동 검출)
- **정확도(accuracy)가 아닌 분포(distribution)** 측정. ground truth 없이 측정 가능
- phase-13b Admin "키워드 풀 정확도 측정 도구"(§7 후속 과제 4)는 본 함수 출력을 baseline 데이터로 활용. 운영자가 카테고리별 책 표본을 추출할 때 모집단 크기를 본 함수에서 확보한다.

### 정확도 측정과의 구분 (명시)

- 본 함수는 결정 7 트리거 임계 "(a) 정확도 ≤ 70%" 측정 도구가 **아니다**. 분포만 본다.
- 정확도 측정은 사용자(또는 큐레이터)가 표본 100건을 라벨링하는 외부 검토를 필요로 한다 — phase-13b Admin에서 구축.
- 본 함수는 결정 7 트리거 임계 "(b) 카테고리 클릭률 ≥ 10%" 측정과도 무관(클릭률은 Vercel Analytics 등 별도 인프라).

### 호출 예상 위치

| 위치 | 사용 시점 | 비고 |
|---|---|---|
| 홈 페이지 (`app/home/page.tsx`) | CategoryGrid 렌더링 직전 1회 | 분포 0건 카테고리에 "현재 책 없음" 표시 가능(선택, CP3-a 결정) |
| phase-13b Admin | 운영자 정확도 검토 모집단 표시 | 본 함수 출력을 카테고리별 모집단으로 사용 |

본 Amendment #1은 §1~§7 본문을 변경하지 않는다.
