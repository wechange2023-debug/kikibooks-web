...
## Amendment #7: ASb 뷰어 넘김 3경로 + 본문 alt 채택 (2026-06-19)
상태 Accepted

배경: 4-viewport 검수 통과(명세 위반 없음). 단 모바일·태블릿에서 스와이프 넘김 부재로
타 유아 서비스(Epic·Vooks 등) 대비 사용성 격차. 본문 이미지 alt 빈값(장식 취급) 접근성 보완 필요.

결정:
- D1. 넘김 3경로(버튼+키보드 ←/→ +터치 스와이프) 채택. 상세는 intent §8.1.
- D2. 본문 일러스트 alt를 제목 기반 대체텍스트로 채택. 상세는 intent §8.2.
- D3. 전환 애니메이션·줌·관성은 비범위(디자인 리뉴얼 트랙으로 이관).

영향: asb-reader.tsx에 키보드 핸들러·터치 핸들러·alt 수정 추가(코드는 별도 작업지시서).
페이지 구성(Amd#6)·완독(FinishButton)·반응형은 변경 없음.
