...
- **[결정]** `components/app/app-header.tsx`의 `NAV_LINKS`는 **이미 3링크**다(홈 / 라이브러리 / **쇼케이스(검수용)**). 따라서 `/mypage` 추가 시 **4링크**가 된다. ADR 본문 **D6·3절 Negative의 "홈·라이브러리 2링크 → 3링크" 기술은 구정보이며 본 항목으로 정정**한다. 쇼케이스 링크는 **팀장 수동 검수에 사용 중이므로 이번 범위에서 제거하지 않는다.** 390px 반응형 확인은 **"실사용 3 + 검수용 1 = 4링크" 기준**으로 수행하며, 이를 구현 단계 필수 항목으로 지정한다. 넘칠 경우 **1순위 대응은 라벨 축약**, 2순위는 쇼케이스 링크 제거(별도 판단).
- **[근거]** 검수 트랙이 진행 중인 상태에서 검수 도구를 제거하면 손실이 크고, 쇼케이스는 서비스 전환 시 어차피 제거 예정이므로 그 시점에 자연 해소된다. 한편 진입 동선이 없으면 마이페이지에 도달할 수 없으므로 링크 추가 자체는 필수다. 아이콘 전환·드로어 전환은 표면을 키우므로 라벨 축약을 우선한다(Hard Rule 10 semantic 토큰 준수).
