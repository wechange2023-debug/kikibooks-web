...
#### G6-a. 발동 조건은 '후보' 선별이다 — 실제 필요 여부는 드라이런이 확정한다 (2026-08-18 보강)

위 조건(`book_audio` 행이 있는 도서의 본문 수정)에 걸린 도서는 **재합성 후보**다. 착수 전에
**처리기 드라이런의 좌표 정합 판정**으로 실제 필요 여부를 확정한다.

```
PYTHONUTF8=1 python scripts/tts_pilot/process_tts_requests.py
```

- `[⚠ 좌표 경고]` 가 뜬 면이 있으면 → **재합성 대상**이다.
- `[좌표] 전 유닛 정합 ✅` 이면 → **재합성 불요**. 여기서 멈춘다.

**공백·개행만 수정한 경우 재합성이 불필요할 수 있다.** `sanitize`가 연속 공백·개행을 접으므로
원문의 공백만 손대면 Polly 입력이 그대로이고, 따라서 음원과 marks도 그대로다.
`book_text.updated_at`이 `book_audio.created_at`보다 나중이라는 사실은 **그 행에 UPDATE가
있었다**는 뜻일 뿐, 정제 후 텍스트가 달라졌다는 뜻이 아니다.

> **근거 — 2026-08-18 `catch-that-cat` 실증.** `updated_at > created_at`만 보고 재합성했다
> (13행 삭제 → run `20260818-152853` → `--upload --overwrite` 26객체 → 13행 적재).
> 결과물은 옛 음원과 **바이트 단위로 동일**했다: Storage 객체 크기 26건 전건 무변동 ·
> 옛 적재 SQL의 `duration_ms` ↔ 새 manifest `out_ms` **13/13 일치** · marks 크기 무변동.
> 비용 $0.1542는 학습 비용으로 종결(팀장 판단). 판별 SQL
> `scripts/sql/trackB_catchthatcat_audit.sql` 헤더에 같은 내용을 박아 두었다.

G6의 결론 자체는 바뀌지 않는다 — **TTS 완료 도서의 텍스트 수정은 재합성 검토 대상**이다.
바뀐 것은 "검토 대상"과 "확정"을 가르는 2단 판별 절차가 명시됐다는 점이다.
