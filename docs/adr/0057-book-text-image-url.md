...
**2026-08-13 D1~D5 실행 완결.** 결정문 전항이 실행됐다. 아래는 실측 결과다.

| 결정 | 실행 | 실측 |
|---|---|---|
| **D1** 컬럼 신설 | `supabase/migrations/008_book_text_image_url.sql` 실행(팀장) | `image_url text` nullable 생성 |
| **D5-①** book_dash 백필 | 순수 SQL UPDATE(팀장) | **191권 2,597행** |
| **D5-②** ASb·Bloom 백필 | `docs/sql/image_backfill/` step0→chunk 9→step2→step3→step4 | **669권 6,780행** (asb 5,211 · bloom 1,569) |
| **D5-④** 완료 게이트 | 실측 COUNT | `not_null` **9,377** / `NULL` **119** / `bad_url` **0** / 총행 9,496·860권 불변 |
| **D2·D3·D4** 코드 전환 | 커밋 `e034dd7` (4파일 +103/−51) | 타입·린트·빌드 통과 · **팀장 브라우저 실측 5/5** |

- **NULL 119행은 정상 결손**이다(ASb 30권 — 매니페스트 이미지 수 < 텍스트 수, ADR-0025 Amd#6 A3).
  권별 배분이 `docs/sql/image_backfill/per_book.csv`의 `missing` 열과 1:1 일치함을 D5-④ (c)로 확인했다.
- 백필 원천이 **ADR-0056 §Context 3의 독립 드라이런(2026-08-08)과 전 항목 폐합**했다 —
  ASb `axis_diff` 분포가 버킷 단위로 일치하고 텍스트 5,330·이미지 5,729·Bloom 1,569/1,569도 일치한다.
  → 2026-08-10 적재 이후 매니페스트 무변경 = `page_index` 축 정합 확인(ADR-0056 **O2 리스크 불발현**).
- **본 ADR이 ADR-0056의 미결 2건을 닫는다**: D14(현행 조립 유지 — 잠정 조치) · O-d(결손 표현).
- 잔여 Open은 **O-2**(NULL 면 폴백 문안의 카피 중앙화) · **O-3**(GDL, 범위 밖) 두 건이며 **둘 다 비차단**이다.
- 결정문(D1~D5)은 `Accepted` 상태 그대로 유효하며, 본 표기는 **실행 트랙의 종결**을 뜻한다.
