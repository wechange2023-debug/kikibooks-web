...
## Amendment #4 (2026-06-25) — 본문 텍스트 레이어 + 표지 호스트 보정

### 배경
검증책 1권 브라우저 검수에서 (1) 본문 영어 텍스트 누락 (2) 표지 미표시가
드러났다. STEP9의 "asb_native 재사용" 결론은 이미지 시퀀스만 비교했고
텍스트 레이어를 누락했다. 재정찰(STEP20·21)로 원인·수정범위를 확정해 보정한다.

### 결정
1. 본문 텍스트(page_text) 채우기 — 매니페스트만 수정:
   - Bloom 책은 numberedPage마다 lang="en"(bloom-contentNational1) 영어
     텍스트를 HTML로 보유(표본 3/3 전수 확인).
   - 조각2 매니페스트 합성에서 각 numberedPage의 lang="en" 텍스트를 추출,
     P<n>\t<영어문장> 형식으로 page_text: 섹션에 채우고 images:와 인덱스 정렬.
   - 주 언어(oks 등)는 버리고 en만 채택(영어 라이브러리 목적).
   - AsbReader·asb-parser는 이미 page_text 렌더 지원(ASb 프로덕션 실증) → 코드 변경 0건.
2. 표지 호스트 허용 — next.config.js 수정:
   - 표지 미표시 원인 = next/image remotePatterns에 Bloom S3 호스트 부재
     → onError 플레이스홀더. (본문은 평문 img라 표시됨)
   - next.config.js images.remotePatterns에 Bloom S3 호스트 추가.
     보안상 s3.amazonaws.com 전체가 아니라 Bloom 버킷 경로로 제한
     (pathname: /bloomharvest/** 및 /BloomLibraryBooks/**).

### 영향
- 조각2 매니페스트에 page_text 추가 → 검증책 재생성 + 재검수 후 배치 진행.
- next.config.js 변경은 Vercel 재배포 시 반영(표지 렌더).
- 텍스트 없는 책(en editable 빈 페이지)은 해당 면 텍스트 공란으로 둠(이미지만).

### 미해결
- en 텍스트가 일부 페이지만 있는 책의 처리(부분 누락)는 배치 드라이런에서 분포 확인.

## Amendment #5 (2026-06-25) — 인코딩 정정 + AI 탐지 확대 + 무텍스트 게이트

### 배경
첫 50권 적재 후 표본 검수에서 (1) 본문 텍스트 mojibake(â€™ 류) (2) AI생성
책이 자동제외 통과("THE WHISPERS OF BLUE FOG") (3) 영어 텍스트 0면 책 혼입이
드러났다. STEP24 정찰로 원인을 실측 확정해 보정한다.

### 결정
1. 인코딩 정정 (버그):
   - 원인: index.htm은 정상 UTF-8이나 HTTP 헤더에 charset이 없어 requests가
     ISO-8859-1로 오판 → r.text가 mojibake.
   - 수정: fetch_index_html에서 resp.encoding="utf-8" 명시(또는
     resp.content.decode("utf-8")). 방어적으로 추출 텍스트에 html.unescape 적용.
2. AI 탐지 확대 (Amd#3 보강):
   - 원인: data-creator="AI"(단독)를 _AI_AUTHOR_RE가 미탐지(gemini ai·chatgpt
     등만 매칭). 이미지명도 타임스탬프라 미매칭.
   - 수정: 단독 \bAI\b 단어경계 시그널 추가 + is_ai_generated가 모든
     data-creator 값을 스캔(현재 최빈값 1개만). 규모 표본 ~5%(모수 ~38권).
3. 무텍스트 게이트 (신규):
   - 영어 텍스트 0면(이미지-only) 책은 영어 라이브러리 가치 낮음(표본 ~5%) →
     자동 스킵. (gate②: en_text_pages == 0 → skip + 로그)

### 영향
- 첫 50권(검증책 포함)은 깨진 텍스트·AI·무텍스트 혼입 → 전량 삭제 후
  수정 파이프라인으로 재적재(STEP26~).
- 최종 순신규 권수는 AI확대·무텍스트게이트·dedup2 반영해 재집계로 확정.

### 미해결
- 글이 이미지에 그려진 정당한 책이 무텍스트 게이트로 소수 탈락 가능 — 베타
  규모(수백권) 대비 수용 가능한 손실로 판단.
