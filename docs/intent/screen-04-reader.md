...
### ★ 자녀 0명 정책 (ADR-0064, 2026-08-20 개정)

미로그인은 가드 2로 차단한다. **자녀 0명은 차단하지 않는다** — 자녀 등록은 선택이며(ADR-0064 A안) 열람은 자녀 유무와 무관하게 허용된다. 본 페이지의 가드에 자녀 항목을 추가하지 않는다.

대신 **기록계 기능만 숨긴다**:

- **읽기 세션 시작** — `getActiveChild`가 null이면 세션 쓰기를 **조용히 건너뛰고** ok를 돌려준다(읽기 자체는 가능, 세션 미기록). 종전과 동일.
- **완독 버튼(FinishButton)** — 자녀 0명이면 **렌더하지 않는다**(ADR-0064 **D4**). 종전에는 무조건 렌더돼 누르면 "자녀 정보를 찾을 수 없습니다" 에러가 떴다 — 세션 시작은 조용히 건너뛰는데 완독만 에러를 내는 **비대칭**이었고, 버튼을 숨겨 대칭으로 맞췄다. 서버의 에러 반환은 안전망으로 유지한다.
- 따라서 자녀 0명은 **책을 끝까지 읽을 수 있지만 완독 처리는 하지 않는다** — 포인트·배지·`/celebrate`는 자녀에게만 쌓인다.
...
## §8 ASb 자체 렌더 뷰어 (asb_native) — 넘김·접근성 명세 (2026-06-19 추가)

배경: §1~§7은 iframe 리더(Book Dash·GDL) 기준으로 작성됨(2026-05-22).
ASb 자체 렌더(content_type='asb_native')는 우리가 직접 페이지를 그리므로 넘김 UI도 우리 책임.
본 절은 ASb 뷰어의 넘김·접근성 요구를 명세한다. 페이지 구성 규칙은 ADR-0025 Amd#6을 따른다.

§8.1 페이지 넘김 — 다음 3경로를 모두 지원한다(OR 관계, 어느 것으로도 넘김 가능):
  - 하단 좌우 화살표 버튼 (현재 구현됨, 유지)
  - 키보드 ← / → (왼쪽=이전, 오른쪽=다음)
  - 터치 스와이프 (좌로 밀기=다음, 우로 밀기=이전) — 모바일·태블릿 대상
  양 끝(첫 면/끝 면)에서 해당 방향은 무효 처리(현 버튼 disabled 동작과 동일하게).

§8.2 접근성:
  - 본문 일러스트 이미지는 의미 전달 요소이므로 alt를 빈 문자열이 아니라 책 제목 기반 대체텍스트로 제공한다
    (예: "{제목} 삽화"). 표지면 alt는 기존 제목 유지.
  - 키보드 넘김 동작 시 현재 페이지 인디케이터(aria-live)가 변경을 읽어주는 현 동작을 유지.

§8.3 비범위(이번 명세에서 제외):
  - 페이지 전환 애니메이션·제스처 관성·줌은 본 명세 밖. 디자인 리뉴얼 트랙에서 별도 검토.
