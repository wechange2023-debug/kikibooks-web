...
## 0. ★ 현행 정정 (2026-08-20) — 이 절이 아래 본문보다 우선한다

본 문서는 phase-10 시점(2026-05-21)에 쓰였다. 그 뒤 ADR 두 건이 전제를 바꿨으므로,
아래 본문에 남은 `/home`·자녀 가드 서술과 충돌하면 **본 절이 정본**이다.

### (a) `/home` 라우트는 폐지됐다 — ADR-0062 D2

홈은 더 이상 별도 라우트가 아니다. **메인 `/` 하나로 흡수**됐고, `/home`은
`next.config.js`의 **308 영구 리다이렉트**로 `/`에 도착한다(기존 북마크 보호).
따라서 아래 본문의 모든 `/home`은 **`/`로 읽는다.** `PROTECTED_PREFIXES`에서도
제거됐으며(`lib/auth/routes.ts`), `/`는 **공개 라우트**다 — 로그인 상태에 따라
블록만 바꿔 렌더한다. 개인화 블록(인사·레벨·추천·카테고리·스트릭)의 내용은
본문 그대로이며, 담는 그릇만 `/home` → `/`로 바뀐다.

### (b) ★ 자녀 0명 정책 — ADR-0064 (자녀 등록은 **선택**이다)

**자녀 0명은 오류 상태가 아니라 정상 상태다.** 공부방·학원 선생님처럼 자녀가
없는 성인도 서비스를 쓴다. 종전의 "자녀 0명 → `/onboarding` 강제 리다이렉트"는
폐기된다.

| 상태 | `/` 도착 시 | 근거 |
|---|---|---|
| 비로그인 | 랜딩 블록(`AnonymousMain`) | ADR-0062 D1 |
| 로그인 + 자녀 0명 | **자녀 미등록 블록(`NoChildMain`)** — 리다이렉트 0회 | **ADR-0064 D1** |
| 로그인 + 자녀 ≥ 1 | 개인화 블록(`MemberMain`) | ADR-0062 D1 |

**자녀 0명이 보는 것**: 상단 히어로("자녀 프로필 만들기" CTA → `/onboarding`) +
인기 책 + 카테고리. 그것으로 `/library`·책 상세·뷰어까지 전부 도달한다.

**자녀 0명이 보지 못하는 것(기록계 기능 — 일괄 숨김)**: 오늘의 추천 · 레벨 선택기 ·
주간 스트릭 · 완독 버튼 · 즐겨찾기 하트. 전부 **자녀 단위 데이터**를 쓰기 때문이며,
화면별로 따로 판단하지 않는다(ADR-0064 §원칙 절). 새 기능도 "자녀 단위 데이터를
쓰는가"로 묻고, 그렇다면 자녀 0명 화면에서 숨긴다.

**온보딩은 없어지지 않는다** — 첫 만남에만 남는다. 자녀 0명이면 로그인 직후 1회
`/onboarding`으로 안내하고, 거기서 "나중에 할게요"를 누르면 그 사실을 쿠키에
기억해 다음부터는 `/`로 보낸다(ADR-0064 D2·D3). 거절한 사람을 매 로그인마다
같은 화면으로 되돌리지 않는다.
...
| 경로 | 공개/보호 | 비고 |
|---|---|---|
| ~~`/home`~~ → **`/`** | **공개** (§0-a: ADR-0062 D2로 흡수, `/home`은 308 리다이렉트) | 로그인 상태로 **블록만 바꿔** 렌더한다. 자녀 0명은 `NoChildMain`(§0-b: ADR-0064 D1) — **redirect 없음** |
...
**middleware.ts는 여전히 수정하지 않는다** — 미들웨어는 자녀 수를 판정하지 않고, 분기는 도착 지점 1회다(ADR-0011 결정 1 — ADR-0064도 이 원칙을 유지한 채 분기 **내용**만 바꿨다).

단 `routes.ts`는 바뀌었다 — ADR-0062 D3이 `PROTECTED_PREFIXES`에서 `'/home'`을 제거했다(§0-a).
...
2. ~~로그인했으나 자녀 0명 상태로 접근 → `redirect('/onboarding')`~~ **폐기(ADR-0064 D1, §0-b).** 자녀 0명은 되돌려지지 않고 `NoChildMain`을 본다. 미들웨어는 여전히 수정하지 않는다.
...
- **분기는 페이지 컴포넌트에서 1회** — `middleware.ts`는 수정하지 않는다. 단 자녀 0명은 이제 **가드가 아니라 분기**다(§0-b: ADR-0064 D1 — redirect 대신 전용 블록 렌더).
