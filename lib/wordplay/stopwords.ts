...
/**
 * ② 청취 검수 제외분 (27종) — ADR-0065 Amendment #1 §사후 청취 검수.
 *
 ...
 *
 * ★ 2026-08-21 추가분은 **개별 청취 없이 같은 기준을 분류로 적용**한 것이다.
 *   21종을 빼자 그 자리에 같은 부류가 올라왔기 때문이다(`shhh`·`shh` 제외 → `shhhh` 진입).
 *   소리 흉내를 빼면 다음 소리 흉내가 올라오는 구조라, 같은 판정을 반복 적용한다.
 *
 * ★ sh 계열은 **일괄 제외로 종결**했다. `Shhhhh!`(Book Dash) 본문에 있는 변형은
 *   `shh`·`shhh`·`shhhh`·`shhhhh`·`shhhhhh`·`shhhhhhhhh` **6종이 전부**임을 원문에서
 *   확인했으므로(2026-08-21 실측), 남은 3종을 한 번에 넣어 회전을 끝냈다.
 *   한 종씩 빼면 3회전이 더 필요했다.
 */
const LISTENING_EXCLUDED: readonly string[] = [
  ...
  // 2026-08-21 2회전 (3) — 위 21종 제외로 카드에 새로 올라온 같은 부류.
  // 개별 청취 없이 동일 기준(소리 흉내 제외)을 분류로 적용했다.
  'shhhh', 'ahuuun', 'toot',

  // 2026-08-21 3회전 (3) — sh 계열 잔여분 일괄. 원문 변형 6종을 모두 덮어 회전을 끝낸다.
  // ※ `beep-beep`은 넣지 않는다 — `beep`을 유지 판정했으므로 일관성을 지킨다.
  'shhhhh', 'shhhhhh', 'shhhhhhhhh',
];
